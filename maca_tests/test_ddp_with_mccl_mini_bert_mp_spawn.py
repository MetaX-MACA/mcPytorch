...
def ddp_program(rank, world_size, local_batch_size):
    ...
    config = BertConfig(
        vocab_size_or_config_json_file=2,
        hidden_size=2,
        num_hidden_layers=1,
    ...
    for iteration in range(1):
        ...
        input_ids = ids_tensor([global_batch_size, 2], 2).to(rank)
        token_type_ids = ids_tensor([global_batch_size, 2], 2).to(rank)
        position_ids = ids_tensor([global_batch_size, 2], 2).to(rank)

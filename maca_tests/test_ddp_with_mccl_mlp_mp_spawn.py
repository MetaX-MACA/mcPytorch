...
if __name__ == "__main__":
    ...
    if dev_cnt <= 1:
        print("Device count is {} < 2, skip {}".format(dev_cnt, os.path.basename(__file__)))
        exit(0)
    ...
    batch_sizes = [2]
    backend_list = ["nccl"]
    for world_size, batch_size, backend in itertools.product(world_sizse, batch_sizes, backend_list):
        try:
            mp.spawn(ddp_program, args=(world_size, batch_size, backend, ), nprocs=world_size, join=True)

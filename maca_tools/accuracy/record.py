    ...
    if _ctx.record_level == 0:
        ...
        for g_in, grad in zip(g_ins, grad_in):
            ...
            if g_in.shape == grad.shape:
                eps = get_eps(grad.dtype)
                rel_err = cal_rel_err(g_in.float().cpu(), grad.cpu().float())
                mark = "error" if rel_err > eps else "     "
                dev_id = get_tensor_device(grad)
                log(f"{mark} backward rel_err: {rel_err.item()}, device.id: {dev_id}, op: {op_name_} dx shape: {grad.shape}, dytpe: {grad.dtype}, max : {torch.max(grad).item()}, min {torch.min(grad).item()}, sum {torch.sum(grad).item()}", LOG_NAME)
            else:
...
class HookTemplate(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if _ctx.record_level == 0:
            is_inplace_op = "inplace" in kwargs.keys() and kwargs['inplace'] == True
            skip = is_inplace_op or "__" not in op_name.split(".")[-3] and op_name.split(".")[-3][-1] == "_"  # skip inplace op
            skip = skip or "dropout" in op_name.split(".")[-3] or "Dropout" in op_name.split(".")[-3] 
            if not skip:
                recover_hijack()
                ...
                if res_cpu is not None:
                    ...
                    dtype = res.dtype if hasattr(res, "dtype") else None
                    self.compare_tensor(op_name, "output", res, res_cpu, get_eps(dtype))
                else:
                    log(f"..... skip {op_name} output not check as it is None. Maybe the op is a inplace op, or it run fail in cpu", LOG_NAME)
                ops_hijack()
            else:
                log(f"..... skip {op_name} as it's inplace op or with random", LOG_NAME)
        ...
        def move_to_cpu(input):
            if torch.is_tensor(input):
                if input.dtype == torch.float16 or input.dtype == torch.bfloat16 or input.dtype == torch.float:
                    t = input.clone().detach().float().cpu()
                    if input.requires_grad:
                        t = t.requires_grad_(True)
                else:
                    t = input.clone().detach().cpu()
                input_tensors.append(t)
                return t
            elif hasattr(input, "parameters"):
                if peek(input.parameters()) is not None:
                    op = copy.deepcopy(input).to(torch.float32).cpu()
                    if hasattr(input, "training"):
                        op = op.train() if input.training else op.eval()
                    return op
                else:
                    return input
                ...
                return type(input)(move_to_cpu(x) for x in input)
            elif isinstance(input, dict):
                return {key: move_to_cpu(value) for key, value in input.items()}
            else:
                return input

        for arg in args:
            args_cpu.append(move_to_cpu(arg))
        try:
            if hasattr(self._op, "parameters"):
                op_cpu = self._op.cpu()
            else:
                op_cpu = self._op
            ret_cpu = op_cpu(*args_cpu, **kwargs)
    ...
    def compare_tensor(self, op_name, tensor_name, t, t_cpu, eps):
        if torch.is_tensor(t) and t.numel() != 0:
            if "cuda" in str(t_cpu.device):
                log(f"..... warn [{op_name}] not run on cpu, its check result is invalid", LOG_NAME)
                if t_cpu.dtype == torch.float16 or t_cpu.dtype == torch.bfloat16:
                    t_cpu = t_cpu.float().cpu()
                else:
                    t_cpu = t_cpu.cpu()
            rel_err = cal_rel_err(t.cpu().float(), t_cpu.float())
            mark = "error" if rel_err > eps else "     "
            dev_id = get_tensor_device(t)
            log(f"{mark} forward rel_err: {rel_err.item()}, device.id: {dev_id}, op: {op_name}, {tensor_name} shape: {t.shape}, dytpe: {t.dtype}, max : {torch.max(t).item()}, min {torch.min(t).item()}, sum: {torch.sum(t.double()).item()}", LOG_NAME)
        elif isinstance(t, (list, tuple)):
            log(f"..... skip compare with cpu as [{op_name}] output is a list/tuple or a empty tensor", LOG_NAME)
        else:
...
def checkattr(object, attr_name):
    if not hasattr(object, attr_name):
        return False
    if not hasattr(getattr(object, attr_name), "__wrapped__"):
        return False
    return True

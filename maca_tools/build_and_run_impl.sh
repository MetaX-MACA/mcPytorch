...
function main() {
    ...
    flash_attn_filename="${maca_path}/wheel/flash_attn*.whl"
    result=$(ls $flash_attn_filename)
    if [ -z "$result" ]; then
        echo "flash_attn whl does not exist."
    else
    ...
    if [[ ${run_test} != "" ]]; then
        ...
        pip install ${maca_path}/wheel/triton-2.0.0*-cp38-cp38-*.whl
        if [[ ${py_setup_cmd} != "install" ]]; then
            pip install dist/torch-2.0.0*-cp38-cp38-*.whl
        fi

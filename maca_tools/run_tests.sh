...
export SEND_MAIL_SUBJECT="pytorch tests ${1}"
...
if [[ $1 == "daily" ]]; then
    ...
    uset PYTORCH_SKIP_COMPILE_CHECK
...
if [[ $1 == "benchmark" ]]; then
    ...
    pip install pynvml
    pip install ${STORAGE_PATH}/pytorch/${pytorch_package_version}/wheel/torchvision-*.whl
    pip install tqdm
    pip install boto3
...
if [[ $1 == "benchmark_master" ]]; then
    ...
    pip install /home/jenkins_sw_bot/ws_pytorch/tools/nvidia_dali_cuda110-1.27.0-8625303-py3-none-manylinux2014_x86_64.whl
    pip install pynvml
    pip install ${STORAGE_PATH}/pytorch/${pytorch_package_version}/wheel/torch-*.whl
    pip install ${STORAGE_PATH}/pytorch/${pytorch_package_version}/wheel/torchvision-*.whl
    pip install tqdm
    pip install boto3
    pip install requests
...
#    bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh nn ${XML_PATH}
#    if [[ $? != 0 ]]; then
#        ERR=$(expr ${ERR} + 1)
#    fi
#    bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh torch ${XML_PATH}
#    if [[ $? != 0 ]]; then
#        ERR=$(expr ${ERR} + 1)
#    fi
# #    make weekly ci stuck, disable it now
# #    bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh cuda ${XML_PATH}
# #    if [[ $? != 0 ]]; then
# #        ERR=$(expr ${ERR} + 1)
# #    fi
#     bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh optim ${XML_PATH}
#     if [[ $? != 0 ]]; then
#         ERR=$(expr ${ERR} + 1)
#     fi
#    bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh reduction ${XML_PATH}
#    if [[ $? != 0 ]]; then
#        ERR=$(expr ${ERR} + 1)
#    fi
#    bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh mccl_and_gloo_weekly ${XML_PATH}
#    if [[ $? != 0 ]]; then
#        ERR=$(expr ${ERR} + 1)
#    fi
#     bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh onnx ${XML_PATH}
#     if [[ $? != 0 ]]; then
#         ERR=$(expr ${ERR} + 1)
#     fi
#     bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh spectral ${XML_PATH}
#     if [[ $? != 0 ]]; then
#         ERR=$(expr ${ERR} + 1)
#     fi
#     bash ${PYTORCH_ROOT}/test/run_maca_pytest.sh pipeline ${XML_PATH}
#     if [[ $? != 0 ]]; then
#         ERR=$(expr ${ERR} + 1)
#     fi
    bash ${PYTORCH_ROOT}/maca_tests/run_pytest.sh $1 ${XML_PATH}

...
def run_test(
    ...
    if USE_MACA:
        if options.pytest and options.export_report_path:
            global EXPORT_REPORT_FILE
            xml_name = f"maca_test_{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.xml"
            EXPORT_REPORT_FILE = os.path.join(os.getcwd(), options.export_report_path, xml_name)
            argv.extend(["--junit-xml", EXPORT_REPORT_FILE])
...
def parse_args():
    ...
    if USE_MACA:
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help='used together with option "--pytest". Export test report file under the input folder.'
        )
...
def parse_xml_report():
    dom_tree = xm.parse(EXPORT_REPORT_FILE)
    testsuites = dom_tree.documentElement
    meta_data = testsuites.getElementsByTagName("testsuite")
    csv_path = EXPORT_REPORT_FILE.replace('.xml', '.csv')
    print(csv_path)
    with open(csv_path, 'w', newline='') as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(['test_filename', 'test_case_name', 'status', 'err_info', 'used_time'])
        for mt in meta_data:
            test_case = mt.getElementsByTagName("testcase")
            for tc in test_case:
                test_file = tc.getAttribute("classname")
                pos_l = test_file.find('.')
                pos_r = test_file.rfind('.')
                test_file = test_file[pos_l + 1:pos_r]
                row_it = [test_file, tc.getAttribute("name")]
                used_time = tc.getAttribute("time")
                is_pass = True
                skipped = tc.getElementsByTagName('skipped')
                if len(skipped) > 0:
                    is_pass = False
                    row_it.append("skipped")
                    skip_info = skipped[0].getAttribute('message')
                    row_it.append(skip_info)
                failure = tc.getElementsByTagName('failure')
                if len(failure) > 0:
                    is_pass = False
                    row_it.append("failed")
                    err_info = failure[0].getAttribute('message')
                    row_it.append(html.unescape(err_info).split('\n')[0])
                                # crashed
                error = tc.getElementsByTagName('error')
                if len(error) > 0:
                    is_pass = False
                    row_it.append("crashed")
                    err_info = error[0].getAttribute('message')
                    row_it.append("crashed while running")
                if is_pass:
                    row_it.extend(["pass", ''])
                row_it.append(used_time)
                csv_writer.writerow(row_it)
...
def main():
    ...
    test_directory = os.getenv("PYTORCH_MACA_TEST_DIRECTORY") if os.getenv("PYTORCH_MACA_TEST_DIRECTORY") else str(REPO_ROOT / "test")
    ...
    try:
        if not USE_MACA:
            ...
            selected_tests_serial = selected_tests
        ...
        if USE_MACA:
            # save report to csv
            if EXPORT_REPORT_FILE is not None and os.path.exists(EXPORT_REPORT_FILE):
                parse_xml_report()

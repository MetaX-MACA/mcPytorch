...
def load_skip_config():
    '''if exist 'MACA_SKIPPED_TESTS_FILE', would load it.
    Support skip test class or single test case
    '''
    disabled_tests_file = os.getenv("MACA_SKIPPED_TESTS_FILE", "")
    if disabled_tests_file and os.path.exists(disabled_tests_file):
        with open(disabled_tests_file) as f:
            return json.load(f)
    return {}
...
def pytest_collection_modifyitems(items: list[Any]) -> None:
    ...
    # USE_MACA
    if USE_MACA:
        skip_config = load_skip_config().get("skip_tests", None)
        if skip_config:
            for item in items:
                test_file = os.path.basename(item.location[0])
                test_name = item.name
                test_class = item.parent.name
                if test_file in skip_config and (test_class in skip_config[test_file] or test_name in skip_config[test_file]):
                    item.add_marker(pytest.mark.skip(reason="Skipped via maca platform not supported temporarily!"))

...
"""
    ...
    def test_cublas_workspace_explicit_allocation(self):
        ...
        # USE_MACA
        elif torch.version.maca:
            if "PYTORCH_FORCE_BLAS_WORKSPACE_32M" in os.environ:
                default_workspace_size = 4096 * 8 * 1024
            elif "PYTORCH_DISABLE_BLAS_WORKSPACE_DEFAULT_32M" in os.environ:
                default_workspace_size = 4096 * 1024 * 2 + 16 * 1024 * 8
            else:
                default_workspace_size = 4096 * 16 * 1024
        else:
...
""",
    ...
    @unittest.skipIf(USE_MACA, "TODO(): USE_AMCA not support")
    ...
    def test_graph_concurrent_replay(self):
        ...
        for share_mem in ("Don't share", "via pool()", "via graph_pool_handle()"):
            ...
            with torch.cuda.stream(s1):
                torch.cuda._sleep(10000000)
                e1.record()
                torch.cuda.current_stream().wait_event(e0)
                g1.replay()
...
"""
    ...
    def test_lazy_init(self):
        ...
        if not TEST_WITH_ROCM and not USE_MACA:

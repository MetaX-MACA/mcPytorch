...
for csv_file in ${csv_file_list[@]}; do
    if [ -n "$2" ]; then
        python ./test/run_test.py --pytest --run-specified-test-cases ${PYTORCH_ROOT}/test_report/${csv_file} --export-report-path $2 --continue-through-error -v
    else
        python ./test/run_test.py --pytest --run-specified-test-cases ${PYTORCH_ROOT}/test_report/${csv_file} --continue-through-error -v
    fi
    if [[ $? != 0 ]]; then
        ERR=$(expr ${ERR} + 1)
    fi

...
def get_cycles_per_ms():
    ...
    if _cycles_per_ms is None:
        ...
        _cycles_per_ms = 100 / start.elapsed_time(end)
...
class TestCuda(TestCase):
    ...
    def test_check_error(self):
        ...
        with self.assertRaisesRegex(torch.cuda.CudaError,
                                    "out of memory|hipErrorOutOfMemory|mcErrorMemoryAllocation"):
...
"""], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
    ...
    def test_huge_index(self):
        ...
        src = torch.empty(15000000, 45, dtype=torch.long).random_(0, 2**22).cuda()
        idx = torch.randperm(src.shape[0]).cuda()
        res = src[idx]
    ...
    def test_tensor_scatterAdd(self):
        AbstractTestCases._TestTorchMixin._test_scatter_base(
            self, lambda t: t.cuda(), 'scatter_add_', test_bounds=False)
...
"""])
    ...
    def test_max_large_axis(self):
        ...
        x = torch.zeros(2**32, dtype=torch.int8).cuda()
        x[-1] = 1
        val, idx = x.max(0)
    ...
    @unittest.skipIf(not torch.version.cuda, "TODO(liuyuxin): cuda version not supported!")

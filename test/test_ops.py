...
class TestCommon(TestCase):
    ...
    def _ref_test_helper(
        ...
        for sample in op.reference_inputs(device, dtype, requires_grad=False):
            ...
            if op.name == "_refs.native_layer_norm":
                atol = 0
                rtol = 1e-5
            elif op.name == "_refs.nn.functional.group_norm":
                atol = 1e-5
                rtol = 0
            elif op.name == "_refs.addcmul" or op.name == "_refs.lerp" or op.name == "_refs.linalg.vector_norm":
                atol = 1e-6
                rtol = 1e-6
            elif op.name == "_refs.linspace":
                atol = 5e-6
                rtol = 1e-7
            elif op.name == "_refs.nn.functional.triplet_margin_loss":
                atol = 5e-6
                rtol = 5e-6
            else:
                atol = 1e-6
                rtol = 1e-6
            try:
                if op.name == "_refs.nn.functional.nll_loss" and dtype == torch.float16:
                    self.assertEqual(
                        ref_result,
                        torch_result,
                        exact_stride=False,
    ...
    def test_python_ref(self, device, dtype, op):
        ...
        if (str(op.name) == "_refs.sub" and device == "cpu" and dtype == torch.complex32) or \
                (str(op.name) == "_refs.add" and device == "cpu" and dtype == torch.complex32) or \
                (str(op.name) == "_refs.nn.functional.triplet_margin_loss" and device == "cpu" and dtype == torch.complex64) or \
                (str(op.name) == "_refs.sgn" and device == "cpu" and dtype == torch.complex32) or \
                (str(op.name) == "_refs.special.xlog1py" and device == "cpu" and dtype == torch.float16) or \
                (str(op.name) == "_refs.xlogy" and device == "cpu" and dtype == torch.float16):
            import pytest
            pytest.skip("CUDA A100 alse failed")
    ...
    def test_python_ref_torch_fallback(self, device, dtype, op):
        ...
        if (str(op.name) == "_refs.sgn" and device == "cpu" and dtype == torch.complex32) or \
                (str(op.name) == "_refs.special.xlog1py" and device == "cpu" and dtype == torch.float16) or \
                (str(op.name) == "_refs.add" and device == "cpu" and dtype == torch.complex32)  or \
                (str(op.name) == "_refs.addr" and device == "cpu" and dtype == torch.complex64):
            import pytest
            pytest.skip("CUDA A100 alse failed")
    ...
    def test_dtypes(self, device, op):
        ...
        if str(op.name) == "lu":
            import pytest
            pytest.skip("TODO: CUDA error: an illegal memory access was encountered")

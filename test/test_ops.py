...
class TestCommon(TestCase):
    ...
    def test_python_ref_meta(self, device, dtype, op):
        #fft half and complex32 not supported
        if "fft" in str(op):
            return
        CHECK_CONJ_SKIPS = {
    ...
    def test_complex_half_reference_testing(self, device, dtype, op):
        if "fft" in str(op) or "nanmean" in str(op) or "nansum" in str(op):
            return
...
class TestCompositeCompliance(TestCase):
    ...
    #(TODO MACA) Copy On Write feature doesn't affect the calculation correctness
    # @ops(op_db, allowed_dtypes=(torch.float,))
...
class TestRefsOpsInfo(TestCase):
    ...
    #(TODO MACA) this should be opened after math failed cases get fixed

...
本工程在 PyTorch 2.0 的基础上增加了对沐曦 (MetaX，https://www.metax-tech.com/) GPU 的支持。MetaX GPU 提供了和 cuda 生态高度兼容的软件栈，包括驱动、编译器以及各类算子库，用户可以使用和 cuda 类似的支持方式完成对 PyTorch 2.0 的 MetaX GPU backend 支持。
...
#### 1.1.1 下载镜像
  登录[沐曦开发者社区](https://developer.metax-tech.com/developer/index)，在搜索栏中搜索“pytorch"，找到相关的镜像包，如下图所示。选择 mxc500-torch2.0-py38-mc2.32.0.3-ubuntu20.04-amd64.container.xz 的镜像包并下载。这里以 torch2.0 python38 的镜像环境为例，mc2.32.0.3 是 maca 的版本，数字越大表示版本越新，建议尽量选择较新的版本，另外如果有 torch2.0 的镜像可以直接选择 torch2.0 的镜像即可，如果没有 torch2.0 的镜像文件，可以使用相近版本的镜像（比如 torch2.1），然后下载 torch2.0 的安装包安装即可。
  
  ![](./docs/source/_static/img/docker.png)
...
``` shell
docker load -i mxc500-torch2.0-py38-mc2.32.0.3-ubuntu20.04-amd64.container.xz
docker run -it --device=/dev/dri --device=/dev/mxcd --group-add video mxc500-torch2.0-py38-mc2.32.0.3-ubuntu20.04-amd64 /bin/bash
```


### 1.2 发布包

  如果已经有对应torch版本的镜像文件，则无需单独下载安装包。如果没有，则可以登录[沐曦开发者社区](https://developer.metax-tech.com/developer/index)，在搜索栏中搜索“pytorch2.0"，找到相关的安装包，如下图所示。2.32.0.3 是 maca 的版本，这里建议选择和运行环境相同的 maca 版本，以免出现兼容性问题。

  ![](./docs/source/_static/img/whl.png)

```shell
python -m pip install torch-2.0.0*.whl
```
...
进行构建。构建成功结束后，可以在 mcPytorch/dist 目录下找到以 "torch-2.0.0*-linux_x86_64.whl" 的 wheel 包安装文件。

...
本工程在 PyTorch 2.10.0 的基础上增加了对沐曦 (MetaX，https://www.metax-tech.com/) GPU 的支持。

## 1 获取构建和运行镜像

构建和运行需要首先完成 MetaX 软件栈的环境准备，包括驱动，编译器和算子库，推荐直接前往[沐曦开发者社区](https://developer.metax-tech.com/softnova/docker)下载 pytorch 镜像（例如 maca-pytorch:3.8.0.11-torch2.10-py312-ubuntu24.04-amd64），镜像中已经集成了 MetaX 软件栈基础环境。

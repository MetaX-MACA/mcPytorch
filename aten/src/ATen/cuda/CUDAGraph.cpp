...
namespace {
bool get_maca_graph_destroy_default() {
  static char const* temp = std::getenv("PYTORCH_GRAPH_DESTROY_DEFAULT");
  return temp != nullptr;
}
}
...
void CUDAGraph::capture_end() {
...
#ifdef USE_MACA
  version = ([]()->int{
    const char* env = getenv("PYTORCH_DRIVER_VERSION");
  ...
  if (!_cuda_graphs_debug) {
    ...
    if (C10_UNLIKELY(at::cuda::get_maca_graph_destroy_default())) {

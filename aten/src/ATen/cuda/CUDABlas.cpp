...
// added bf16 support
#if !defined(USE_ROCM) && !defined(_MSC_VER) && !defined(USE_MACA)
#include <cublasLt.h>
#endif
...
#if defined(USE_MACA)
void bgemm_int8(char transa, char transb, int64_t m, int64_t n, int64_t k, int32_t alpha,    \
      const int8_t *a, int64_t lda, int64_t stridea,                                           \
      const int8_t *b, int64_t ldb, int64_t strideb,                                           \
      int32_t beta, int32_t *c, int64_t ldc, int64_t stridec, int64_t num_batches) {
  globalContext().alertCuBLASConfigNotDeterministic();
  BGEMM_CHECK_ARGVALUES(int8_t);
  cublasHandle_t handle = at::cuda::getCurrentCUDABlasHandle();
  cublasOperation_t opa = _cublasOpFromChar(transa);
  cublasOperation_t opb = _cublasOpFromChar(transb);
  _cublasAdjustLdLevel3(transa, transb, m, n, k, &lda, &ldb, &ldc);

  if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
    printf("[cublasGemmStridedBatchedExFix] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, Atype: %d, lda: %ld, stridea: %ld, Btype: %d, ldb: %ld, strideb: %ld, beta: %f, Ctype: %d, ldc: %ld, stridec: %ld, batchCount: %ld, computeType: %d, algo: %d\n",
            opa, opb, m, n, k, alpha, CUDA_R_8I, lda, stridea, CUDA_R_8I, ldb, strideb, beta, CUDA_R_32I, ldc, stridec, num_batches, CUDA_R_32I, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
  }
  TORCH_CUDABLAS_CHECK(cublasGemmStridedBatchedExFix(handle,
                                  opa, opb, (int)m, (int)n, (int)k,
                                  (void*)&alpha, a, CUDA_R_8I, (int)lda, stridea,
                                  b, CUDA_R_8I, (int)ldb, strideb,
                                  (void*)&beta, c, CUDA_R_32I, (int)ldc, stridec,
                                  (int)num_batches, CUDA_R_32I, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
}
#endif
...
void bgemm<double>(CUDABLAS_BGEMM_ARGTYPES(double)) {
  ...
  if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
    printf("[cublasDgemmStridedBatched] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, lda: %ld, stridea: %ld, ldb: %ld, strideb: %ld, beta: %f, ldc: %ld, stridec: %ld, num_batches: %ld\n",
            opa, opb, m, n, k, alpha, lda, stridea, ldb, strideb, beta, ldc, stridec, num_batches);
  }
...
void bgemm<float>(CUDABLAS_BGEMM_ARGTYPES(float)) {
  ...
  if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
    printf("[cublasSgemmStridedBatched] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, lda: %ld, stridea: %ld, ldb: %ld, strideb: %ld, beta: %f, ldc: %ld, stridec: %ld, num_batches: %ld\n",
            opa, opb, m, n, k, alpha, lda, stridea, ldb, strideb, beta, ldc, stridec, num_batches);
  }
...
void bgemm<at::Half>(CUDABLAS_BGEMM_ARGTYPES(at::Half)) {
  ...
  if (prop->major >= 5){
    if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
      printf("[cublasGemmStridedBatchedExFix] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, Atype: %d, lda: %ld, stridea: %ld, Btype: %d, ldb: %ld, strideb: %ld, beta: %f, Ctype: %d, ldc: %ld, stridec: %ld, batchCount: %ld, computeType: %d, algo: %d\n",
              opa, opb, m, n, k, falpha, CUDA_R_16F, lda, stridea, CUDA_R_16F, ldb, strideb, fbeta, CUDA_R_16F, ldc, stridec, num_batches, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
    }
    ...
    for (const auto i : c10::irange(num_batches)) {
      if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
        printf("[at::cuda::blas::gemm<at::Half>] transa: %d, transb: %d, m: %ld, n: %ld, k: %ld, alpha: %f, lda: %ld, stridea: %ld, ldb: %ld, strideb: %ld, beta: %f, ldc: %ld, stridec: %ld, num_batches: %ld\n",
                transa, transb, m, n, k, alpha, lda, stridea, ldb, strideb, beta, ldc, stridec, num_batches);
      }
...
void bgemm<at::BFloat16>(CUDABLAS_BGEMM_ARGTYPES(at::BFloat16)) {
  ...
  _cublasAdjustLdLevel3(transa, transb, m, n, k, &lda, &ldb, &ldc);
    ...
    if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
      printf("[cublasGemmStridedBatchedExFix] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, Atype: %d, lda: %ld, stridea: %ld, Btype: %d, ldb: %ld, strideb: %ld, beta: %f, Ctype: %d, ldc: %ld, stridec: %ld, batchCount: %ld, computeType: %d, algo: %d\n",
              opa, opb, m, n, k, falpha, CUDA_R_16BF, lda, stridea, CUDA_R_16BF, ldb, strideb, fbeta, CUDA_R_16BF, ldc, stridec, num_batches, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
    }
    TORCH_CUDABLAS_CHECK(cublasGemmStridedBatchedExFix(handle,
                                    opa, opb, (int)m, (int)n, (int)k,
...
void gemm<double>(CUDABLAS_GEMM_ARGTYPES(double)) {
  ...
  if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
    printf("[cublasDgemm] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, lda: %ld, ldb: %ld, beta: %f, ldc: %ld\n",
            opa, opb, m, n, k, alpha, lda, ldb, beta, ldc);
  }
...
void gemm<float>(CUDABLAS_GEMM_ARGTYPES(float)) {
  ...
  if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
    printf("[cublasSgemm] opa: %d, opb: %d, m: %ld, n: %ld, k: %ld, alpha: %f, lda: %ld, ldb: %ld, beta: %f, ldc: %ld\n",
              opa, opb, m, n, k, alpha, lda, ldb, beta, ldc);
  }
...
#if defined(USE_MACA)
void gemm_int8(char transa, char transb, int64_t m, int64_t n, int64_t k, int32_t alpha, const int8_t *a, int64_t lda, const int8_t *b, int64_t ldb, int32_t beta, int32_t *c, int64_t ldc) {
  // See Note [Writing Nondeterministic Operations]
  globalContext().alertCuBLASConfigNotDeterministic();
  cublasHandle_t handle = at::cuda::getCurrentCUDABlasHandle();
  cublasOperation_t opa = _cublasOpFromChar(transa);
  cublasOperation_t opb = _cublasOpFromChar(transb);
  _cublasAdjustLdLevel3(transa, transb, m, n, k, &lda, &ldb, &ldc);
  GEMM_CHECK_ARGVALUES(int8_t);
  if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
   printf("[cublasGemmEx int8_t] opa: %d, opb: %d, m: %ld, n: %ld, k: %ld, alpha: %d, Atype: %d, lda: %ld, Btype: %d, ldb: %ld, beta: %d, Ctype: %d, ldc: %ld, computeType: %d, algo: %d\n",
              opa, opb, m, n, k, alpha, CUDA_R_8I, lda, CUDA_R_8I, ldb, beta, CUDA_R_32I, ldc, CUDA_R_32I, CUBLAS_GEMM_DFALT_TENSOR_OP);
  }
...
void gemm<at::Half>(CUDABLAS_GEMM_ARGTYPES(at::Half)) {
  ...
  if (prop->major >= 5) {
    ...
    if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
      printf("[cublasGemmEx] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, Atype: %d, lda: %ld, Btype: %d, ldb: %ld, beta: %f, Ctype: %d, ldc: %ld, computeType: %d, algo: %d\n",
              opa, opb, m, n, k, falpha, CUDA_R_16F, lda, CUDA_R_16F, ldb, fbeta, CUDA_R_16F, ldc, CUDA_R_32F, CUBLAS_GEMM_DFALT_TENSOR_OP);
    }
    TORCH_CUDABLAS_CHECK(cublasGemmEx(
        handle,
        opa,
    ...
    if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
      printf("[cublasSgemmEx] opa: %d, opa: %d, m: %ld, n: %ld, k: %ld, alpha: %f, Atype: %d, lda: %ld, Btype: %d, ldb: %ld, beta: %f, Ctype: %d, ldc: %ld\n",
              opa, opb, m, n, k, falpha, CUDA_R_16F, lda, CUDA_R_16F, ldb, fbeta, CUDA_R_16F, ldc);
    }
...
void gemm<at::BFloat16>(CUDABLAS_GEMM_ARGTYPES(at::BFloat16)) {
  ...
  TORCH_CUDABLAS_CHECK(cublasSetMathMode(handle, cublas_flags));
  if (maca_unlikely(at::maca::get_maca_enable_gemm_api_info())) {
    printf("[cublasGemmEx] opa: %d, opb: %d, m: %ld, n: %ld, k: %ld, alpha: %f, Atype: %d, lda: %ld, Btype: %d, ldb: %ld, beta: %f, Ctype: %d, ldc: %ld, computeType: %d, algo: %d\n",
            opa, opb, m, n, k, falpha, CUDA_R_16BF, lda, CUDA_R_16BF, ldb, fbeta, CUDA_R_16BF, ldc, CUDA_R_32F, CUBLAS_GEMM_DFALT_TENSOR_OP);
  }
  TORCH_CUDABLAS_CHECK(cublasGemmEx(
      handle,
      opa,

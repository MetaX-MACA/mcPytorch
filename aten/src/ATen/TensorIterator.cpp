...
std::ostream& operator<<(std::ostream& out, TensorIterator t) {
  out << "[[ ndim: " << t.ndim()
      << ", numel: " << t.numel()
      << ", noutpus: " << t.noutputs()
      << ", ninputs: " << t.ninputs()
      << ", view_offsets: " << t.view_offsets()
      << ", num_output_elements: " << t.num_output_elements()
      << ", num_reduce_dims: " << t.num_reduce_dims()
      << ", is_trivial_1d: " << t.is_trivial_1d()
      << ", is_contiguous: " << t.is_contiguous()
      << ", shape: " << t.shape() << " | ";
  for (int i = 0; i < t.ntensors(); ++i) {
    out << "(" << i << ") " << t.strides(i) << ", " << t.input_dtype(i) << ", " << t.element_size(i) << " | ";
  }
  return out;
}

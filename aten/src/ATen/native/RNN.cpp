...
#ifdef USE_MACA
bool lstm_cudnn_is_acceptable(const TensorBase& self) {
  if (!globalContext().userEnabledCuDNN()) return false;
  if (!self.is_cuda()) return false;
  auto st = self.scalar_type();
  if (!(st == kDouble || st == kFloat || st == kHalf || st == at::kBFloat16)) return false;
  if (!detail::getCUDAHooks().compiledWithCuDNN()) return false;
  if (self.sym_numel() == 0) return false;
  return true;
}

bool lstm_cudnn_is_acceptable(const Tensor& self) {
  return lstm_cudnn_is_acceptable(static_cast<const TensorBase&>(self));
}
#endif
...
#ifdef USE_MACA
  bool is_cudnn = (at::maca::get_maca_enable_lstm_cudnn_bf16() && lstm_cudnn_is_acceptable(_input))
                  || at::cudnn_is_acceptable(_input);
  if (is_cudnn) {
#else
  if (use_cudnn(_input)) {
#endif // USE_MACA
...
std::tuple<Tensor, Tensor, Tensor> lstm(
  ...
  TORCH_CHECK(hx.size() == 2, "lstm expects two hidden states");
#ifdef USE_MACA
  bool is_cudnn = (at::maca::get_maca_enable_lstm_cudnn_bf16() && lstm_cudnn_is_acceptable(data))
                  || at::cudnn_is_acceptable(data);
  if (is_cudnn) {
#else
  if (use_cudnn(data)) {
#endif // USE_MACA

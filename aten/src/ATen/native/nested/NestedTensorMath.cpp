...
Tensor view_nested(const Tensor& self, IntArrayRef proposed_shape) {
  ...
  if (default_nchw) {
    ...
    if (viewable) {
      ...
      c10::call_once(once, [] {
        printf("WARNING: view size is not compatible with input tensor's size and stride "
        "(in mcPyTorch, for performance reasons, 4 dimensional tensor's memcpyH2D will generate a tensor with NHWC layout on device) "
        "thus .view() is replaced with .contiguous().view().  "
        " If needed, please use 'export PYTORCH_DEFAULT_NCHW=1' to align to PyTorch's official behavior, "
        "and the replacement may be removed.\n");
      });
      return view_nested(self.contiguous(), proposed_shape);
    }
  }
#endif

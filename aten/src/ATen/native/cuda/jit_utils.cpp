...
#if defined(USE_ROCM) && HIP_VERSION_MAJOR < 7 || defined(USE_MACA)
    return R"ESCAPE(
namespace std {
#ifndef __MACACC_RTC__
template <class _Tp>
_Tp&& __declval(int);
template <class _Tp>
_Tp __declval(long);
...
const std::string jit_common_types = R"ESCAPE(
  #if defined(__HIPCC__) || defined(__MACACC_RTC__)
...
struct alignas(2) BFloat16 {
  ...
  #if __CUDA_ARCH__ >= 800 && !defined(__MACACC_RTC__)
  ...
  struct static_cast_with_inter_type<uint8_t, src_t> {
    static inline uint8_t apply(
      ...
      return static_cast<uint8_t>(
          static_cast<int64_t_>(maybe_real<real, src_t>::apply(src)));
    }
  };

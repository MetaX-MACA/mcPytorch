...
#if defined(USE_MACA)
  #include "maca_kernels/Normalization_opt.cuh"
#else
  static_assert(0);
#endif
...
#ifdef USE_MACA
  GradOp<stat_accscalar_t, stat_accscalar_t, GenericPackedTensorAccessor<const input_scalar_t, 3, DefaultPtrTraits, index_t>> g(mean, input, grad_output);
  auto res = reduce<Float2<stat_accscalar_t, stat_accscalar_t>>(g, grad_output, plane);
#else
...
#ifdef USE_MACA
  GradOp<stat_accscalar_t, stat_accscalar_t, GenericPackedTensorAccessor<input_scalar_t, 3, DefaultPtrTraits, index_t>> g(r_mean, input, grad_output);
  auto res = reduce<Float2<stat_accscalar_t, stat_accscalar_t>>(g, grad_output, plane);
#else
...
#if defined(USE_MACA)
  bool opt_batchnorm_nhwc = at::maca::get_maca_enable_opt_batchnorm_nhwc();
  bool cond0 = input_.dim() == 3 &&  grad_out_.dim() == 3
                      && (input_.scalar_type() == ScalarType::Half || input_.scalar_type() == ScalarType::BFloat16)
                      && grad_out_.scalar_type() == input_.scalar_type()
                      && input_.sizes()[1] == 256 &&  input_.sizes()[0] * input_.sizes()[2] >= 1024
                      && input_.stride(1) == 1
                      && input_.stride(2) == input_.sizes()[1]
                      && input_.stride(0) == input_.sizes()[1] * input_.sizes()[2]
                      && grad_out_.stride(1) == 1
                      && grad_out_.stride(2) == input_.stride(2)
                      && grad_out_.stride(0) == input_.stride(0);
  const input_scalar_t* input = input_.data_ptr<input_scalar_t>();
  const input_scalar_t* grad_output = grad_out_.data_ptr<input_scalar_t>();
  bool cond1 = reinterpret_cast<uint64_t>(input) % 4 == 0 &&
              reinterpret_cast<uint64_t>(grad_output) % 4 == 0;
#endif

#if defined(USE_MACA)
  if (maca_unlikely(opt_batchnorm_nhwc) && cond0 && cond1) {
    const index_t stride = input_.sizes()[1];
    const index_t reduction_size = input_.sizes()[0] * input_.sizes()[2];

    if (grad_input_mask[0]) {
      grad_input_ = at::empty_like(input_, input_.options());
    }
    ...
    const int MAX_BLOCK_SIZE_OPT=512;

    const int vecsize_split = 8;
    const int vecsize = 4;

    const int grid_x0 = std::min((reduction_size / MAX_BLOCK_SIZE_OPT), vecsize_split * (1024 / MAX_BLOCK_SIZE_OPT) );
    const int grid_x1 = std::min(grid_x0, C10_WARP_SIZE);
    const int grid_x = std::max(grid_x1, 1);
    dim3 blocks0(grid_x, stride / vecsize_split);
    dim3 threads0(MAX_BLOCK_SIZE_OPT);

    if (blocks0.x == 1) {
      dim3 blocks1(stride / vecsize);
      dim3 threads1(MAX_BLOCK_SIZE_OPT);

      batchnorm::batch_norm_backward_kernel_opt0<vecsize, input_scalar_t, stat_scalar_t, accscalar_t, index_t>
       <<<blocks1, threads1, 0, stream>>>(
       input,
       grad_output,
       grad_input_.data_ptr<input_scalar_t>(),
       grad_weight_.data_ptr<stat_scalar_t>(),
       grad_bias_.data_ptr<stat_scalar_t>(),
       weight_.data_ptr<stat_scalar_t>(),
       running_mean_.data_ptr<stat_scalar_t>(),
       running_var_.data_ptr<stat_scalar_t>(),
       save_mean_.data_ptr<accscalar_t>(),
       save_invstd_.data_ptr<accscalar_t>(),
       train, 
       epsilon,
       stride,
       reduction_size
      ...
      at::Tensor staging_sum0, staging_sum1;
      at::Tensor semaphores;

      staging_sum0 = at::empty({blocks0.x * stride}, save_mean_.options().dtype(
                                  c10::CppTypeToScalarType<accscalar_t>::value));
      staging_sum1 = at::empty({blocks0.x * stride}, save_mean_.options().dtype(
                                c10::CppTypeToScalarType<accscalar_t>::value));
      semaphores = at::zeros({blocks0.y}, input_.options().dtype(at::kInt));

      accscalar_t* staging_sum0_ptr = staging_sum0.data_ptr<accscalar_t>();
      accscalar_t* staging_sum1_ptr = staging_sum1.data_ptr<accscalar_t>();
      int* semaphores_ptr = semaphores.data_ptr<int>();

      batchnorm::batch_norm_backward_kernel_opt1<vecsize_split, input_scalar_t, stat_scalar_t, accscalar_t, index_t>
        <<<blocks0, threads0, 0, stream>>>(
        input,
        grad_output,
        grad_input_.data_ptr<input_scalar_t>(),
        grad_weight_.data_ptr<stat_scalar_t>(),
      ...
      dim3 blocks2(1, stride / vecsize);
      batchnorm::batch_norm_backward_kernel_grad_input_opt1<vecsize, input_scalar_t, stat_scalar_t, accscalar_t, index_t>
        <<<blocks2, MAX_BLOCK_SIZE_OPT, 0, stream>>>(
        input,
        grad_output,
        grad_input_.data_ptr<input_scalar_t>(),
        grad_weight_.data_ptr<stat_scalar_t>(),
...
#if defined(USE_MACA)
  bool meet_opt_condition = (stride % 64 == 0) && (reduction_size >= 64) && (input.scalar_type() == grad_output.scalar_type()) && \
                            (input.scalar_type() == ScalarType::Half || input.scalar_type() == ScalarType::Float) && \
                            (weight.defined() && weight.scalar_type() == ScalarType::Float);
  bool batch_norm_backward_env = at::maca::get_maca_disable_batch_norm_backward_reduce_channels_last_kernel();
#endif

#if defined(USE_MACA)
  if (!batch_norm_backward_env && meet_opt_condition) {
    dim3 block;
    dim3 grid;

    at::Tensor staging_data;
    at::Tensor semaphores;
    batchnorm::flexible_launch_configs_opt(reduction_size, stride, block, grid);
    ...
    if (input.scalar_type() == ScalarType::Half) {
      AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, input.scalar_type(), "batchnorm_backward_reduce", [&] {
        ...
        batchnorm::batch_norm_backward_reduce_channels_last_kernel_opt<ELEMENTS_PER_ITER,scalar_t,accscalar_t,accscalar_t>
            <<<grid, block, 0, stream>>>(
      ...
      AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, input.scalar_type(), "batchnorm_backward_reduce", [&] {
        ...
        accscalar_t* staging_data_ptr = grid.y > 1 ? staging_data.data_ptr<accscalar_t>() : nullptr;
        int* semaphores_ptr = grid.y > 1 ? semaphores.data_ptr<int>() : nullptr;
        batchnorm::batch_norm_backward_reduce_channels_last_kernel_opt<ELEMENTS_PER_ITER,scalar_t,accscalar_t,scalar_t>
            <<<grid, block, 0, stream>>>(
            input.data_ptr<scalar_t>(),
            grad_output.data_ptr<scalar_t>(),
            mean.data_ptr<accscalar_t>(),

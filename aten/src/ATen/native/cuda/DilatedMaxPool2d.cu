...
#ifdef USE_MACA
#include <ATen/native/cuda/maca_kernels/DilatedMaxPool2d.cuh>
#else
static_assert(0);
#endif
...
#ifdef USE_MACA
          if (nInputPlane <= 512 && (!at::maca::get_maca_disable_max_pool_nhwc_opt())) {
            dim3 grid_(outputWidth, outputHeight, output.size(0));
            dim3 block_(nInputPlane);
            max_pool_forward_nhwc_opt<scalar_t, scalar_t>
            <<<grid_, block_, 0, at::cuda::getCurrentCUDAStream()>>>(
                input_data, nbatch,
...
#ifdef USE_MACA
          if (nInputPlane <= 512 && (!at::maca::get_maca_disable_max_pool_nhwc_opt())) {
            dim3 grid_(inputWidth, inputHeight, gradInput.size(0));
            dim3 block_(nInputPlane);
            max_pool_backward_nhwc_opt<scalar_t, accscalar_t>
            <<<grid_, block_, 0, at::cuda::getCurrentCUDAStream()>>>(
                    gradOutput_data,
                    indices_data,
                    nbatch,

...
#ifdef USE_MACA
#include "maca_kernels/AveragePool2d_opt.cuh"
#endif
...
#ifdef USE_MACA
  const auto memory_format = MemoryFormat::Contiguous;
#else
  const auto memory_format = input_.suggest_memory_format();
#endif
...
#ifdef USE_MACA
  if(maca_unlikely(at::maca::get_maca_enable_avgpool_backward_info())){
    printf("Avgpool Backward: Channels:%ld, Input H:%ld W:%ld, PooledOut H:%ld W:%ld, Kernel kH:%d kW:%d, Stride dH:%d dW:%d, Pad pH:%d pW:%d\n",
          nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth, kH, kW, dH, dW, padH, padW);
  }
#endif
      ...
      AT_DISPATCH_INDEX_TYPES(
        ...
        [&] {
              switch (memory_format) {
                ...
                case MemoryFormat::Contiguous: {
#ifdef USE_MACA
          gradInput.unsafeGetTensorImpl()->empty_tensor_restride(MemoryFormat::Contiguous);
          bool get_maca_disable_avgpool_backward_opt = at::maca::get_maca_disable_avgpool_backward_opt();
          if ( maca_likely(!get_maca_disable_avgpool_backward_opt) &&
              (sizeof(scalar_t)==2 || sizeof(scalar_t)==4) &&
              kH == dH && kW == dW && padH == 0 && padW == 0 && inputHeight % kH == 0 && inputWidth % kW == 0 &&
              (reinterpret_cast<uintptr_t>(gradInput_data) % 4 == 0) &&
              (reinterpret_cast<uintptr_t>(gradOutput_data) % 4 == 0)) {
              avgpool::launch_avg_pool2d_backward_out_cuda_frame_opt<64, scalar_t, accscalar_t, index_t>(
                count,
                gradOutput_data,
                nInputPlane,
                inputHeight, inputWidth,

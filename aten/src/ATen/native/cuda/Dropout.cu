...
#ifdef USE_MACA
 #include "maca_kernels/Droupout_opt.cuh"
#endif
...
int get_vector_size(at::Tensor self, at::Tensor ret, at::Tensor mask) {
  ...
  if (!self.is_non_overlapping_and_dense() || !ret.is_non_overlapping_and_dense() || !mask.is_non_overlapping_and_dense()) {
    ...
    bool disable_opt = env_value != nullptr && std::string(env_value) == "1";
    if(maca_likely(!at::maca::get_maca_disable_fast_fused_dropout_kernel() && !disable_opt)){
...
#ifdef USE_MACA
            case 8:
              dropout::fused_dropout_kernel_vec<
                  scalar_t,
                  accscalar_t,
                  index_type,
                  1,

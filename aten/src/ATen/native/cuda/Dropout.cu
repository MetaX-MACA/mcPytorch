...
#endif
#ifdef USE_MACA
#include "maca_kernels/Droupout.cuh"
#endif
...
void masked_scale_kernel(at::Tensor& ret, const at::Tensor& src, const at::Tensor& mask, accscalar_t scale){
   ...
   at::native::gpu_kernel_maca_arity2(
       iter,
...
int get_vector_size(at::Tensor self, at::Tensor ret, at::Tensor mask) {
  ...
  if (!self.is_non_overlapping_and_dense() || !ret.is_non_overlapping_and_dense() || !mask.is_non_overlapping_and_dense()) {
...
#else
#ifdef USE_MACA
    bool enable_same = at::maca::get_maca_enable_same_rand_a100();
    ...
    if(maca_likely(!at::maca::get_maca_disable_fast_fused_dropout_kernel() && !enable_same && \
      (multi_processor_num == -1 || maxthreads_per_sm == -1))) {
      vec_size = memory::can_vectorize_up_to_<scalar_t>((char*)self.data_ptr());
    } else {
#endif // USE_MACA
...
dropout_cuda(CUDAGeneratorImpl* gen, const Tensor& self, double p){
  ...
  if (is_enable_same_rand_a100) {
    // here we simply hardcode A100's situation
    grid.x = std::min((unsigned int)(108 * 8), grid.x);
  } else if (multi_processor_num != -1 && maxthreads_per_sm != -1) {
    // here we use sm count config from PYTORCH_ENABLE_SAME_RAND_CONF
    blocks_per_sm = (unsigned int)maxthreads_per_sm/(unsigned int)block_size;
    grid.x = std::min((unsigned int)multi_processor_num * blocks_per_sm, grid.x);
  } else {

...
struct _cuda_scatter_gather_internal_kernel {
  ...
  void operator() (
    ...
    bool disable_scatter_gather_dim2_opt_assign = at::maca::get_maca_disable_scatter_gather_dim2_opt_assign();
    int ndim = iter.ndim();
    bool enable_scatter_pw_opt = at::maca::get_maca_enable_scatter_pw_opt();
    if (ndim == 2 && maca_likely(!disable_scatter_gather_dim2_opt) && (sizeof(scalar_t) == 2 || sizeof(scalar_t) == 4) && !is_scatter_like &&
    ...
    } else if (ndim == 2 && maca_likely(!disable_scatter_gather_dim2_opt_assign) && 
      typeid(f) == typeid(TensorAssign) && (sizeof(scalar_t) == 2 || sizeof(scalar_t) == 4) && is_scatter_like &&
      ...
      get_elementwise_info<3>(ndim, 2, offset_calc, "p_e_launch_scatter_elementwise_kernel_dim2_opt_assign", f);
      _launch_scatter_kernel_dim2_opt_assign<num_threads(), thread_work_size(), is_scatter_like, scalar_t>(
        iter.numel(), index_size, index_stride, self_ptr, src_ptr, index_ptr,
        offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor,
        offset_calc.strides_[0][0], offset_calc.strides_[0][1], offset_calc.strides_[0][2],
        offset_calc.strides_[1][0], offset_calc.strides_[1][1], offset_calc.strides_[1][2], f);
      ...
      _launch_scatter_kernel_dim2_opt<num_threads(), thread_work_size(), is_scatter_like, scalar_t>(
        ...
        offset_calc.strides_[0][0], offset_calc.strides_[0][1], offset_calc.strides_[0][2],
        offset_calc.strides_[1][0], offset_calc.strides_[1][1], offset_calc.strides_[1][2], f);
    } else if(is_scatter_like && maca_likely(enable_scatter_pw_opt) && index_stride >= 32  && typeid(f)==typeid(TensorAssign)&& (sizeof(scalar_t)==4 || sizeof(scalar_t)==2 || sizeof(scalar_t)==8)){
    ...
    } else if(is_scatter_like && maca_likely(enable_scatter_pw_opt) && index_stride >= 32   && typeid(f)==typeid(ReduceAdd)  ){
      get_elementwise_info<3>(ndim, 2, offset_calc, "p_e_launch_gather_elementwise_kernel_pw_opt", f);

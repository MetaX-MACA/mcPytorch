...
// #ifdef __NVCC__
#if defined(__NVCC__) && (defined(USE_MACA) && defined(TORCH_MACA_LAMBDA))  // USE_MACA
...
void gpu_kernel_impl_nocast(TensorIteratorBase& iter, const func_t& f) {
  ...
  if (contiguous) {
#ifdef USE_MACA
    get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_noopt_vec_kernel", f);
#endif
...
void gpu_kernel_impl(TensorIteratorBase& iter, const func_t& f) {
  ...
  if (contiguous) {
    ...
    auto output_offset_calculator = TrivialOffsetCalculator<1>();
#ifdef USE_MACA
  get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_noopt_unroll_kernel", f);
#endif
...
#if defined(USE_MACA) && !defined(USE_MACA_COPY)
  #include <ATen/native/cuda/maca_kernels/CUDALoopsArity0.cuh>
  #include <ATen/native/cuda/maca_kernels/CUDALoopsArity1.cuh>
  #include <ATen/native/cuda/maca_kernels/CUDALoopsArity2.cuh>
  #include <ATen/native/cuda/maca_kernels/CUDALoopsArity3.cuh>
  #include <ATen/native/cuda/maca_kernels/CUDALoopsArity5.cuh>
  #include <ATen/native/cuda/maca_kernels/CUDALoopsMultiout.cuh>
#endif

...
#ifdef USE_MACA
  #include <ATen/native/cuda/maca_kernels/indexing_opt.cuh>
...
// TODO(chli): so far we change it to make the activate be same with cuda version, we should optimize this later
#if USE_MACA
        if (sliceSize <= 32) {
#else
  ...
  void print_indexing_info(
    ...
    const Tensor& source
      ...
      for (int64_t i = 0; i < self.dim(); ++i)
         oss << to_string(self.size(i)) << ",";
      oss << ") stride(";
      for (int64_t i = 0; i < self.dim(); ++i)
         oss << to_string(self.stride(i)) << ",";
      oss << ") dtype:" << c10::str(self.scalar_type()) <<",";

      oss << "index shape(";
      for (int64_t i = 0; i < index.dim(); ++i)
         oss << to_string(index.size(i)) << ",";
      oss << ") stride(";
      for (int64_t i = 0; i < index.dim(); ++i)
         oss << to_string(index.stride(i)) << ",";
      oss << ") dtype:" << c10::str(index.scalar_type()) <<",";

      oss << "source shape(";
      for (int64_t i = 0; i < source.dim(); ++i)
         oss << to_string(source.size(i)) << ",";
      oss << ") stride(";
      for (int64_t i = 0; i < source.dim(); ++i)
         oss << to_string(source.stride(i)) << ",";
      oss << ") dtype:" << c10::str(source.scalar_type()) <<",";

      oss << "dim:" << to_string(dim) << "\n";
...
#endif
#ifdef USE_MACA
  const dim3 smallIndexGrid(std::min(ceil_div(sliceSize, (uint64_t)128), (uint64_t)(mpc * 8 * 4)));
  const dim3 smallIndexBlock(std::min(sliceSize, (uint64_t)128));

  const dim3 largeIndexGrid(std::min(ceil_div(sourceTotalSize, (uint64_t)128), (uint64_t)(mpc * 8 * 4 * 2)));
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(at::ScalarType::Bool, at::ScalarType::Half, at::ScalarType::BFloat16, at::ScalarType::ComplexHalf, result.scalar_type(), "index_add", [&] {
      ...
      AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "index_add_cuda_", [&] () {
        ...
        if (numIndex <= 16) {
          ...
          bool opt_large0 = self_.dim() >= 2 && dim <= (self_.dim() -2 ) && index.dim() == 1 &&
          ...
          bool opt_large1 = self_.dim() == 2 && source.dim() == 2 && index.dim() == 1 &&
                            self_.is_contiguous() && source.is_contiguous() && index.is_contiguous() &&
                            dim == 0 && sliceSize % 256 == 0 &&
                            (self_.scalar_type() == ScalarType::BFloat16 || self_.scalar_type() == ScalarType::Half) && 
                            (self_.scalar_type() == source.scalar_type()) &&
                            reinterpret_cast<uint64_t>(selfInfo.data) % 4 == 0 && reinterpret_cast<uint64_t>(sourceInfo.data) % 4 == 0;

          if (maca_likely(!disable_opt_indexing) && opt_large0) {
              dim3 grid(std::min(ceil_div(sourceTotalSize, (uint64_t)128), (uint64_t)(mpc * 8 * 4 * 2)));
              dim3 block(std::min(sourceTotalSize, (uint64_t)128));
              indexFuncLargeIndex_3_3_true_half<
                ...
                reduce_add, 
                alpha_value);
          } else if(maca_likely(!disable_opt_indexing) && opt_large1) {
            dim3 grid(std::min(ceil_div(sourceTotalSize, (uint64_t)128), (uint64_t)(mpc * 8 * 4 * 2)));
            dim3 block(std::min(sourceTotalSize, (uint64_t)128));
            indexFuncLargeIndexAdd_2_2_true<scalar_t, index_t, unsigned int, 2, 2, -2, true>
            <<<grid, block, 0, stream>>>(
              selfInfo,
              sourceInfo,
              indexInfo,
...
#if defined(USE_MACA)
  bool is_print = at::maca::get_maca_enable_print_indexing_info();
  if (C10_UNLIKELY(is_print)) {
    print_indexing_info("reduce", self, dim, index, source);
  }
#endif
...
#ifdef USE_MACA
  dim3 smallIndexGrid(std::min(ceil_div(sliceSize, (uint64_t)128), (uint64_t)(mpc * 8 * 4)));
  dim3 smallIndexBlock(std::min(sliceSize, (uint64_t)128));

  dim3 largeIndexGrid(std::min(ceil_div(sourceTotalSize, (uint64_t)128), (uint64_t)(mpc * 8 * 4 * 2)));
...
#define SMALL_INDEX_WITHOUT_ASSERT(TENSOR_TYPE, INDICES_TYPE, TYPE, DST_DIM, SRC_DIM, IDX_DIM)       \
  indexSelectSmallIndexWithoutAssert<TENSOR_TYPE, INDICES_TYPE, TYPE, DST_DIM, SRC_DIM, IDX_DIM>     \
    <<<smallIndexGrid, smallIndexBlock, 0, stream>>>(                                                \
      outInfo, selfInfo, indicesInfo,                                                                \
      outSelectDim, selfSelectDim, static_cast<TYPE>(sliceSize),                                     \
      selfSelectDimSize);                                                                            \

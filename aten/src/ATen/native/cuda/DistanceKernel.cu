...
#ifdef USE_MACA
#include <ATen/native/cuda/maca_kernels/DistanceKernel_opt.cuh>
#endif
...
__global__ static void pdist_kernel_cuda_impl(
  ...
  for(int64_t k = blockIdx.x; k < numel; k += gridDim.x) {
...
void cdist_kernel_impl(Tensor& result, const Tensor& x1, const Tensor& x2, double p) {
...
#ifdef USE_MACA
  bool is_opt = distance::is_opt_cdist_forward_wave(
                  result, x1, x2, p,
                  r1, r2, m, l1_size, l2_size, r_size);
  if(is_opt) return;
#endif
  ...
  void pdist_forward_kernel_impl(Tensor& result, const Tensor& self, double p) {
    ...
    int64_t grixMax = UINT32_MAX / kCUDANumThreads - 1;
    int64_t grid = std::min(numel, grixMax);
    const dim3 block(kCUDANumThreads);
    ...
    AT_DISPATCH_FLOATING_TYPES(self.scalar_type(), "pdist_cuda", [&] {
      ...
      impl_fptr<<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
        ...
        n, m, p, n2, n2_squared_minus_1, numel);
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    });
  }
#else

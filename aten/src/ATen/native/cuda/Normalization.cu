...
void batch_norm_elementwise(
  ...
  case Impl::General: {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(kBFloat16, kHalf, self.scalar_type(),
      ...
      gpu_kernel_maca_arity5(iter, [] GPU_LAMBDA (scalar_t input, acc_t weight, acc_t bias,
...
Tensor batch_norm_elementwise_backward_eval(
  ...
  if (weight.defined()) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, grad_out.scalar_type(),
                                    "batch_norm_eval_backward", [&]{
      using accscalar_t = at::acc_type<scalar_t, true>;
      gpu_kernel_maca_arity3(iter, [] GPU_LAMBDA (scalar_t gO, accscalar_t invstd, accscalar_t weight)
                 -> scalar_t {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, grad_out.scalar_type(),
                                    "batch_norm_eval_backward", [&]{
      using accscalar_t = at::acc_type<scalar_t, true>;
      gpu_kernel_maca_arity2(iter, [] GPU_LAMBDA (scalar_t gO, accscalar_t invstd) -> scalar_t {
          return gO * invstd;
      });
    });
  }
...
#if defined(USE_MACA)  
  bool is_print = at::maca::get_maca_enable_print_batchnorm_info();
  if(C10_UNLIKELY(is_print)){
    std::ostringstream oss;

    oss << "batchnorm_backward info: grad_out shape(";
    for (int64_t i = 0; i < grad_out.dim(); ++i){
      oss << to_string(grad_out.size(i))<<",";
    }
    oss << ") stride(";
    for (int64_t i = 0; i < grad_out.dim(); ++i){
      oss << to_string(grad_out.stride(i))<<",";
    }
    oss << ") dtype:" << c10::str(grad_out.scalar_type()) <<",";

    oss << " input shape(";
    for (int64_t i = 0; i < input.dim(); ++i){
      oss << to_string(input.size(i))<<",";
    }
    oss << ") stride(";
    for (int64_t i = 0; i < input.dim(); ++i){
      oss << to_string(input.stride(i))<<",";
    }
    oss << ") dtype:" << c10::str(input.scalar_type()) <<",";

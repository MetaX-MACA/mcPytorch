...
__device__ inline void elementwise_kernel_helper_4_float_to_half(func_t f, char *data0, char *data1) {

  typedef float args_t;
  typedef at::Half return_t;

  int64_t idx = blockIdx.x;
  int thread_idx = threadIdx.x;

  using LoadT = at::native::memory::aligned_vector<args_t, thread_work_size()>;
  using StoreT = at::native::memory::aligned_vector<return_t, thread_work_size()>;
  ...
  size_t offset_ld = (thread_idx * thread_work_size() + block_work_size() * idx) * sizeof(args_t);
  size_t offset_sr = (thread_idx * thread_work_size() + block_work_size() * idx) * sizeof(return_t);
  *p_ld = *reinterpret_cast<LoadT*>(data1 + offset_ld);
  results[0] = c10::convert<return_t>(args[0]);
  results[1] = c10::convert<return_t>(args[1]);
  results[2] = c10::convert<return_t>(args[2]);
  results[3] = c10::convert<return_t>(args[3]);
  StoreT* out = reinterpret_cast<StoreT*>(data0 + offset_sr);
  *out = *p_sr;
}

template<typename func_t, typename size_t>
__device__ inline void elementwise_kernel_helper_4_float_to_half_no_align(int64_t N, func_t f, char *data0, char *data1) {
  ...
  size_t offset=thread_idx * thread_work_size() + block_work_size() * idx;
  ...
  if(blockIdx.x != gridDim.x-1){
    ...
    *out = *p_sr;
    ...
    if(offset <= (N/thread_work_size()-1)* thread_work_size() ){
      *p_ld = *reinterpret_cast<LoadT*>(data1 + offset * sizeof(args_t));
      results[0] = c10::convert<return_t>(args[0]);
      results[1] = c10::convert<return_t>(args[1]);
      results[2] = c10::convert<return_t>(args[2]);
      ...
      StoreT* out = reinterpret_cast<StoreT*>(data0 + offset * sizeof(return_t));
      *out = *p_sr;
    }else if(offset == (N/thread_work_size())* thread_work_size()){
      while(offset<N){
...
template<typename func_t, typename size_t>
__device__ inline void elementwise_kernel_helper_4_half_to_float(func_t f, char *data0, char *data1) {

  typedef at::Half args_t;
...
template<typename func_t, typename size_t>
__device__ inline void elementwise_kernel_helper_4_half_to_float_no_align(int64_t N, func_t f, char *data0, char *data1) {
  typedef at::Half args_t;
  typedef float return_t;
...
template<typename func_t, typename size_t>
__device__ inline void elementwise_kernel_helper_4_float_to_bfloat(func_t f, char *data0, char *data1) {

  typedef float args_t;
  typedef at::BFloat16 return_t;
...
template<typename func_t, typename size_t>
__device__ inline void elementwise_kernel_helper_4_float_to_bfloat_no_align(int64_t N, func_t f, char *data0, char *data1) {
  typedef float args_t;
  typedef at::BFloat16 return_t;
...
template<typename func_t, typename size_t>
__device__ inline void elementwise_kernel_helper_4_bfloat_to_float(func_t f, char *data0, char *data1) {

  typedef at::BFloat16 args_t;
  typedef float return_t;
...
template<typename func_t, typename size_t>
__device__ inline void elementwise_kernel_helper_4_bfloat_to_float_no_align(int64_t N, func_t f, char *data0, char *data1) {
...
#if !defined(USE_ROCM)
  #include <ATen/native/cuda/maca_kernels/CUDALoops_copy.cuh>
#else
...
void gpu_kernel_maca_arity1_copy(TensorIteratorBase& iter, const func_t& f) {
  ...
  gpu_kernel_impl_maca_arity1_copy(iter, f);
}

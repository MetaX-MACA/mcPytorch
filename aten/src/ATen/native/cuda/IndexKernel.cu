...
#include <ATen/native/cuda/maca_kernels/IndexKernelMaca.cuh>
...
void gpu_index_kernel(TensorIteratorBase& iter, IntArrayRef index_size, IntArrayRef index_stride, const func_t& f) {
  ...
  if (num_indices == 2 && iter.ndim() == 1) {
    get_elementwise_info<3>(1, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_2_1", f);
    ...
    get_elementwise_info<3>(2, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_2_2", f);
    ...
    get_elementwise_info<3>(3, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_2_3", f);
    ...
    get_elementwise_info<3>(1, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_1_1", f);
    ...
    get_elementwise_info<3>(2, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_1_2", f);
    ...
    get_elementwise_info<3>(3, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_1_3", f);
    ...
    get_elementwise_info<3>(1, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_3_1", f);
    ...
    get_elementwise_info<3>(2, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_3_2", f);
    ...
    get_elementwise_info<3>(3, 2, offset_calc, "p_e_launch_legacy_index_kernel_maca_3_3", f);
...
#ifdef USE_MACA
    index_kernel_impl_maca<dtype>(iter, index_size, index_stride, false);
#else
...
void flip_kernel_impl(TensorIterator& iter) {
  ...
  if (iter.ndim() == 2 && iter.ninputs() == 2) {
    // see TensorTransformations.cpp
    // create dummy output with 0 strides at flipped dimension, to prevent tensorIterator from coalescing flipped dims
    launch_kernel_flip_1_2<launch_size_nd, launch_bound2, scalar_t>(
      iter.numel(),
      out_ptr, in_ptr,

...
bool should_use_sort(const Tensor& self, int64_t dim) {
  ...
  void print_topk_info(
    ...
    oss << "largest:" << to_string(largest) << ",";
    oss << "sorted:" << to_string(sorted) << "\n";
...
#if defined(USE_MACA)
    bool is_print = at::maca::get_maca_enable_print_topk_info();
    if (C10_UNLIKELY(is_print)) {
      print_topk_info(self, k, dim, largest, sorted);
    }
#endif
...
#ifdef USE_MACA
{
  bool is_success = launch_gather_topk_kernel_opt(self, k, dim, largest, values, indices);

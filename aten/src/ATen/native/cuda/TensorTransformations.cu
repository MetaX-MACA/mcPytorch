...
C10_LAUNCH_BOUNDS_1(cuda::getApplyBlockSize())
__global__ void vectorized_roll_cuda_kernel(
    scalar_t* in_tensor,
    scalar_t* out_tensor,
    int64_t N,
    int64_t roll_dim,
    ...
    int64_t stride,
    int64_t total_dims) {
  using vec_type = aligned_vector<scalar_t, vec_size>;
  
  int64_t begin_index = blockIdx.x * blockDim.x + threadIdx.x;
  int64_t increment = blockDim.x * gridDim.x;
  for (int64_t index = begin_index; index * vec_size < N; index += increment) {
    int64_t linear_index = index * vec_size;
    ...
    reinterpret_cast<vec_type*>(out_tensor)[index] = 
            reinterpret_cast<vec_type*>(in_tensor)[source_idx/vec_size];
  }
}
...
Tensor roll_cuda(const Tensor& self, IntArrayRef shifts, IntArrayRef dims) {
  ...
  bool env = at::maca::get_maca_disable_opt_roll();
  int64_t pos_dim = dim;
  if (pos_dim < 0) pos_dim += total_dims;
  int64_t last_dim = in_tensor.size(total_dims-1);
  bool opt_cond0 = pos_dim < (total_dims -1) 
                && last_dim % 8 == 0
                && N >= 12845056;
  bool opt_cond1 = in_tensor.scalar_type() == at::kHalf ||
                   in_tensor.scalar_type() == at::kBFloat16;
  if (maca_likely(!env) && opt_cond0 && opt_cond1) {
    const int vec_size = 8;
    const int MAX_BLOCK_SIZE = 512;
    const int grid0 = ((N / vec_size) + MAX_BLOCK_SIZE -1) / MAX_BLOCK_SIZE;
    const int grid = std::min((32 * 64 / MAX_BLOCK_SIZE) * 104, grid0);
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
        ...
        [&] {
          vectorized_roll_cuda_kernel<vec_size>
           <<<grid, MAX_BLOCK_SIZE, 0, at::cuda::getCurrentCUDAStream()>>>(
            in_tensor.data_ptr<scalar_t>(), out_tensor.data_ptr<scalar_t>(), N,
            pos_dim, start,
            size,
            in_tensor.stride(pos_dim),
            total_dims);

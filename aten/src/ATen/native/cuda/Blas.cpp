...
Tensor& addmm_out_cuda_impl(Tensor& result, const Tensor& self, const Tensor& mat1, const Tensor& mat2, const Scalar& beta, const Scalar& alpha, Activation activation=Activation::None) {
  ...
  #if defined(USE_MACA)
  if (scalar_type == at::ScalarType::Int){
...
#endif
  {
    ...
    if (scalar_type == at::ScalarType::Char){
        int8_t alpha_val = alpha.to<int8_t>();
        int8_t beta_val = beta.to<int8_t>();
        int8_t* mat1_ptr = mat1_->data_ptr<int8_t>();
        int8_t* mat2_ptr = mat2_->data_ptr<int8_t>();
        int32_t* result_ptr = result_->data_ptr<int32_t>();
        at::cuda::blas::gemm_int8(
...
const Tensor& baddbmm_out_cuda_impl(const Tensor& result, const Tensor& self, const Tensor& batch1, const Tensor& batch2, const Scalar& beta, const Scalar& alpha) {
  ...
  if (scalar_type == at::ScalarType::Char){
        int8_t alpha_val = alpha.to<int8_t>();
        int8_t beta_val = beta.to<int8_t>();
        int8_t* batch1_ptr = batch1_->data_ptr<int8_t>();
        int8_t* batch2_ptr = batch2_->data_ptr<int8_t>();
        int32_t* result_ptr = result_->data_ptr<int32_t>();
        at::cuda::blas::bgemm_int8(
  ...
  #endif
      AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, scalar_type, "baddbmm_cuda", [&] {
        using opmath_t = at::opmath_type<scalar_t>;

...
#include <ATen/native/cuda/maca_kernels/group_norm_kernel_opt.cuh>
...
#ifdef USE_MACA
  // blockDim.x < C10_WARP_SIZE
  sum1 = cuda_utils::WarpXReduceSum<T_ACC>(sum1);
  sum2 = cuda_utils::WarpXReduceSum<T_ACC>(sum2);
#else
  sum1 = cuda_utils::WarpReduceSum<T_ACC>(sum1);
  sum2 = cuda_utils::WarpReduceSum<T_ACC>(sum2);
#endif
...
template <typename T>
bool y_can_vectorized(int64_t HxW,
    T *y_data,
    const int vec_size) {
...
#endif
  return HxW % vec_size == 0 && can_vectorize(y_data, alignment); 
}
...
void GroupNormKernelImplInternal(
  ...
  if (maca_unlikely(at::maca::get_maca_print_group_norm_shape())) {
    print_group_norm_info<T>(N, C, HxW, group, "forward");
  }
  if (N == 0) {
    return;
  }
  ...
  const int64_t rowwise_num_threads =
      D * HxW < cuda_utils::kCUDABlockReduceNumThreads
      ? at::cuda::warp_size()
      : cuda_utils::kCUDABlockReduceNumThreads;

  if (is_launch_group_norm_forward_opt_kernel<T>(X)) {
    bool is_fused_kernel = launch_group_norm_forward_kernel(N, C, G, HxW, eps, X, gamma_data, beta_data, Y, mean_data, rstd_data);
    if (is_fused_kernel) {
      return;
    }
  } else {
    RowwiseMomentsCUDAKernel<T><<<N * G, rowwise_num_threads, 0, cuda_stream>>>(
  ...
  if (HxW == 1) {
    ...
    // using maunal kernel here. Make it using gpu_kernel_multiple_outputs
    // once the issue fixed.
    ...
    auto iter = TensorIteratorConfig()
                    ...
                    .build();
    gpu_kernel_maca_arity3(iter, [] GPU_LAMBDA(T x, T_ACC a, T_ACC b) -> T {
...
void GroupNorm1dBackward(
  ...
  if (dgamma.defined() || dbeta.defined()) {
    ...
    if (N <= 128) {
      ...
      // The algorithm for colwise reduction here is to accumulate each 32
      // cols to a 32 * 32 tile and write the tile to shared memmory. Then do
      // warp reduce for each col in the tile. So here the blockDim must be
      // (32, 16).
...
void GroupNormBackwardKernelImplInternal(
  ...
  cudaStream_t cuda_stream = at::cuda::getCurrentCUDAStream();
  if (maca_unlikely(at::maca::get_maca_print_group_norm_shape())) {
    print_group_norm_info<T>(N, C, HxW, group, "backward");
  }
  if (N == 0) {
    if (dgamma.defined()) {
      dgamma.fill_(T(0));
  ...
  if (is_launch_group_norm_backward_opt_kernel<T>(X, dX, D, HxW, sizeof(T))) {
    launch_backward_fused_kernel<T>(
        dY, X, mean, rstd, gamma, N, C, HxW, G, dX, ds, db);
  } else {
    ...
    if (dX.defined()) {
      ...
      if (gamma.defined()) {
        auto iter = TensorIteratorConfig()
                        ...
                        .add_owned_input(gamma.view({1, G, D}))
                        .build();
        gpu_kernel_maca_arity2(iter, [] GPU_LAMBDA(T rstd, T gamma) -> T_ACC {

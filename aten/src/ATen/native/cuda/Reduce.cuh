...
static void launch_reduce_kernel(const ReduceConfig& config, R& reduction) {
  ...
  if (config.should_global_reduce()) {
      ...
      reduction.cta_buf = buffer.get();
      reduction.semaphores = (int*)semaphores.get();
    }
...
inline void gpu_reduce_kernel(TensorIterator& iter, const ops_t& ops, ident_t ident=0,
  ...
  auto output_calc = make_output_calculator<int64_t>(iter);
  auto input_calc = make_input_calculator<int64_t>(iter);
  auto reduce = ReduceOp<scalar_t, ops_t, int64_t, out_scalar_t, vt0>(
      ops,
      config,
      input_calc,
      output_calc,
  ...
  if (address_can_vectorize && is_launch_reduce_maca_kernel<scalar_t, out_scalar_t, arg_t, vt0>(iter, use_maca_kernel))  {
  ...
  if (maca_unlikely(at::maca::get_maca_enable_print_reduce_kernel())) {
    ...
    print_reduce_info<scalar_t, out_scalar_t, arg_t>(reduce_type, iter);
  }
}

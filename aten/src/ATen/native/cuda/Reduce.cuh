...
#include <ATen/native/cuda/maca_kernels/reduce_interface.cuh>
#endif
...
inline void gpu_reduce_kernel(TensorIterator& iter, const ops_t& ops, ident_t ident=0,
  ...
  bool use_maca_kernel = acc_buf_ptr == NULL && can_use_32bit_indexing;
  ...
  char* acc_data = acc_buf_ptr->get_acc_slice(out_data);
    ...
    // USE_MACA
  auto can_vectorize = [&](void * ptr, size_t alignment){uint64_t addr = reinterpret_cast<uint64_t>(ptr); return addr % alignment == 0;};
  bool address_can_vectorize = can_vectorize((void *)in_data, sizeof(int32_t)) && can_vectorize((void *)out_data, sizeof(int32_t))
                             && can_vectorize((void *)(out_data_extra.value_or(nullptr)), sizeof(int32_t));
  // max dim of input calculator and output calculator, refer to make_input_calculator() and make_output_calculator()
  // maca kernel use offset claculator with max_dims=8 for performance considerations (original kernel use max_dims=25 for the consideration of universality)
  bool max_dim_le_8 = iter.num_reduce_dims() <= 8 && (iter.ndim()-iter.num_reduce_dims()) <= 8;
  bool should_launch_maca_opt_kernel = address_can_vectorize && max_dim_le_8 && is_launch_reduce_maca_kernel<scalar_t, out_scalar_t, arg_t, vt0>(iter, use_maca_kernel);

  ReduceConfig config = setReduceConfig<arg_t, scalar_t, vt0, input_vec_size>(iter);
  at::DataPtr buffer;
  at::DataPtr semaphores;
  if (config.should_global_reduce() && !should_launch_maca_opt_kernel) { // USE_MACA: maca kernels will execute memset before the kernel launch
    auto& allocator = *c10::cuda::CUDACachingAllocator::get();
  ...
  if (maca_unlikely(at::maca::get_maca_enable_print_reduce_kernel())) {
    ReduceKernelType reduce_type = get_reduce_kernel_type<scalar_t, out_scalar_t, arg_t, vt0>(iter, use_maca_kernel);
    print_reduce_info<scalar_t, out_scalar_t, arg_t>(reduce_type, iter, reduce);
    if (reduce_type ==  ReduceOriginKernel) {
      std::cout << "cuda config: " << config << std::endl;
    }
  }


  if (should_launch_maca_opt_kernel)  {
    launch_reduce_maca_kernel<scalar_t, out_scalar_t, arg_t, vt0, mnt_wrapper<scalar_t>::MAX_NUM_THREADS>(iter, reduce, use_maca_kernel);
  } else {

...
void nll_loss2d_backward_out_cuda_template(
  ...
  check_inputs_nll_loss2d(input_, target_, weight);
  grad_input.resize_as_(input_, at::MemoryFormat::Contiguous);
  grad_input.zero_();
  ...
  if (reduction == at::Reduction::None) {
    TORCH_CHECK(
        ...
        grad_output.sizes());
    TORCH_CHECK(
        grad_output.size(0) == target_.size(0) &&
            grad_output.size(1) == target_.size(1) &&
            grad_output.size(2) == target_.size(2),
        ...
        [&] {
          nll_loss2d_backward_no_reduce_kernel<scalar_t>
              <<<GET_BLOCKS(count),
                 ...
                 at::cuda::getCurrentCUDAStream()>>>(
                  count,
                  target_.packed_accessor64<int64_t, 3>(),
  ...
  auto target_numel = target_.numel();

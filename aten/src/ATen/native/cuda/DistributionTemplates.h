...
#ifdef USE_MACA
  #include "maca_kernels/DistributionTemplates_opt.cuh"
#endif
...
#ifdef USE_MACA
  bool is_enable_same_rand_a100 = at::maca::get_maca_enable_same_rand_a100();
  size_t multi_processor_num = CUDARandConfig::multi_processor_count(); // USE_MACA: use env PYTORCH_ENABLE_SAME_RAND_CONF
  size_t maxthreads_per_sm = CUDARandConfig::maxthreads_per_multiprocessor(); // USE_MACA: use env PYTORCH_ENABLE_SAME_RAND_CONF
  if (is_enable_same_rand_a100) {
    uint32_t blocks_per_sm = 2048 / block_size;
    grid.x = std::min(
        static_cast<uint32_t>(108) * blocks_per_sm, grid.x);
  } else if (multi_processor_num != -1 && maxthreads_per_sm != -1) {
    uint32_t blocks_per_sm = maxthreads_per_sm / block_size;
    grid.x = std::min(
        static_cast<uint32_t>(multi_processor_num) * blocks_per_sm, grid.x);
  } else {
#endif // USE_MACA
...
#ifdef USE_MACA
  bool is_enable_same_rand_a100 = at::maca::get_maca_enable_same_rand_a100();
  bool disable_exponsional_opt = at::maca::get_maca_disable_expontial_opt();
  bool opt_expon = !is_enable_same_rand_a100 && !disable_exponsional_opt &&
                   iter.is_trivial_1d() && iter.is_contiguous() &&
                   (sizeof(scalar_t) == 4 || sizeof(scalar_t) == 2) && (uint64_t)iter.data_ptr(0) % 4 == 0 &&
                   numel >= 16384 && numel % 8 == 0;
  uint64_t counter_offset;
  dim3 grid;
  dim3 block;
  if (opt_expon) {
    auto execution_policy = calc_execution_policy_opt(numel);
    counter_offset = std::get<0>(execution_policy);
    grid = std::get<1>(execution_policy);
    block = std::get<2>(execution_policy);
  } else {
...
#ifdef USE_MACA
  if (opt_expon) {
    distribution_elementwise_grid_stride_kernel_opt<scalar_t, accscalar_t, 4><<<grid, block, 0, stream>>>(
        (scalar_t*)out_data,
        numel,
        rng_engine_inputs,
        dist_func,

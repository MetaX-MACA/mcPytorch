...
std::tuple<uint64_t, dim3, dim3> calc_execution_policy(int64_t total_elements) {
  ...
  char *env_value = getenv("PYTORCH_ENABLE_SAME_RAND_A100");
  if (env_value != nullptr && std::string(env_value) == "1") {
    blocks_per_sm = 2048 / block_size;
    grid.x = std::min(
...
#ifdef USE_MACA
  char *a100_value = getenv("PYTORCH_ENABLE_SAME_RAND_A100");
  bool disable_exponsional_opt = at::maca::get_maca_disable_expontial_opt();
  bool opt_expon = a100_value == nullptr && !disable_exponsional_opt &&

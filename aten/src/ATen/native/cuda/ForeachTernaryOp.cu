...
std::vector<at::Tensor> foreach_tensor_lerp_ternary_cuda(TensorList tensors1, TensorList tensors2, TensorList tensors3) {
  ...
  bool is_opt = at::maca::get_maca_enable_fast_multitensorapply() && at::cuda::currentStreamCaptureStatus() == at::cuda::CaptureStatus::None;
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&]() {
          ...
          if (maca_likely(is_opt)) {
            multi_tensor_apply_opt<4>(
              tensor_lists,
              TernaryOpListFunctorOpt<scalar_t, /* depth */ 4, /* r_args_depth */ 3, /* res_arg_index */ 3>(),
...
void foreach_tensor_lerp_ternary_cuda_(TensorList tensors1, TensorList tensors2, TensorList tensors3) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        ...
        [&]() {
            using opmath_t = typename at::opmath_type<scalar_t>;
            if (maca_likely(is_opt)) {
              multi_tensor_apply_opt<3>(
                tensor_lists,
                TernaryOpListFunctorOpt<scalar_t, /* depth */ 3, /* r_args_depth */ 3, /* res_arg_index */ 0>(),
                LerpFunctor<opmath_t>());
            } else {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&]() {
          ...
          if (maca_likely(is_opt)) {
            multi_tensor_apply_opt<3>(
              tensor_lists,
              TernaryOpScalarFunctorOpt<scalar_t, /* depth */ 3, /* r_args_depth */ 2, /* res_arg_index */ 2>(),
              LerpFunctor<opmath_t>(),
              weight.to<opmath_t>());
          } else {
            multi_tensor_apply<3>(
...
void foreach_tensor_lerp_list_cuda_(TensorList tensors1, TensorList tensors2, const Scalar& weight) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        ...
        [&]() {
            using opmath_t = typename at::opmath_type<scalar_t>;
            if (maca_likely(is_opt)) {
              multi_tensor_apply_opt<2>(
                tensor_lists,
                TernaryOpScalarFunctorOpt<scalar_t, /* depth */ 2, /* r_args_depth */ 2, /* res_arg_index */ 0>(),
                LerpFunctor<opmath_t>(),
                weight.to<opmath_t>());
            } else {

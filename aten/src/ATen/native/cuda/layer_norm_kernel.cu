...
void launch_vectorized_layer_norm_kernel(
  ...
  T_ACC* rstd_data
    ...
    #ifdef USE_MACA
      if (!rms_norm && layernorm::IsOpt_launch_vectorized_layer_norm_kernel(
          N,
          M,
          eps,
          X_data,
...
#ifdef USE_MACA
  if (!rms_norm && layernorm::IsOpt_launch_layer_norm_kernel(
      N,
      M,
      eps,
      X_data,
...
void LayerNormBackwardKernelImplInternal(
  ...
  if (!rms_norm && maca_likely(!disable_opt_layernorm_grad_input) && can_vec) {
      ...
      if ((M % kWarpSize == 0) && (N % kWarpSize == 0)) {
        ...
        if (is_opt) {
          ...
          if (vec == 4) {
            ...
            layernorm::GammaBetaBackwardCUDAKernel_32x32<T, T_ACC>
                <<<blocks, threads, shmem_sz, cuda_stream>>>(
                    M,
                    N,
                    dY_data,
        ...
        layernorm::GammaBetaBackwardCUDAKernel<T, T_ACC>
            <<<blocks, threads, shmem_sz, cuda_stream>>>(
                M,
                N,
                dY_data,

...
#include "maca_kernels/layer_norm_kernel_opt.cuh"
...
constexpr int vec_size = 8; //we could make it dependent on dtype, but that would lead to different results between float and low-p types
...
void launch_vectorized_layer_norm_kernel(
  ...
  T_ACC* rstd_data
    ...
    bool enable_original_vectorized_layernorm = 
         at::maca::get_maca_enable_original_vectorized_layernorm() || M == 0 || N == 0;
    if (maca_unlikely(enable_original_vectorized_layernorm)) {
      ...
      const dim3 blocks(M);
      bool disable_vectorized_layernorm_opt = at::maca::get_maca_disable_vectorized_layernorm_opt();
      bool can_vec_model = (reinterpret_cast<uint64_t>(gamma_data) % 4 == 0)
                        && (reinterpret_cast<uint64_t>(beta_data) % 4 == 0);  //gamma,beta can vectorized read
      if (maca_likely(!disable_vectorized_layernorm_opt) && can_vec_model) {
        ...
        if (cond0) {
          ...
          const int pow2_x = layernorm::min_pow2(thread1.x);
          layernorm::vectorized_layer_norm_kernel_opt_wave1<vec_maca><<<blocks, thread1, 0, stream>>>(N, pow2_x, eps, X_data,
          gamma_data, beta_data, mean_data, rstd_data, Y_data);
        } else if (cond1) {
          ...
          const int pow2_y = layernorm::min_pow2(thread1.y);
          int nshared = thread1.y > 1 ? 3 * pow2_y : 0;
          layernorm::vectorized_layer_norm_kernel_opt_pass1<vec_f><<<blocks, thread1, nshared, stream>>>(N, pow2_y, eps, X_data,
          gamma_data, beta_data, mean_data, rstd_data, Y_data);
        } else if (cond4) {
          int thread_dim0 = warp_size;
          ...
          const int pow2_y = layernorm::min_pow2(thread1.y);
          int nshared = thread1.y > 1 ? 3 * pow2_y : 0;
          layernorm::vectorized_layer_norm_kernel_opt_pass1<vec_h><<<blocks, thread1, nshared, stream>>>(N, pow2_y, eps, X_data,
          gamma_data, beta_data, mean_data, rstd_data, Y_data);
        } else {
          const int vec_maca = 8;
          ...
          int nshared = threads.y > 1 ? 3 * pow2 : 0;
          layernorm::vectorized_layer_norm_kernel_opt<vec_maca><<<blocks, threads, nshared, stream>>>(N, pow2, eps, X_data,
          gamma_data, beta_data, mean_data, rstd_data, Y_data);
        }
      } else {
        const int max_num_threads = 512;
        int thread_dim0 = N / vec_size < warp_size ? N / vec_size : warp_size;
        int thread_dim1 = N <= warp_size * vec_size ? 1 : ((N / vec_size) + warp_size - 1) / warp_size;
        thread_dim1 = std::min(thread_dim1, int(max_num_threads / thread_dim0));
        const dim3 threads(thread_dim0, thread_dim1, 1);
        const dim3 blocks(M);
        const int last_pow2_size = layernorm::last_pow2(thread_dim1);
        const int inter_warp_reduce_size = last_pow2_size * 3 / 2;
        const int inter_warp_reduce_remaining_size =
            thread_dim1 > last_pow2_size ? (thread_dim1 - last_pow2_size) * 3 : 0;
        int nshared = threads.y > 1
            ? inter_warp_reduce_size + inter_warp_reduce_remaining_size
            : 0;
        layernorm::vectorized_layer_norm_kernel_opt_discard<<<blocks, threads, nshared, stream>>>(N, last_pow2_size, eps, X_data,
        gamma_data, beta_data, mean_data, rstd_data, Y_data);
      }
    }
...
void LayerNormKernelImpl(
    ...
    Tensor* mean,
    Tensor* rstd) {
  if(maca_unlikely(at::maca::get_maca_enable_layernorm_kernel_info())){
    printf("layernorm M: %ld, N: %ld, input type: %s\n", M, N, c10::str(X.scalar_type()).c_str());
  }
...
void LayerNormBackwardKernelImplInternal(
  ...
  if (dX_data != nullptr) {
    ...
    bool is_float = std::is_same<T, float>::value;
    bool is_half = std::is_same<T, at::Half>::value || std::is_same<T, at::BFloat16>::value;
    bool can_vec = (is_float && N % vec_f == 0
                  || is_half && N % vec_size == 0)
                   && (reinterpret_cast<uint64_t>(dY_data) % 4 == 0)
                   && (reinterpret_cast<uint64_t>(X_data) % 4 == 0)
                   && (reinterpret_cast<uint64_t>(dX_data) % 4 == 0)
                   && ((gamma_data == nullptr) || 
                       (gamma_data != nullptr) && (reinterpret_cast<uint64_t>(gamma_data) % 4 == 0));
    bool disable_opt_layernorm_grad_input = at::maca::get_maca_disable_opt_layernorm_grad_input() || M == 0 || N == 0;
    if (maca_likely(!disable_opt_layernorm_grad_input) && can_vec) {

      const int max_num_threads = 512;
      int threads;
      if (std::is_same<T, float>::value) {
        threads = (( N / vec_f + warp_size -1) / warp_size) * warp_size ;
      } else {
        threads = (( N / vec_size + warp_size -1) / warp_size) * warp_size ;
      }
      threads = std::min(max_num_threads, threads);
      const dim3 blocks(M);
      int nshared = (threads / warp_size + 1) * sizeof(T_ACC);

      bool cond0 = is_float && N == 256;
      bool cond1 = (is_float && N / vec_f <= warp_size) ||
                   (is_half && N / vec_size <= warp_size);
      bool cond2 = (is_float && N / vec_f <= max_num_threads) ||
                   (is_half && N / vec_size <= max_num_threads);
      bool cond3 = (is_float && N / vec_f <= (2 * max_num_threads)) ||
                   (is_half && N / vec_size <= (2 * max_num_threads));

      if (cond0) {
        int nshared1 = 2 * sizeof(T_ACC);
        layernorm::vectorized_layer_norm_grad_input_kernel_fp_256<vec_f><<<blocks, warp_size, nshared1, cuda_stream>>>(dY_data,
        X_data, mean_data, rstd_data, gamma_data, dX_data, N);
      } else if (cond1) {
        int nshared1 = 2 * sizeof(T_ACC);
        if (is_float) {
          layernorm::vectorized_layer_norm_grad_input_kernel_wave1<vec_f><<<blocks, warp_size, nshared1, cuda_stream>>>(dY_data,
          X_data, mean_data, rstd_data, gamma_data, dX_data, N);
        } else {
          layernorm::vectorized_layer_norm_grad_input_kernel_wave1<vec_size><<<blocks, warp_size, nshared1, cuda_stream>>>(dY_data,
          X_data, mean_data, rstd_data, gamma_data, dX_data, N);
        }
      } else if (cond2) {
         int pow2 = (max_num_threads / warp_size);
         int nshared1 = 2 * pow2 * sizeof(T_ACC);
         if (is_float) {
          layernorm::vectorized_layer_norm_grad_input_kernel_pass1<vec_f><<<blocks, threads, nshared1, cuda_stream>>>(dY_data,
          X_data, mean_data, rstd_data, gamma_data, dX_data, N, pow2);
        } else {
          layernorm::vectorized_layer_norm_grad_input_kernel_pass1<vec_size><<<blocks, threads, nshared1, cuda_stream>>>(dY_data,
          X_data, mean_data, rstd_data, gamma_data, dX_data, N, pow2);
        }
      } else if (cond3) {
         int pow2 = (max_num_threads / warp_size);
         int nshared1 = 2 * pow2 * sizeof(T_ACC);
         if (is_float) {
          layernorm::vectorized_layer_norm_grad_input_kernel_pass2<vec_f><<<blocks, threads, nshared1, cuda_stream>>>(dY_data,
          X_data, mean_data, rstd_data, gamma_data, dX_data, N, pow2);
        } else {
          layernorm::vectorized_layer_norm_grad_input_kernel_pass2<vec_size><<<blocks, threads, nshared1, cuda_stream>>>(dY_data,
          X_data, mean_data, rstd_data, gamma_data, dX_data, N, pow2);
        }
      } else if (is_float) {
        layernorm::vectorized_layer_norm_grad_input_kernel<vec_f><<<blocks, threads, nshared, cuda_stream>>>(dY_data,
        X_data, mean_data, rstd_data, gamma_data, dX_data, N);
      } else {
        layernorm::vectorized_layer_norm_grad_input_kernel<vec_size><<<blocks, threads, nshared, cuda_stream>>>(dY_data,
        X_data, mean_data, rstd_data, gamma_data, dX_data, N);
      }
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    } else {
  ...
  if (dgamma->defined() || dbeta->defined()) {
    ...
    if (M < 128) {
      ...
      if ((M % kWarpSize == 0) && (N % kWarpSize == 0)) {
        ...
        bool is_opt = (!at::maca::get_maca_disable_opt_layernorm_gammabeta()) &&
                      M >= 2048 && N >= 64 ;

        if (is_opt) {
          int vec = 4;
          if (N < 96 || sizeof(T_ACC) > 4) vec = 2;       //share memory is limit

          dim3 threads(kWarpSize, kWarpSize);
          dim3 blocks((N + threads.x * vec - 1) / (threads.x * vec),
                      kWarpSize);

          size_t shmem_sz = 2 * sizeof(T_ACC) * (threads.x * vec + 1) * threads.y;
          at::Tensor dg_sum_data = at::empty({blocks.y * N}, X.options().dtype(
                                              c10::CppTypeToScalarType<T_ACC>::value));
          at::Tensor db_sum_data = at::empty({blocks.y * N}, X.options().dtype(
                                              c10::CppTypeToScalarType<T_ACC>::value));
          at::Tensor semaphores = at::zeros({blocks.x}, X.options().dtype(at::kInt));

          T_ACC* dg_sum_ptr = dg_sum_data.data_ptr<T_ACC>();
          T_ACC* db_sum_ptr = db_sum_data.data_ptr<T_ACC>();
          int* semaphores_ptr = semaphores.data_ptr<int>();

          if (vec==4) {
            layernorm::GammaBetaBackwardCUDAKernel_32x32_opt<4, T, T_ACC>
            ...
            layernorm::GammaBetaBackwardCUDAKernel_32x32_opt<2, T, T_ACC>
              <<<blocks, threads, shmem_sz, cuda_stream>>>(
                  M,
                  N,
                  dY_data,

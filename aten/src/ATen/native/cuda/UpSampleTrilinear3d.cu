...
#ifdef USE_MACA
#include "maca_kernels/UpSampleTrilinear3d_opt.cuh"
#endif
...
#ifdef USE_MACA
  if(maca_unlikely(at::maca::get_maca_enable_print_upsample3d_info())){
    printf("Upsample trilineard info: InputSize = %d,%d,%d,%d,%d, InputStride = %d,%d,%d,%d,%d, \
OutputSize = %d,%d,%d,%d,%d, OutputStride = %d,%d,%d,%d,%d, \
output_size = %d,%d,%d, align_corners = %d\n",
    ...
    output_depth, output_height, output_width, int(align_corners));
  }
#endif
...
#ifdef USE_MACA
        if (output.size(0) * output.size(1) * num_kernels > 0 &&
          input.is_contiguous(at::MemoryFormat::ChannelsLast3d) &&
          output.is_contiguous(at::MemoryFormat::ChannelsLast3d) &&
          maca_likely(!at::maca::get_maca_disable_upsample_trilinear3d_opt())) {
            int batchsize = output.size(0);
            int channels = output.size(1);
            const int nelements = num_kernels * channels;
            dim3 grid(ceil_div(nelements, num_threads), batchsize, 1);
            upsample_trilinear3d_out_frame_opt_NDHWC<scalar_t, accscalar_t>
            <<<grid,
              num_threads,
              0,
              stream>>>(

        ...
        if (is_opt) {
          ...
          if (vec == 2) {
            softmax_warp_forward_opt2_nd<input_t, output_t, acc_t, 7, 2, is_log_softmax>
                    ...
                    (dst, const_cast<input_t*>(src), batch_count, softmax_elements_stride, softmax_elements); // 128
            C10_CUDA_KERNEL_LAUNCH_CHECK();                                       \
          } else if (vec == 4) {
            if (sizeof(input_t) == 2) {
              softmax_warp_forward_opt2_nd<input_t, output_t, acc_t, 8, 4, is_log_softmax>
                    <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>
                    (dst, const_cast<input_t*>(src), batch_count, softmax_elements_stride, softmax_elements); // 256
              C10_CUDA_KERNEL_LAUNCH_CHECK();                                       \
            }
                ...
                softmax_warp_forward_512_float32<input_t, output_t, acc_t, 9, 4, is_log_softmax>
                    <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(dst, const_cast<input_t*>(src), batch_count, softmax_elements_stride, softmax_elements);
                C10_CUDA_KERNEL_LAUNCH_CHECK();                                       \
            }
                ...
                softmax_warp_forward_opt2_nd<input_t, output_t, acc_t, L2E, 4, is_log_softmax>   \
                    <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(dst,   \
                        const_cast<input_t*>(src), batch_count, softmax_elements_stride, softmax_elements); \
                C10_CUDA_KERNEL_LAUNCH_CHECK();                                       \
                break;
            ...
            softmax_warp_forward_dim_49_half_bhalf<input_t, output_t, acc_t, is_log_softmax>
                <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(dst, const_cast<input_t*>(src), batch_count, softmax_elements_stride);
            C10_CUDA_KERNEL_LAUNCH_CHECK();
        }
            ...
            softmax_warp_forward_combine_load_nd<input_t, output_t, acc_t, is_log_softmax>
                <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(dst, const_cast<input_t*>(src), batch_count, softmax_elements_stride, middle_thread);
            C10_CUDA_KERNEL_LAUNCH_CHECK();                                       \
        }
        else {
          if(sizeof(input_t) != 8){
...
template <typename input_t, typename output_t, typename acc_t, bool is_log_softmax, bool is_masked>
__global__ void softmax_warp_backward_1000_vec(
    output_t *gradInput, 
    input_t *grad, 
    input_t *output, 
    int batch_size,
    ...
    constexpr int SOFTMAX_DIM = 1000;

    using LoadT = at::native::memory::aligned_vector<input_t, VEC_SIZE>;
    using StoreT = at::native::memory::aligned_vector<output_t, VEC_SIZE>;

    const int first_batch = blockIdx.x * blockDim.y + threadIdx.y;
    if (first_batch >= batch_size) return;

    const int local_idx = threadIdx.x;
    const int step = first_batch * 1000;
    grad += step;
    output += step;
    gradInput += step;

    acc_t grad_reg[ELEMS_PER_THREAD] = {0.0f};
    acc_t output_reg[ELEMS_PER_THREAD] = {0.0f};

    input_t grad_pack0[4];
    input_t grad_pack1[4];
    input_t grad_pack2[4];
    input_t grad_pack3[4] = {0.0, 0.0, 0.0, 0.0};
    input_t output_pack0[4];
    input_t output_pack1[4];
    input_t output_pack2[4];
    input_t output_pack3[4] = {0.0, 0.0, 0.0, 0.0};
    LoadT* l_pack0 = reinterpret_cast<LoadT*>(&grad_pack0);
    *l_pack0 = (reinterpret_cast<LoadT*>(grad))[local_idx];
    LoadT* l_pack1 = reinterpret_cast<LoadT*>(&grad_pack1);
    *l_pack1 = (reinterpret_cast<LoadT*>(grad))[local_idx + 64];
    LoadT* l_pack2 = reinterpret_cast<LoadT*>(&grad_pack2);
    *l_pack2 = (reinterpret_cast<LoadT*>(grad))[local_idx + 128];

    LoadT* outputl_pack0 = reinterpret_cast<LoadT*>(&output_pack0);
    *outputl_pack0 = (reinterpret_cast<LoadT*>(output))[local_idx];
    LoadT* outputl_pack1 = reinterpret_cast<LoadT*>(&output_pack1);
    *outputl_pack1 = (reinterpret_cast<LoadT*>(output))[local_idx + 64];
    LoadT* outputl_pack2 = reinterpret_cast<LoadT*>(&output_pack2);
    *outputl_pack2 = (reinterpret_cast<LoadT*>(output))[local_idx + 128];
    if (threadIdx.x < 58){
        LoadT* l_pack3 = reinterpret_cast<LoadT*>(&grad_pack3);
        *l_pack3 = (reinterpret_cast<LoadT*>(grad))[local_idx + 192];
        LoadT* outputl_pack3 = reinterpret_cast<LoadT*>(&output_pack3);
        *outputl_pack3 = (reinterpret_cast<LoadT*>(output))[local_idx + 192];
    }
    
    #pragma unroll
    for (int j = 0; j < 4; ++j) {
        grad_reg[j] = grad_pack0[j];
        output_reg[j] = output_pack0[j];
        grad_reg[j + 4] = grad_pack1[j];
        output_reg[j + 4] = output_pack1[j];
        grad_reg[j + 8] = grad_pack2[j];
        output_reg[j + 8] = output_pack2[j];
        grad_reg[j + 12] = grad_pack3[j];
        output_reg[j + 12] = output_pack3[j];
    }

    acc_t local_sum = {0.0f};
    #pragma unroll
    for (int j = 0; j < ELEMS_PER_THREAD; ++j) {
        local_sum += grad_reg[j];
    }

    warp_reduce<acc_t, 1, 64, Add>(&local_sum);

    output_t gradInput_pack0[4];
    output_t gradInput_pack1[4];
    output_t gradInput_pack2[4];
    output_t gradInput_pack3[4];
    StoreT* out_pack0 = reinterpret_cast<StoreT*>(&gradInput_pack0);
    #pragma unroll
    for (int j = 0; j < VEC_SIZE; ++j) {
        acc_t val = (is_log_softmax) 
            ? (grad_reg[j] - __expf(output_reg[j]) * local_sum)
            : (grad_reg[j] - output_reg[j] * local_sum);
        gradInput_pack0[j] = val;
    }
    (reinterpret_cast<StoreT*>(gradInput))[local_idx + 0 * 64] = *out_pack0;

    StoreT* out_pack1 = reinterpret_cast<StoreT*>(&gradInput_pack1);
    #pragma unroll
    for (int j = 0; j < VEC_SIZE; ++j) {
        int idx = j + VEC_SIZE;
        acc_t val = (is_log_softmax) 
            ? (grad_reg[idx] - __expf(output_reg[idx]) * local_sum)
            : (grad_reg[idx] - output_reg[idx] * local_sum);
        gradInput_pack1[j] = val;
    }
    (reinterpret_cast<StoreT*>(gradInput))[local_idx + 1 * 64] = *out_pack1;

    StoreT* out_pack2 = reinterpret_cast<StoreT*>(&gradInput_pack2);
    #pragma unroll
    for (int j = 0; j < VEC_SIZE; ++j) {
        int idx = j + 2*VEC_SIZE;
        acc_t val = (is_log_softmax) 
            ? (grad_reg[idx] - __expf(output_reg[idx]) * local_sum)
            : (grad_reg[idx] - output_reg[idx] * local_sum);
        gradInput_pack2[j] = val;
    }
    (reinterpret_cast<StoreT*>(gradInput))[local_idx + 2 * 64] = *out_pack2;
    if (threadIdx.x < 58){
        StoreT* out_pack3 = reinterpret_cast<StoreT*>(&gradInput_pack3);
        #pragma unroll
        for (int j = 0; j < VEC_SIZE; ++j) {
            int idx = j + 3*VEC_SIZE;
            acc_t val = (is_log_softmax) 
                ? (grad_reg[idx] - __expf(output_reg[idx]) * local_sum)
                : (grad_reg[idx] - output_reg[idx] * local_sum);
            gradInput_pack3[j] = val;
        }
        (reinterpret_cast<StoreT*>(gradInput))[local_idx + 3 * 64] = *out_pack3;
    }
}
...
void dispatch_softmax_backward(output_t *grad_input, const input_t *grad, const input_t *output, int softmax_elements, int softmax_elements_stride, int batch_count,  const bool *mask = nullptr)
{
    ...
    if (softmax_elements == 0) {
       return;
        ...
        bool disable_softmax_opt = at::maca::get_maca_disable_softmax_opt();
        auto gi_ptr = reinterpret_cast<uintptr_t>(grad_input);
        auto grad_ptr = reinterpret_cast<uintptr_t>(grad);
        auto output_ptr = reinterpret_cast<uintptr_t>(output);
        bool aligned = (!(gi_ptr % 4)) && (!(grad_ptr % 4)) && (!(output_ptr % 4));
        if (softmax_elements == 1000 && !is_masked && aligned && !disable_softmax_opt){
            softmax_warp_backward_1000_vec<input_t, output_t, acc_t, is_log_softmax, is_masked>
                <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>
                (grad_input, const_cast<input_t*>(grad), const_cast<input_t*>(output), batch_count);
            C10_CUDA_KERNEL_LAUNCH_CHECK();    
        } else {
            switch (log2_elements) {

...
const std::string reduction_template_0 = R"ESCAPE(
  ...
  template <typename T>
  __device__ __forceinline__ T WARP_SHFL_DOWN(T value, unsigned int delta, int width = warpSize, unsigned long long mask = 0xffffffffffffffff)
  {
  #if defined(__clang__) && defined(__HIP__) && !defined(USE_MACA)
  ...
  #if ${complex}
  template <typename T>
  __device__ __forceinline__ std::complex<T> WARP_SHFL_DOWN(std::complex<T> value, unsigned int delta, int width = warpSize, unsigned long long mask = 0xffffffffffffffff)
  {

...
void i1_kernel_cuda(TensorIteratorBase& iter) {
  ...
  #else
    // here would have compile error as chebyshev_coefficients_i1e_A only defined for double & float
    // so it is not only for maca, it's would also be fail if not AT_USE_JITERATOR in cuda ACL-12239
#ifdef USE_MACA

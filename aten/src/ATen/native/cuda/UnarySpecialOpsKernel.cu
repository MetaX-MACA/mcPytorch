...
void sigmoid_kernel_cuda(TensorIteratorBase& iter) {
  ...
  if (at::isComplexType(common_dtype)) {
    ...
    #else
      AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, common_dtype, "sigmoid_cuda", [&]() {
        gpu_kernel_maca_arity1(iter, []GPU_LAMBDA(scalar_t a) -> scalar_t {
          using opmath_t = at::opmath_type<scalar_t>;
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2_DOUBLE_SPECIAL(at::ScalarType::Half, at::ScalarType::BFloat16, common_dtype, "sigmoid_cuda", [&]() {
      bool disable_sigmoid_opt = at::maca::get_maca_disable_sigmoid_opt();
      if (disable_sigmoid_opt){
        gpu_kernel_maca_arity1(iter, []GPU_LAMBDA(scalar_t a) -> scalar_t {
                using opmath_t = at::opmath_type<scalar_t>;
                const auto one = opmath_t{1};
                return static_cast<scalar_t>(one/(one + std::exp(-opmath_t{a})));
        ...
        gpu_kernel_maca_arity1(iter, []GPU_LAMBDA(scalar_t a) -> scalar_t {
                using opmath_t = at::opmath_type<scalar_t>;
                const auto one = opmath_t{1};
                return static_cast<scalar_t>(__builtin_mxc_rcpf(one + __expf(-opmath_t{a})));
        });
      }
    },
    [&]() {

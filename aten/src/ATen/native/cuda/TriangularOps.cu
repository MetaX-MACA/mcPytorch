...
template <typename scalar_t, typename IndexType, bool upper, int elements_per_thread, bool inplace>
C10_LAUNCH_BOUNDS_1(block_size)
__global__ void triu_tril_kernel_vec(
    cuda::detail::TensorInfo<scalar_t, IndexType> result_info,
  ...
  using LoadT0 = at::native::memory::aligned_vector<scalar_t, elements_per_thread>;
  using LoadT1 = at::native::memory::aligned_vector<scalar_t, elements_per_thread>;
  using StoreT = at::native::memory::aligned_vector<scalar_t, elements_per_thread>;
  LoadT0 ld_0;
  LoadT1 ld_1;
  StoreT ld_out;
  ...
  #pragma unroll
  for (int i = 0; i < elements_per_thread; i++) {
    ld_out.val[i] = scalar_t(0);
  }

  if constexpr (inplace) {
    ld_0 = *(reinterpret_cast<LoadT0*>(result_info.data + result_offset));
    #pragma unroll
    for (int i = 0; i < elements_per_thread && col + i < self_info.sizes[dims - 1]; i++) {
      bool mask = upper ? (col + i - row >= k) : (col + i - row <= k);
      if (mask) {
        ld_out.val[i] = ld_0.val[i];
      }
    }
  } else {
    bool has_mask = (upper && col + elements_per_thread - row >= k) || (!upper && col - row <= k);
    if (has_mask) {
      ld_1 = *(reinterpret_cast<LoadT1*>(self_info.data + self_offset));
      #pragma unroll
      for (int i = 0; i < elements_per_thread && col + i < self_info.sizes[dims - 1]; i++) {
        bool mask = upper ? (col + i - row >= k) : (col + i - row <= k);
        if (mask){
          ld_out.val[i] = ld_1.val[i];
        }
      }
    }
  }
  *(reinterpret_cast<StoreT*>(result_info.data + result_offset)) = ld_out;
}
...
void triu_tril_cuda_template(const Tensor& result, const Tensor& self, int64_t k, const char* name) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
    ...
    constexpr int elements_per_thread = sizeof(scalar_t) < 8 ? 16 / sizeof(scalar_t) : 1;
    ...
    bool use_vec = result.is_contiguous() && self.is_contiguous() && (sizes.back() % elements_per_thread == 0) && maca_likely(!diasble_triu_tril_opt);
    if (cuda::detail::canUse32BitIndexMath(result) && cuda::detail::canUse32BitIndexMath(self)) {
      ...
      if (use_vec){
        BOOL_SWITCH(self.is_same(result), inplace, [&] {
          ...
          triu_tril_kernel_vec<scalar_t, int32_t, upper, elements_per_thread, inplace>
      ...
      if(use_vec){
        ...
        BOOL_SWITCH(self.is_same(result), inplace, [&] {
          triu_tril_kernel_vec<scalar_t, int64_t, upper, elements_per_thread, inplace>
            <<<dim_grid, dim_block, 0, at::cuda::getCurrentCUDAStream()>>>(
              result_info, self_info, k, N_padded, last_dim_padded);
        });
      } else {

           ...
           bool ceil_mode)
{
  ...
  bool channels_last = input_.ndimension() == 5 && input_.suggest_memory_format() == at::MemoryFormat::ChannelsLast3d;
  Tensor _input = input_;
  if (input_.ndimension() == 4) {
    Tensor input_channels_last_check = input_.unsqueeze(0);
    // work around buggy behavior of suggest_memory_format here where
    // suggested format of unsqueezed tensor is contiguous while it is
    // really only contiguous in ChannelsLast3d
    channels_last = (!input_channels_last_check.is_contiguous()) &&
  ...
  if (!channels_last) {
    gradInput.resize_as_(input_, at::MemoryFormat::Contiguous);
  } else {
  ...
  const int64_t otime = gradOutput_.size(-3);
  const int64_t oheight = gradOutput_.size(-2);
  const int64_t owidth = gradOutput_.size(-1);
  ...
  Tensor work_indices;
  if (!channels_last) {
    work_grad_output = gradOutput_.contiguous();

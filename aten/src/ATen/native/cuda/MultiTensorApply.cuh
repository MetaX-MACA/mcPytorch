...
C10_LAUNCH_BOUNDS_1(kBlockSize)
__global__ void
multi_tensor_apply_kernel_opt(
    void *tensorListMeta,
    U callable,
    ArgTypes... args) {
  // Hand the chunk information to the user-supplied functor to process however it likes.
  auto dTensorListMeta = reinterpret_cast<T *>(tensorListMeta);
  callable(kChunkSize, dTensorListMeta, args...);
}
...
void multi_tensor_apply_opt(
    ...
    ArgTypes... args) {
        ...
        auto dStruct = allocator.allocate(sizeof(TensorListScalarListMetadata<scalar_vals_t, depth>));
            ...
            for (int chunk = 0; chunk < chunks; chunk++) {
                ...
                if (tensors_full || blocks_full || last_chunk) {
                    AT_CUDA_CHECK(cudaMemcpyAsync(
                            dStruct.get(), &tensorListMeta,
                            sizeof(TensorListScalarListMetadata<scalar_vals_t, depth>),
                            cudaMemcpyHostToDevice,
                            c10::cuda::getCurrentCUDAStream()
                        ));
                    AT_CUDA_CHECK(cudaStreamSynchronize(c10::cuda::getCurrentCUDAStream()));
                    multi_tensor_apply_kernel_opt<TensorListScalarListMetadata<scalar_vals_t, depth>><<<loc_block_info, kBlockSize, 0, at::cuda::getCurrentCUDAStream()>>>(
                        dStruct.get(),
                        callable,
                        args...);
                    C10_CUDA_KERNEL_LAUNCH_CHECK();
...
void multi_tensor_apply_opt(
    ...
    ArgTypes... args) {
        ...
        auto& allocator = *c10::cuda::CUDACachingAllocator::get();
        auto dStruct = allocator.allocate(sizeof(TensorListMetadata<depth>));
        ...
        for(size_t t = 0; t < n_tensors; t++) {
            ...
            for (int chunk = 0; chunk < chunks; chunk++) {
                ...
                if (tensors_full || blocks_full || last_chunk) {
                    AT_CUDA_CHECK(cudaMemcpyAsync(dStruct.get(), &tensorListMeta, sizeof(TensorListMetadata<depth>), cudaMemcpyHostToDevice, c10::cuda::getCurrentCUDAStream()));
                    AT_CUDA_CHECK(cudaStreamSynchronize(c10::cuda::getCurrentCUDAStream()));
                    multi_tensor_apply_kernel_opt<TensorListMetadata<depth>><<<loc_block_info, kBlockSize, 0, at::cuda::getCurrentCUDAStream()>>>(
                        dStruct.get(),
                        callable,
                        args...);
                    C10_CUDA_KERNEL_LAUNCH_CHECK();
...
template<int depth, typename T, typename... ArgTypes>
void multi_tensor_apply_for_fused_optimizer_opt(
    std::vector<std::vector<at::Tensor>>& tensor_lists,
    at::TensorList state_steps,
    T callable,
    ArgTypes... args) {
  ...
  auto& allocator = *c10::cuda::CUDACachingAllocator::get();
  auto dStruct = allocator.allocate(sizeof(FusedOptimizerTensorListMetadata<depth>));
  ...
  for (const auto & tensor_index : c10::irange(num_tensors)) {
    ...
    for (const auto & chunk : c10::irange(chunks)) {
      ...
      if (tensor_full || blocks_full || last_chunk) {
        AT_CUDA_CHECK(cudaMemcpyAsync(dStruct.get(), &tensorListMeta, sizeof(FusedOptimizerTensorListMetadata<depth>), cudaMemcpyHostToDevice, c10::cuda::getCurrentCUDAStream()));
        AT_CUDA_CHECK(cudaStreamSynchronize(c10::cuda::getCurrentCUDAStream()));
        multi_tensor_apply_kernel_opt<FusedOptimizerTensorListMetadata<depth>><<<loc_block_info, kBlockSize, 0, at::cuda::getCurrentCUDAStream()>>>(
            dStruct.get(),
            callable,
            args...);
        C10_CUDA_KERNEL_LAUNCH_CHECK();

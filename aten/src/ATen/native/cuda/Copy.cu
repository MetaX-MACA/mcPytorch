...
#ifdef USE_MACA
#include <ATen/native/cuda/maca_kernels/Loops_copy.cuh>
#else
...
void direct_copy_kernel_cuda(TensorIteratorBase &iter) {
  ...
  if (isQIntType(dtype)) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND6(
        kHalf, kBool, kBFloat16, kComplexHalf, kFloat8_e4m3fn, kFloat8_e5m2, dtype, "copy_", [&] {
          gpu_kernel_maca_arity1_copy(iter, [] GPU_LAMBDA(scalar_t x) { return x; });
    });
  }
#else
  ...
  if (isQIntType(dtype)) {
    AT_DISPATCH_QINT_TYPES(dtype, "copy_", [&] {
     ...
     float8_copy_kernel_cuda(iter);
// #ifndef USE_MACA  // call gpu_kernel_maca_arity1_copy for maca
...
void copy_device_to_device(TensorIterator& iter,
  ...
  if (memcpy_eligible) {
    ...
    #ifdef USE_MACA
      if (src_device == dst_device && size <= CUDACachingAllocator::getSmallMemcpyByteSize() &&
          maca_likely(!at::maca::get_maca_disable_direct_copy_kernel_replace_memcpy())) {

...
__global__ void MACA_CatArrayBatchedCopyAdaptGridSpe(
    ...
    *p_input = *reinterpret_cast<const LoadT*>(data + tid);
    if (blockIdx.y == 1){
      #pragma unroll
      for(int i = 0;i < vec;i++){
        IndexType elementOffset = CatArrIndexToOffset<IndexType, Dims>::compute(
...
__global__ void MACA_CatArrayBatchedCopyNoPartialWrite(
    ...
    if (output_coord[1] >= input0_size1){
      ...
      *p_input = *reinterpret_cast<const LoadT*>(inputs.input[1] + input1_offset);
    } else{
      ...
      LoadT* p_input = reinterpret_cast<LoadT*>(&output_data);
      *p_input = *reinterpret_cast<const LoadT*>(inputs.input[0] + input0_offset);
    }
...
__global__ void MACA_CatArrayBatchedCopyNoPartialWrite_3_4_3_0(
    ...
    if (output_coord[3] >= (input0_size3 + input1_size3)){
      ...
      *p_input = *reinterpret_cast<const LoadT*>(inputs.input[2] + input2_offset);
    } 
...
__global__ void MACA_CatArrayBatchedCopyNoPartialWrite_any_any_any_0(
    ...
    for (int i = infos.input_tensor_num - 1; i >= 0; i--){
      ...
      if (output_coord[concatDim] >= threshold){
        ...
        *p_input = *reinterpret_cast<const LoadT*>(inputs.input[i] + offset);
        StoreT* p_pack0 = reinterpret_cast<StoreT*>(&output_data);
        (reinterpret_cast<StoreT*>(output))[tid] = *p_pack0;
        break;
      }
...
template <typename T, typename IndexType, int Dims, int vec, int batch_size, int stride_size>
__global__ void MACA_CatArrayBatchedCopyAdaptGrid_special(
    T* output,
    const T* input0, const T* input1, IndexType offset0, IndexType offset1, IndexType dimSize0, IndexType dimSize1, IndexType nElements0, IndexType nElements1,
    TensorSizeStride<IndexType, CAT_ARRAY_MAX_INPUT_DIMS> os,
    const int concatDim,
    IndexType dimStride) {
    ...
    if (blockIdx.y == 0){
      IndexType nElements = nElements0;
      if(tid >= nElements) return;
      const T* data = input0;
      IndexType offset = offset0;
      IndexType dimSize = dimSize0;
      IndexType dataOffset = offset * dimStride;
      using LoadT = at::native::memory::aligned_vector<T, vec>;
      T vec_input[vec];
      LoadT* p_input = reinterpret_cast<LoadT*>(&vec_input);
      *p_input = *reinterpret_cast<const LoadT*>(data + tid);
      ...
      IndexType nElements = nElements1;
      if(tid >= nElements) return;
      const T* data = input1;
      IndexType offset = offset1;
      IndexType dimSize = dimSize1;
      IndexType dataOffset = offset * dimStride;
      using LoadT = at::native::memory::aligned_vector<T, vec>;
      T vec_input[vec];
      LoadT* p_input = reinterpret_cast<LoadT*>(&vec_input);
...
void maca_parallel_cat_not_allcontiguous(const Tensor &out, const MaterializedITensorListRef& inputs, int64_t dimension,
  ...
  scalar_t *data = (scalar_t *)(out.data_ptr());
  ...
  for (batchCounter = 0; batchCounter < inputs.size(); ++batchCounter) {
    catMetaData.input[batchCounter] = (scalar_t*)inputs[batchCounter].get().data_ptr();
  }
  const int vec8 = 8;
  const int vec4 = 4;
  const int vec2 = 2;
  int not_allcontiguous_no_partial_write_batch_threshold = at::maca::get_maca_cat_not_allcontiguous_batch_threshold();
  //Get grid where x dim fills half gpu and y dim is number of tensors.
  //This will have cating two tensors fill the entire grid, but prevent
  //many threads from needlessly load meta data if their sizes is small.
  if(out.numel() % num_threads_per_block_no_partial == 0 && batchCounter < not_allcontiguous_no_partial_write_batch_threshold){
...
void maca_parallel_cat_not_allsamedtype(const Tensor &out, const MaterializedITensorListRef& inputs, int64_t dimension,
  ...
  const int vec4 = 4;
  const int vec2 = 2;
  int not_allsamedtype_no_partial_write_batch_threshold = at::maca::get_maca_cat_not_allsamedtype_batch_threshold();
  //Get grid where x dim fills half gpu and y dim is number of tensors.
  //This will have cating two tensors fill the entire grid, but prevent
  //many threads from needlessly load meta data if their sizes is small.
  if(out.numel() % num_threads_per_block_no_partial == 0 && all_types_in_float_half_bhalf && batchCounter < not_allsamedtype_no_partial_write_batch_threshold){
    bool is_all_input_concat_dimension_vec8 = true;
    bool is_all_input_concat_dimension_vec4 = true;
    bool is_all_input_concat_dimension_vec2 = true;
    CatArrTensorMetadataNoPartialWriteInfo infos;
...
void maca_parallel_cat(const Tensor &out, const MaterializedITensorListRef &inputs, int64_t dimension,
  ...
  for (int i = 0; i < inputs.size() ; i += batch_size) {
    ...
    for (batchCounter = 0;
      ...
      catMetaData.input[batchCounter] = (scalar_t*)inputs[i+batchCounter].get().data_ptr();
    ...
    int contiguous_no_partial_write_batch_threshold = at::maca::get_maca_cat_contiguous_batch_threshold();
    bool aligned = true;
    ...
    for (int m = 0; m < batchCounter; m++){
      uint64_t addr = reinterpret_cast<uint64_t>(inputs[i + m].get().data_ptr());
    ...
    if (!disable_cat_opt && input_output_all_aligned && dimension == 3 && batchCounter == 2 && memory_format == c10::MemoryFormat::ChannelsLast && nDims == 4 && out.numel() % (num_threads_per_block_no_partial * vec8) == 0 && inputs[0].get().size(1) % vec8 == 0 && inputs[1].get().size(1) % vec8 == 0){
    ...
    else if (!disable_cat_opt && input_output_all_aligned && memory_format == c10::MemoryFormat::Contiguous && out.numel() % num_threads_per_block_no_partial == 0 && batchCounter < contiguous_no_partial_write_batch_threshold){
    ...
    else if (!disable_cat_opt && aligned && can_vectorize8 && !std::is_same<scalar_t, float>::value){
      getAdaptCatGrid(batchCounter, catGrid, max_input_numel, vec8);
      #define HANDLE_CASE_OPT(DIMS) \
      ...
      if (batchCounter == 2) {
        #define HANDLE_CASE_OPT(DIMS) \
        MACA_CatArrayBatchedCopyAdaptGrid_special<scalar_t, unsigned int, DIMS, vec4, batch_size, stride_size><<<\
            catGrid, applyBlock, 0, stream.stream()>>>(\
                data, catMetaData.input[0], catMetaData.input[1], catMetaData.offset[0], catMetaData.offset[1], catMetaData.dimSize[0], catMetaData.dimSize[1], catMetaData.nElements[0], catMetaData.nElements[1], outputParam, dimension, outputParam.tensorStride[dimension]); \
        C10_CUDA_KERNEL_LAUNCH_CHECK();
        switch (nDims) {
          case 1:
            HANDLE_CASE_OPT(1);
...
__global__ void maca_cat_for_dim5_copy_contiguous(scalar_t* dst, scalar_t* src0, scalar_t* src1, int64_t N){
...
template <typename scalar_t>
__global__ void maca_cat_for_dim5_copy_non_contiguous_same_inputs_stride(scalar_t* dst, scalar_t* src0, scalar_t* src1, int size0, int size1, int size2, int size3, int size4, int stride00, int stride10, int stride20, int stride30, int stride40, int64_t N){
...
void maca_parallel_cat_coalesce_write_copy_contiguous(const Tensor &out, const MaterializedITensorListRef &inputs, int64_t dimension,
    ...
    auto output_ptr = (scalar_t*)out.data_ptr();
    auto input0_ptr = (scalar_t*)inputs[0].get().data_ptr();
    auto input1_ptr = (scalar_t*)inputs[1].get().data_ptr();
    maca_cat_for_dim5_copy_contiguous<scalar_t><<<blocks, threads>>>(output_ptr, input0_ptr, input1_ptr, inputs[0].get().numel());
    return;
}
...
void maca_parallel_cat_coalesce_write_copy_non_contiguous(const Tensor &out, const MaterializedITensorListRef &inputs, int64_t dimension,
    ...
    if (stride00 == stride01 && stride10 == stride11 && stride20 == stride21 && stride30 == stride31 && stride40 == stride41){
      maca_cat_for_dim5_copy_non_contiguous_same_inputs_stride<<<blocks, threads>>>(output_ptr, input0_ptr, input1_ptr, size0, size1, size2, size3, size4, stride00, stride10, stride20, stride30, stride40, inputs[0].get().numel());
    } else{
      maca_cat_for_dim5_copy_non_contiguous_not_same_inputs_stride<<<blocks, threads>>>(output_ptr, input0_ptr, input1_ptr, size0, size1, size2, size3, size4, stride00, stride10, stride20, stride30, stride40, stride01, stride11, stride21, stride31, stride41, inputs[0].get().numel());
    }
    return;
}
...
bool is_scalar_type_in_targets(c10::ScalarType scalar_type) {
    const std::unordered_set<c10::ScalarType> target_types = {
        at::kDouble,       // double
        at::kFloat,        // float
        at::kHalf,         // khalf
        at::kBFloat16,     // kbfloat16
        at::kBool,         // kBool
        at::kComplexHalf,  // kcomplexhalf
        at::kComplexDouble,// complexdouble
        at::kComplexFloat  // complexfloat
    };
    return target_types.count(scalar_type) > 0;
}
...
(const ITensorListRef& tensors,
 ...
 const Tensor& result) {
  ...
  if (materialized.size() == 2 && materialized[0].get().dim() == 4) {
    if (dim == 2 && ((materialized[0].get().size(0) == 4) && (materialized[0].get().size(1) == 40) && (materialized[0].get().size(3) == 128) && (materialized[0].get().stride(2) == 128) && (materialized[0].get().stride(3) == 1)) \
    && ((materialized[1].get().size(0) == 4) && (materialized[1].get().size(1) == 40) && (materialized[1].get().size(3) == 128) && (materialized[1].get().stride(2) == 5120) && (materialized[1].get().stride(3) == 1))) {
      disable_opt_cat_for_not_allcontiguous = true;
    }
  }
  bool enable_cat_contiguous_using_copy = at::maca::get_maca_enable_cat_contiguous_using_copy();

#if defined(USE_MACA)
  if (!enable_cat_contiguous_using_copy && materialized.size() > 1 &&
      result.dim() <= CAT_ARRAY_MAX_INPUT_DIMS &&
      at::cuda::detail::canUse32BitIndexMath(result) &&
      all_contiguous &&
      all32BitIndexable &&
      all_same_dtype && is_scalar_type_in_targets(result.scalar_type())) {
      ...
      } else if (!enable_cat_contiguous_using_copy && materialized.size() > 1 &&
        result.dim() <= CAT_ARRAY_MAX_INPUT_DIMS &&
        at::cuda::detail::canUse32BitIndexMath(result) &&
        all_contiguous &&
        all32BitIndexable &&
        ...
        if (isBitsType(result.scalar_type())) {
          AT_DISPATCH_BIT_TYPES(result.scalar_type(), "cat_cuda", [&]() {
            ...
            maca_parallel_cat<dtype, CAT_ARRAY_BATCH_SIZE, 1>(result, materialized, dim, nDims, memory_format);
          });
      } else {
        AT_DISPATCH_V2(
            result.scalar_type(),

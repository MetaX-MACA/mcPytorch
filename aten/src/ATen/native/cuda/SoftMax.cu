...
#include <ATen/native/cuda/maca_kernels/SoftMax.cuh>
...
struct LogSoftMaxForwardEpilogue<T, float, OutT> {
  __device__ __forceinline__ LogSoftMaxForwardEpilogue(float max_input, float sum)
    : max_input(max_input),  logsum(__logf(sum)) {}
...
template<typename T, typename OutT>
struct LogSoftMaxBackwardEpilogue<T, float, OutT> {
  __device__ __forceinline__ LogSoftMaxBackwardEpilogue(float sum)
    : sum(sum) {}

  __device__ __forceinline__ T operator()(OutT gradOutput, OutT output) const {
    return static_cast<T>(gradOutput - __expf(static_cast<float>(output)) * sum);
  }
...
template<typename T, typename OutT>
struct SoftMaxForwardEpilogue<T, float, OutT> {
...
__global__ void cunn_SpatialSoftMaxForward_special(
    ...
    uint32_t outer_size, uint32_t dim_size, uint32_t inner_size)
{
  ...
  using LoadT = at::native::memory::aligned_vector<float, 8>;
  using StoreT = at::native::memory::aligned_vector<float, 8>;
  const uint32_t outer_offset = blockIdx.x * outer_stride;
  float max_input[8] = {at::numeric_limits<float>::lowest()};
  float sum[8] = {0};
  float input_vec[64][8];
  uint32_t iteration = 0;
  for (uint32_t inner_index = threadIdx.x; inner_index < dim_size; inner_index += blockDim.x, iteration += 1) {
    const uint32_t data_offset = outer_offset + inner_index * 8;
    LoadT* l_pack = reinterpret_cast<LoadT*>(&input_vec[iteration]);
    *l_pack = (reinterpret_cast<LoadT*>(input + data_offset))[0];
    #pragma unroll
    for (uint32_t i = 0; i < 8; i++){
      max_input[i] = Max<float>()(max_input[i], input_vec[iteration][i]);
    }
  }
  #pragma unroll
  for (uint32_t i = 0; i < 8; i++){
    max_input[i] = spatialBlockReduceX<float, Max>(smem[i], max_input[i]);
  }
  iteration = 0;
  for (uint32_t inner_index = threadIdx.x; inner_index < dim_size; inner_index += blockDim.x, iteration += 1) {
    #pragma unroll
    for (uint32_t i = 0; i < 8; i++){
      sum[i] += __expf(static_cast<float>(input_vec[iteration][i]) - max_input[i]);
    }
  }
  #pragma unroll
  for (uint32_t i = 0; i < 8; i++){
    sum[i] = spatialBlockReduceX<float, Add>(smem[i], sum[i]);
  }
  iteration = 0;
  for (uint32_t inner_index = threadIdx.x; inner_index < dim_size; inner_index += blockDim.x, iteration += 1) {
    const uint32_t data_offset = outer_offset + inner_index * 8;
    ...
    #pragma unroll
    for (uint32_t i = 0; i < 8; i++){
      Epilogue<float, float, float> epilogue(max_input[i], sum[i]);
      result[i] = epilogue(input_vec[iteration][i]);
    }
    StoreT* p_pack = reinterpret_cast<StoreT*>(&result);
    (reinterpret_cast<StoreT*>(output + data_offset))[0] = *p_pack;
  }
}
...
// default for SoftMaxForwardEpilogue
template <int ILP, typename scalar_t, typename accscalar_t, typename outscalar_t, template<typename, typename, typename> class Epilogue>
__global__ void
cunn_SoftMaxBackward(scalar_t *gradInput, outscalar_t *output, outscalar_t *gradOutput, int64_t classes)
{
  using LoadT = at::native::memory::aligned_vector<scalar_t, ILP>;
  using StoreT = at::native::memory::aligned_vector<outscalar_t, ILP>;
...
Tensor host_softmax(const Tensor & input_, const int64_t dim_, const bool half_to_float, const Tensor& output){
  ...
  if (input.numel() > 0) {
    bool print_softmax_info = at::maca::get_maca_print_softmax_shape();
    if(maca_unlikely(print_softmax_info)){
      printf("softmax info: shape(");
      for (int64_t i = 0; i < input.dim(); ++i)
      {printf("%d,",input.size(i));}
      printf(") stride(");
      for (int64_t i = 0; i < input.dim(); ++i)
      {printf("%d,",input.stride(i));}
      printf(")  dim:%d half_to_float:%d  is_log_softmax:%d\n",dim,half_to_float,is_log_softmax);
    }
    ...
    if (inner_size == 1) {
      ...
      auto disable_softmax_opt = at::maca::get_maca_disable_softmax_opt();
      AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "host_softmax", [&] {
        using accscalar_t = acc_type<scalar_t, true>;
        bool is_scalar_t_double = std::is_same<double, scalar_t>::value;
        bool is_accscalar_t_double = std::is_same<double, accscalar_t>::value;
        if (!half_to_float) {
          if (dim_size <= 1024 && dim_size*sizeof(scalar_t) <= 4096) {
            ...
            const int share_size = sizeof(accscalar_t) * warp_num;
            if(!(dim_size % ILP) && !is_scalar_t_double && !is_accscalar_t_double){
              accscalar_t max_default = -at::numeric_limits<accscalar_t>::max();
              int block_size = block.x;
              int num_packs = dim_size / ILP;

              auto output_ptr = output.data_ptr<scalar_t>();
              auto input_ptr = input.data_ptr<scalar_t>();
              auto ip = reinterpret_cast<uintptr_t>(input_ptr);
              auto op = reinterpret_cast<uintptr_t>(output_ptr);
              bool ip_aligned = !(ip % (ILP * sizeof(scalar_t)));
              bool op_aligned = !(op % (ILP * sizeof(scalar_t)));
              int packs_num = num_packs / block_size;
              int packs_rem = num_packs % block_size;
              // XXX: packs_rem > 0 will cause accumulative round-off error.
              // so use PYTORCH_DISABLE_SOFTMAX_OPT to disable optimization if requiring high accuracy.
              if (ip_aligned && op_aligned && !(packs_rem && disable_softmax_opt)){
                if (warp_num == 8) {
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 4, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 3, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 2, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 1, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                  ...
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 4, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 3, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 2, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 1, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                    cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, scalar_t, Epilogue>
                  ...
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 4, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 3, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 2, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 1, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                    cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, scalar_t, Epilogue>
                  ...
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 4, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 3, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 2, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 1, scalar_t, accscalar_t, scalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                    cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, scalar_t, Epilogue>
              ...
              // TODO(liuyuxin): cunn_SoftMaxForward_opt is not generic, need to be fixed
              //   cunn_SoftMaxForward_opt<ILP, scalar_t, accscalar_t, scalar_t, Epilogue>
              //     <<<grid, block, share_size, stream>>>(
              //         output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size, block_size,
              //         num_packs, max_default, warp_num);
              cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, scalar_t, Epilogue>
                <<<grid, block, block.x * sizeof(accscalar_t), stream>>>(
                  output.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(), dim_size);
              }
          ...
          if (dim_size <= 1024 && dim_size*sizeof(scalar_t) <= 4096) {
            ...
            if(!(dim_size % ILP) && !is_scalar_t_double && !is_accscalar_t_double){
              ...
              auto op = reinterpret_cast<uintptr_t>(output_ptr);
              bool ip_aligned = !(ip % (ILP * sizeof(scalar_t)));
              bool op_aligned = !(op % (ILP * sizeof(accscalar_t)));
              int packs_num = num_packs / block_size;
              int packs_rem = num_packs % block_size;
              if (ip_aligned && op_aligned && !(packs_rem && disable_softmax_opt)){
                if (warp_num == 8) {
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 4, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 3, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 2, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 8, 1, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                  ...
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 4, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 3, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 2, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 4, 1, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                    cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, accscalar_t, Epilogue>
                  ...
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 4, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 3, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 2, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 2, 1, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                    cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, accscalar_t, Epilogue>
                  ...
                  if (packs_num == 4) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 4, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 3) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 3, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 2) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 2, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else if (packs_num == 1) {
                    cunn_SoftMaxForward_opt_no_shm_nd<ILP, 1, 1, scalar_t, accscalar_t, accscalar_t, is_log_softmax>
                      <<<grid, block, share_size, stream>>>(
                          output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, max_default, packs_rem);
                  } else {
                    cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, accscalar_t, Epilogue>
                ...
                // TODO(liuyuxin): cunn_SoftMaxForward_opt is not generic, need to be fixed
                // cunn_SoftMaxForward_opt<ILP, scalar_t, accscalar_t, accscalar_t, Epilogue>
                //   <<<grid, block, share_size, stream>>>(
                //       output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size, block_size,
                //       num_packs, max_default, warp_num);
                cunn_SoftMaxForward<ILP, scalar_t, accscalar_t, accscalar_t, Epilogue>
                  <<<grid, block, block.x * sizeof(accscalar_t), stream>>>(
                    output.data_ptr<accscalar_t>(), input.data_ptr<scalar_t>(), dim_size);
    ...
    // Reductions over dim are done in a single-threaded manner.
      ...
      AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "host_softmax", [&] {
	    ...
	    "host_softmax_launcher", [&] {
            if (!half_to_float) {
                bool disable_softmax_internal_dim_opt = at::maca::get_maca_disable_softmax_internal_dim_opt();
                if (!disable_softmax_internal_dim_opt && outer_size == 600 && dim_size == 7200 && inner_size == 8 && dim == 2 && std::is_same<scalar_t, float>::value && reinterpret_cast<uint64_t>(input.data_ptr<scalar_t>()) % 4 == 0 && reinterpret_cast<uint64_t>(output.data_ptr<scalar_t>()) % 4 == 0){
                    grid = dim3(outer_size,1, 1);
                    block = dim3(256, 1, 1);
                    cunn_SpatialSoftMaxForward_special<scalar_t, accscalar_t, scalar_t, Epilogue>
                      <<<grid, block, 0, stream>>>(
  ...
  if (inner_size == 1) {
    ...
    if (!half_to_float) {
      if (dim_size <= 1024 && dim_size*sizeof(scalar_t) <= 4096) {
        ...
        dim3 block = SoftMax_getBlockSize(ILP, dim_size);
        const int warp_num = block.x / C10_WARP_SIZE;
        const int shift = ((uint64_t)gI.data_ptr<scalar_t>()) % ALIGN_BYTES / sizeof(scalar_t);
        const int output_shift = ((uint64_t)output.data_ptr<scalar_t>()) % ALIGN_BYTES / sizeof(scalar_t);
        const int grad_output_shift = ((uint64_t)grad.data_ptr<scalar_t>()) % ALIGN_BYTES / sizeof(scalar_t);
        bool disable_softmax_backward_opt = at::maca::get_maca_disable_softmax_backward();
        if (!disable_softmax_backward_opt && !(dim_size % ILP) && dim_size / ILP % block.x == 0 && warp_num == 2 && shift == 0 && output_shift == 0 && grad_output_shift == 0){
          const int share_size = sizeof(accscalar_t) * warp_num;
          cunn_SoftMaxBackward_opt2_no_shm<ILP, scalar_t, accscalar_t, scalar_t, Epilogue, 0>
         <<<grid, block, share_size, stream>>>(
...
Tensor masked_softmax_cuda(const Tensor& input_, const Tensor& mask_, const std::optional<int64_t> dim_, const std::optional<int64_t> mask_type_) {
  auto input_contiguous = input_.contiguous();
  Tensor output = at::empty_like(input_contiguous, input_contiguous.options());
  ...
  bool is_BxT_mask = (mask_type == 1) && (input_contiguous.dim() == 4 && mask_.dim() == 2 && input_contiguous.size(0) == mask_.size(0) && input_contiguous.size(2) == mask_.size(1) && input_contiguous.size(3) == mask_.size(1));
  ...
  bool is_TxT_mask = (mask_type == 0) && input_contiguous.dim() == 4 && mask_.dim() == 2 && input_contiguous.size(3) == mask_.size(1) && input_contiguous.size(2) == mask_.size(0) && mask_.size(0) == mask_.size(1);
  // If mask_type == 2, then mask_.sizes() must equal input_.sizes()
  TORCH_CHECK(mask_.sizes() == input_contiguous.sizes() || is_BxT_mask || is_TxT_mask, "Mask shape should match input. mask: ", mask_.sizes(), " input: ", input_contiguous.sizes());

  auto input = input_contiguous.dim() == 0 ? input_contiguous.view(1) : input_contiguous;

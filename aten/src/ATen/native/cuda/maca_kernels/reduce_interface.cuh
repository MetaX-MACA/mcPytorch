...
static ReduceKernelType get_reduce_kernel_type(const TensorIterator& iter, bool is_not_over_i32_scope) {
  ReduceConfigMaca config_maca = setReduceConfigMaca<arg_t, scalar_t, out_scalar_t, vt0>(iter, ReduceContinuousKernel);
  if(maca_likely(!at::maca::get_maca_disable_continuous_reduce_transpose_kernel()) \
    && is_not_over_i32_scope && is_ndim_3_reduce_dims_1_transpose_all_continuous_case(iter, sizeof(scalar_t), sizeof(out_scalar_t))){
    return ReduceContinuousTransposeKernel;
  } else if (maca_likely(!at::maca::get_maca_disable_continuous_reduce_kernel()) && is_launch_continuous_reduce_kernel(iter, sizeof(scalar_t), sizeof(out_scalar_t)) &&
      (config_maca.should_block_x_reduce() || is_ndim_3_reduce_dims_1_transpose_partially_continuous_case(iter, sizeof(scalar_t), sizeof(out_scalar_t)))) {
  ...
  } else if (maca_likely(!at::maca::get_maca_disable_imcontinuous_reduce_kernel()) && is_launch_imcontinuous_reduce_kernel(iter, sizeof(scalar_t), sizeof(out_scalar_t))) {
...
static void launch_reduce_maca_kernel(const TensorIterator& tensor_iterator, const R& reduction, bool is_over_i32_scope) {
  ...
  if (maca_unlikely(at::maca::get_maca_enable_print_reduce_kernel())) {
    if (reduce_kernel_type == ReduceContinuousKernel || reduce_kernel_type == ReduceImcontinuousKernel || reduce_kernel_type == ReduceContinuousTransposeKernel \
        || reduce_kernel_type == ReduceContinuousMultiReduceDimsKernel) {
  ...
  } else if (reduce_kernel_type == ReduceContinuousTransposeKernel){
    launch_continuous_transpose_kernel<scalar_t, out_scalar_t,
          arg_t>(config_maca, reduction, tensor_iterator);
  } else {
...
template <typename scalar_t, typename out_scalar_t, typename arg_t, typename reduction_t>
static void print_reduce_info(
    ReduceKernelType reduce_type,
    const TensorIterator& iter,
    reduction_t reduction) {
  std::stringstream ss;
  ss << "reduce_type: " << static_cast<typename std::underlying_type<ReduceKernelType>::type>(reduce_type);
  ...
  out_scalar_t temp2;
  arg_t temp3;
  // print demangled type name
  ss << " scalar_t: " << abi::__cxa_demangle(typeid(temp1).name(), nullptr, nullptr, nullptr) \
  <<  " out_scalar_t: " << abi::__cxa_demangle(typeid(temp2).name(), nullptr, nullptr, nullptr) \
  << " arg_t: " << abi::__cxa_demangle(typeid(temp3).name(), nullptr, nullptr, nullptr) \
  << " op_t: " << abi::__cxa_demangle(typeid(reduction.ops).name(), nullptr, nullptr, nullptr);

...
static void launch_legacy_kernel_maca_3_3_broadcast(
    ...
    stride_t stride10, stride_t stride11, stride_t stride12, stride_t stride13,
    stride_t stride20, stride_t stride21, stride_t stride22, stride_t stride23,
    const int vec, const func_t& f) {assert(0);}
...
void gpu_kernel_impl_maca_arity3(TensorIteratorBase& iter, const func_t& f) {
  ...
  if (!dynamic_casting) {
    if (contiguous) {
      get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_vec_1_3", f);
      launch_vectorized_kernel(numel, f, data);
    } else {
        bool isElemOpt = false;
        if (ndim == 2 && narity == 3) {
          get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_2_3",f);
          ...
          if(maca_likely(!at::maca::get_maca_disable_elementwise_3_3_broadcast_kernel()) && ((sizeof(arg0_t) == 4 && sizeof(arg1_t) == 4 &&
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_3_broadcast", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_3", f);
    ...
    if (contiguous) {
      ...
      get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_unroll_1_3", f);
      launch_unrolled_kernel(numel, f, data, input_offset_calculator, output_offset_calculator, loader, storer);
    } else {
      bool isElemOpt = false;
      if(ndim == 3 && narity == 3){
        get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_3_cast",f);

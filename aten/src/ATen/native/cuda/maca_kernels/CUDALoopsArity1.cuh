...
__global__ void vectorized_elementwise_kernel_unary_opt(int N, func_t f, char* data0, char* data1) {
  ...
  if (idx < N) {
    res_t tmp_load[vt];
    auto offset = idx * st;
    using LoadT = memory::aligned_vector<res_t, vt>;
...
__global__ void elementwise_kernel_2_1_input_lowdim_contiuous(
    ...
    func_t f) {
  // ndim = 2, arity = 1, narg = 2
  using load_vec = memory::aligned_vector<arg0_t, vt>;
  using save_vec = memory::aligned_vector<res_t, vt>;
    ...
    if (contiguous) {
      ...
      launch_vectorized_kernel_arity1(numel, f, data);
        ...
        if (ndim == 1) {
            ...
            if (maca_likely(!disable_opt_1_1)) {

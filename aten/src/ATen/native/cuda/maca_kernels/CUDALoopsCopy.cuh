...
template<typename func_t, typename size_t, typename return_t, typename args_t, int vt>
C10_LAUNCH_BOUNDS_1(num_threads())
__global__ void unrolled_elementwise_kernel_copy_cast(func_t f, char* data0, char* data1)
{
...
template<typename func_t, typename size_t, typename return_t, typename args_t, int vt>
C10_LAUNCH_BOUNDS_1(num_threads())
__global__ void unrolled_elementwise_kernel_copy_cast_no_align(int64_t N, func_t f, char* data0, char* data1)
{
  int64_t idx = blockIdx.x;
  int thread_idx = threadIdx.x;
      ...
      char* data0, char* data1)
{
  ...
  if (st0 == ScalarType::Half && st1 == ScalarType::Float) {
    ...
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, at::Half, float, 4>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, at::Half, float, 4>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
  } else if (st0 == ScalarType::Float && st1 == ScalarType::Half) {
    int vt = 4;
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, float, at::Half, 4>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, float, at::Half, 4>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, at::BFloat16, float, 4>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, at::BFloat16, float, 4>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, float, at::BFloat16, 4>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, float, at::BFloat16, 4>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, int, float, 4>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, int, float, 4>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, at::Half, int, 4>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
       unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, at::Half, int, 4>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, long, int, 2>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, long, int, 2>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, int, long, 2>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, int, long, 2>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, at::Half, at::BFloat16, 8>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, at::Half, at::BFloat16, 8>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, at::BFloat16, at::Half, 8>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, at::BFloat16, at::Half, 8>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, float, long, 2>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, float, long, 2>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, bool, float, 4>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, bool, float, 4>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
    ...
    int64_t grid = (N + (block * vt) - 1) / (block * vt);
    if (N % (block * vt) == 0) {
      unrolled_elementwise_kernel_copy_cast<func_t, size_t, long, bool, 2>
        <<<grid, block, 0, stream>>>(f, data0, data1);
    } else {
      unrolled_elementwise_kernel_copy_cast_no_align<func_t, size_t, long, bool, 2>
        <<<grid, block, 0, stream>>>(N, f, data0, data1);
    }
  } else {
    assert(0);
...
template<int nt, typename res_t , typename arg0_t, typename func_t, typename index_t, typename stride_t, int vt, int shmSize>
C10_LAUNCH_BOUNDS_2(nt, 4)
__global__ void elementwise_kernel_4_1_transpose02(
    int64_t N,
    char* data0, char* data1,
    index_t size0, index_t size1, index_t size2, index_t size3,
    stride_t stride00, stride_t stride01,
    ...
    __shared__ arg0_t tile[shmSize][shmSize + 2];

    const int64_t n = blockIdx.z;
    const int64_t r = blockIdx.y;
    const int64_t c = blockIdx.x;

    int64_t offset_in  = n * stride31;
    int64_t offset_out = n * stride30;

    int64_t x = c * shmSize + threadIdx.x * vt;
    int64_t y = r * shmSize + threadIdx.y ;
    int64_t dim0 = y%size0;
    int64_t dim1 = y/size0;
    LoadT ld0 = *(reinterpret_cast<LoadT*>(data1 + (offset_in + dim0 * stride01 + dim1 * stride11 + x * stride21)));
    *(reinterpret_cast<LoadT*>(&tile[threadIdx.y][threadIdx.x*vt])) = ld0;
    __syncthreads();

    x = r * shmSize + threadIdx.x * vt;
    y = c * shmSize + threadIdx.y;
    LoadT st0;

    #pragma unroll
    for (int i = 0; i < vt; ++i) {
      st0.val[i] = tile[threadIdx.x * vt + i][threadIdx.y];
    }

    *reinterpret_cast<LoadT*>(data0 + (offset_out + y * stride20 + x * sizeof(res_t))) = st0;
}
...
template<int nt, int vt, typename res_t , typename arg0_t, typename func_t, typename index_t, typename stride_t,
        bool align, typename out_t>
C10_LAUNCH_BOUNDS_2(nt, 4)
__global__ void elementwise_kernel_1_1_cast_dilation(
    int64_t N,
    char* data0, char* data1,
    ScalarType st0, ScalarType st1,
    index_t size0,
    stride_t stride00, stride_t stride01, 
    func_t f, out_t val = 0) {
  using StoreT = at::native::memory::aligned_vector<out_t, 8>;
  using LoadT  = at::native::memory::aligned_vector<float, 4>;
  int64_t lid = (blockIdx.x * blockDim.x + threadIdx.x);
  if ((lid * 4) >= N) return;

  StoreT st;
  LoadT  lt = *(reinterpret_cast<LoadT*>(data1 + lid * 4 * sizeof(float)));
  ...
  if (align) {
    ...
    if(lid == (N-4)) {
      #pragma unroll
      for(int i=0; i<4; i++) {
        *(reinterpret_cast<out_t*>(data0 + (slid+i*2) * sizeof(out_t))) = lt.val[i];
      }
    } else {
      st = *(reinterpret_cast<StoreT*>(data0 + slid * sizeof(out_t)));
      #pragma unroll
      for(int i=0; i<4; i++) {
        st.val[i*2] = lt.val[i];
      }
      ...
      *(reinterpret_cast<StoreT*>(data0 + slid * sizeof(out_t))) = st;
    }
  } else {
    int64_t slid = lid * 8;
    if (lid == 0) {
      #pragma unroll
      for(int i=0; i<4; i++) {
        *(reinterpret_cast<out_t*>(data0 + (slid+i*2+1) * sizeof(out_t))) = lt.val[i];
      }
    } else {
      st = *(reinterpret_cast<StoreT*>(data0 + slid * sizeof(out_t)));
      #pragma unroll
      for(int i=0; i<4; i++) {
        st.val[i*2+1] = lt.val[i];
      }
      #pragma unroll
      for(int i=0; i<4; i++) {
        st.val[i*2] += val;
...
template<int nt, int vt, typename res_t , typename arg0_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)!=2 && sizeof(res_t)!=4, int>::type = 0>
static void launch_legacy_kernel_maca_1_1_broadcast(
    int64_t N,
    char* data0, char* data1,
    index_t size0,
    stride_t stride00, stride_t stride01, 
    const func_t& f) {assert(0);}
...
template<int nt, int vt, typename res_t , typename arg0_t, typename func_t, typename index_t, typename stride_t>
static void launch_legacy_kernel_maca_1_1_cast_dilation(
    int64_t N,
    char* data0, char* data1,
    ScalarType st0, ScalarType st1,
    index_t size0,
  ...
  if (st0 == ScalarType::Half && reinterpret_cast<uintptr_t>(data0) % 4 == 0) {
    elementwise_kernel_1_1_cast_dilation<nt, vt, res_t, arg0_t, func_t, index_t, stride_t, true, at::Half><<<grid, block, 0, stream>>>(
      N, data0, data1, st0, st1, size0, stride00, stride01, f);
  } else if (st0 == ScalarType::Half && reinterpret_cast<uintptr_t>(data0) % 2 == 0) {
    elementwise_kernel_1_1_cast_dilation<nt, vt, res_t, arg0_t, func_t, index_t, stride_t, false, at::Half><<<grid, block, 0, stream>>>(
      N, data0 -2, data1, st0, st1, size0, stride00, stride01, f);
  } else if (st0 == ScalarType::BFloat16 && reinterpret_cast<uintptr_t>(data0) % 4 == 0) {
    elementwise_kernel_1_1_cast_dilation<nt, vt, res_t, arg0_t, func_t, index_t, stride_t, true, at::BFloat16><<<grid, block, 0, stream>>>(
      N, data0, data1, st0, st1, size0, stride00, stride01, f);
  } else if (st0 == ScalarType::BFloat16 && reinterpret_cast<uintptr_t>(data0) % 2 == 0) {
    elementwise_kernel_1_1_cast_dilation<nt, vt, res_t, arg0_t, func_t, index_t, stride_t, false, at::BFloat16><<<grid, block, 0, stream>>>(
      N, data0-2, data1, st0, st1, size0, stride00, stride01, f);
  } else {
    assert(false);
  }
...
template<int nt, int vt, typename res_t , typename arg0_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)!=4, int>::type = 0>
...
template<int nt, typename res_t, typename arg0_t, typename TIndex, typename func_t, int v>
C10_LAUNCH_BOUNDS_2(nt, 4)
...
typename std::enable_if<sizeof(res_t)!=2, int>::type = 0>
...
static void launch_legacy_kernel_maca_3_1_copy_opt(
  ...
  if (size0 % kTileDimMacaT == 0){
    ...
    dim3 grid(dh * dw);
    if (v == 2) {
      elementwise_kernel_3_1_copy_64_opt<512, res_t, arg0_t, uint32_t, func_t, v><<<grid, block, 0, stream>>>(
        data0, data1,
        (uint32_t)size0, (uint32_t)size1, (uint32_t)size2,
        (uint32_t)stride00, (uint32_t)stride01,
        (uint32_t)stride10, (uint32_t)stride11,
      ...
      elementwise_kernel_3_1_copy_64_opt<1024, res_t, arg0_t, uint32_t, func_t, v><<<grid, block, 0, stream>>>(
        data0, data1,
        (uint32_t)size0, (uint32_t)size1, (uint32_t)size2,
        (uint32_t)stride00, (uint32_t)stride01,
        (uint32_t)stride10, (uint32_t)stride11,
...
template<int nt, int vt, typename res_t , typename arg0_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)!=2 && sizeof(res_t)!=4, int>::type = 0>
static void launch_legacy_kernel_maca_4_1_transpose02(
    int64_t N,
    char* data0, char* data1,
    index_t size0, index_t size1, index_t size2, index_t size3,
    stride_t stride00, stride_t stride01,
...
static void launch_legacy_kernel_maca_4_1_transpose02(
  ...
  if (sizeof(res_t)==4) {
    const uint32_t dh = (size0 * size1) / kTileDimMacaT_32;
    const uint32_t dw = size2 / kTileDimMacaT_32;
    auto stream = at::cuda::getCurrentCUDAStream();
    dim3 block(kBlockRowsMacaT, kTileDimMacaT_32);
    dim3 grid(dw , dh, size3);
    elementwise_kernel_4_1_transpose02<nt, res_t, arg0_t, func_t, index_t, stride_t, 4, kTileDimMacaT_32><<<grid, block, 0, stream>>>(
          N,
          data0, data1,
          ...
          dh, dw,
          f);
  } else if (sizeof(res_t)==2) {
    const uint32_t dh = (size0 * size1) / kTileDimMacaT;
    const uint32_t dw = size2 / kTileDimMacaT;
    auto stream = at::cuda::getCurrentCUDAStream();
    dim3 block(kBlockRowsMacaT, kTileDimMacaT);
    dim3 grid(dw , dh, size3);
    elementwise_kernel_4_1_transpose02<nt, res_t, arg0_t, func_t, index_t, stride_t, 8, kTileDimMacaT><<<grid, block, 0, stream>>>(
          N,
          data0, data1,
          size0, size1, size2, size3,
          stride00, stride01,
...
static void launch_legacy_kernel_maca_4_1_transpose12_copy(
  ...
  if (size0 % kTileDimMacaT == 0) {
    ...
    if (v == 2) {
      elementwise_kernel_4_1_transpose12_copy_64<512, res_t, arg0_t, uint32_t, func_t, v><<<grid, block, 0, stream>>>(
        data0, data1,
      ...
      elementwise_kernel_4_1_transpose12_copy_64<1024, res_t, arg0_t, uint32_t, func_t, v><<<grid, block, 0, stream>>>(
        data0, data1,
        (uint32_t)size0, (uint32_t)size1, (uint32_t)size2, (uint32_t)size3,
        (uint32_t)stride00, (uint32_t)stride01,
        (uint32_t)stride10, (uint32_t)stride11,
...
static void launch_legacy_kernel_maca_5_1_lowdim_contiguous(
  ...
  int vec = vt;
  if (sizeof(res_t)==2 && size0%8==0) {vec=8;}
  else if (size0%4==0) {vec=4;}

  dim3 block(nt);
  dim3 grid((N + block.x * vec - 1) / (block.x * vec));
  auto stream = at::cuda::getCurrentCUDAStream();
  if (vec==8) {
    elementwise_kernel_5_1_lowdim_contiguous<nt, 8, res_t, arg0_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
          N, data0, data1,
          size0, size1, size2, size3, size4,
          stride00, stride01,
          stride10, stride11,
    ...
    elementwise_kernel_5_1_lowdim_contiguous<nt, 4, res_t, arg0_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
          N, data0, data1,
          size0, size1, size2, size3, size4,
          stride00, stride01,
          stride10, stride11,
    ...
    elementwise_kernel_5_1_lowdim_contiguous<nt, 2, res_t, arg0_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
          N, data0, data1,
          size0, size1, size2, size3, size4,
          stride00, stride01,
          stride10, stride11,
...
void gpu_kernel_impl_maca_arity1_copy(TensorIteratorBase& iter, const func_t& f) {
  ...
  int arch = at::cuda::getMacaCurrentDeviceAbility();
  ...
  if (!dynamic_casting) {
    if (contiguous) {
      ...
      launch_vectorized_kernel(numel, f, data);
        ...
        if (ndim == 1) {
          ...
          bool disable_opt_1_1 = at::maca::get_maca_disable_elementwise_1_1();
          if ((sizeof(arg0_t) == 2 || sizeof(arg0_t) == 4) && maca_likely(!disable_opt_1_1_broadcast) &&
            ...
            launch_legacy_kernel_maca_1_1_broadcast<1024, unroll_factor, arg0_t, typename traits::template arg<0>::type>(
              numel,
              data[0], data[1],  // data
              ...
              f);
            isElemOpt  = true;
          } else if (maca_likely(!disable_opt_1_1)) {
          ...
          if ((sizeof(arg0_t) == 2 || sizeof(arg0_t) == 4) && maca_likely(!disable_elementwise_4_1_transpose12_copy) &&
            ...
            launch_legacy_kernel_maca_4_1_dim0_pad<64, 8, arg0_t, typename traits::template arg<0>::type>(
              ...
              f);
            isElemOpt = true;
          } else if ((sizeof(arg0_t) == 2 && sizeof(arg1_t) == 2 || sizeof(arg0_t) == 4 && sizeof(arg1_t) == 4) &&
                     offset_calc.strides_[0][0] == sizeof(arg0_t) && offset_calc.strides_[1][0] == s0 * sizeof(arg0_t) &&
                     offset_calc.strides_[2][0] == s01 * sizeof(arg0_t) && offset_calc.strides_[3][0] == s012 * sizeof(arg0_t) &&
                     offset_calc.strides_[0][1] == s1 * s2 * sizeof(arg0_t) && offset_calc.strides_[1][1] == s2 * sizeof(arg0_t) &&
                     offset_calc.strides_[2][1] == sizeof(arg0_t) && offset_calc.strides_[3][1] && offset_calc.strides_[3][0] &&
                     (reinterpret_cast<uintptr_t>(data[0]) % 4) == 0 && (reinterpret_cast<uintptr_t>(data[1]) % 4) == 0 &&
                     (s0 * s1) % 64 == 0 && s2 % 64 == 0 && !at::maca::get_maca_disable_elementwise_kernel_cp_4_1_transpose02()) {
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_cp_launch_legacy_kernel_maca_4_1_transpose02", f);
            launch_legacy_kernel_maca_4_1_transpose02<64, 8, arg0_t, typename traits::template arg<0>::type>(
              numel,
              data[0], data[1],  // data
              offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor, offset_calc.sizes_[2].divisor, offset_calc.sizes_[3].divisor,
              offset_calc.strides_[0][0], offset_calc.strides_[0][1],
    ...
    if (contiguous) {
      ...
      bool isElemOpt = false;
      if (ndim == 1) {
        if (numel >= 851968 && numel % 4 ==0 &&
            (dtypes[0] == ScalarType::Half || dtypes[0] == ScalarType::BFloat16) &&
            dtypes[1] == ScalarType::Float &&
            offset_calc.strides_[0][0] == 4 && offset_calc.strides_[0][1] == 4 &&
            (reinterpret_cast<uintptr_t>(data[0]) % 2) == 0 && (reinterpret_cast<uintptr_t>(data[1]) % 4) == 0 &&
            at::maca::get_maca_enable_elementwise_kernel_1_1_cast_dilation()) {
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_cp_launch_legacy_kernel_maca_1_1_cast_dilation", f);
            launch_legacy_kernel_maca_1_1_cast_dilation<128, 4, arg0_t, typename traits::template arg<0>::type>(
              numel,
              data[0], data[1],  // data
              dtypes[0], dtypes[1], 
              offset_calc.sizes_[0].divisor,

...
static inline void launch_unrolled_kernel_4_float_to_half(int64_t N, const func_t& f, char* data0, char* data1)
{
  TORCH_INTERNAL_ASSERT(N > 0 && N <= std::numeric_limits<int32_t>::max());
  int64_t grid = (N + block_work_size() - 1) / block_work_size();
  auto stream = at::cuda::getCurrentCUDAStream();
  unrolled_elementwise_kernel_4_float_to_half<func_t, size_t><<<grid, num_threads(), 0, stream>>>(f, data0, data1);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

template<typename func_t>
static inline void launch_unrolled_kernel_4_half_to_float(int64_t N, const func_t& f, char* data0, char* data1)
{
  TORCH_INTERNAL_ASSERT(N > 0 && N <= std::numeric_limits<int32_t>::max());
  int64_t grid = (N + block_work_size() - 1) / block_work_size();
  auto stream = at::cuda::getCurrentCUDAStream();
  unrolled_elementwise_kernel_4_half_to_float<func_t, size_t><<<grid, num_threads(), 0, stream>>>(f, data0, data1);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

template<typename func_t>
static inline void launch_unrolled_kernel_4_float_to_bfloat(int64_t N, const func_t& f, char* data0, char* data1)
{
  TORCH_INTERNAL_ASSERT(N > 0 && N <= std::numeric_limits<int32_t>::max());
  int64_t grid = (N + block_work_size() - 1) / block_work_size();
  auto stream = at::cuda::getCurrentCUDAStream();
  unrolled_elementwise_kernel_4_float_to_bfloat<func_t, size_t><<<grid, num_threads(), 0, stream>>>(f, data0, data1);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

template<typename func_t>
static inline void launch_unrolled_kernel_4_bfloat_to_float(int64_t N, const func_t& f, char* data0, char* data1)
{
  TORCH_INTERNAL_ASSERT(N > 0 && N <= std::numeric_limits<int32_t>::max());
  int64_t grid = (N + block_work_size() - 1) / block_work_size();
  auto stream = at::cuda::getCurrentCUDAStream();
  unrolled_elementwise_kernel_4_bfloat_to_float<func_t, size_t><<<grid, num_threads(), 0, stream>>>(f, data0, data1);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
  ...
  if (!dynamic_casting) {
    if (contiguous) {
      launch_vectorized_kernel(numel, f, data);
        ...
        int ndim = iter.ndim();
        get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel", f);
...
static void launch_legacy_kernel_maca_2_1_broadcast(
    ...
    const func_t& f) {
  // ndim = 2, arity = 1
  // std::cout << "launch_legacy_kernel_maca_2_1_broadcast" << "\n";
  // std::cout << "kernel info :" << typeid(f).name() << "\n";
  // std::cout << "N :" << N << "\n";
  // std::cout << "size0 :" << size0 << "\n";
  // std::cout << "size1 :" << size1 << "\n";
  // std::cout << "stride00:" << stride00 << "\n";
  // std::cout << "stride01:" << stride01 << "\n";
  // std::cout << "stride10:" << stride10 << "\n";
  // std::cout << "stride11:" << stride11 << "\n";
  TORCH_INTERNAL_ASSERT(N >= 0 && N <= std::numeric_limits<int32_t>::max());
...
//  printf("float_copy: %d, %d, %d, %d，%d\n", batch, dh, dw, H, W);
  elementwise_kernel_transpose_float_copy<res_t, arg0_t, uint32_t><<<batch * dh * dw, dim3(kTileDimMacaT, kBlockRowsMacaT), 0, stream>>>(
      out, in, (uint32_t)H, (uint32_t)W, (uint32_t)dh, (uint32_t)dw);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
      ...
      launch_vectorized_kernel(numel, f, data);
        ...
        if (ndim == 1) {
          ...
          if ((sizeof(arg0_t) == 2 || sizeof(arg0_t) == 4) && maca_likely(!disable_opt_1_1_broadcast) &&
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_1_1_broadcast", f);
            launch_legacy_kernel_maca_1_1_broadcast<8*C10_WARP_SIZE, unroll_factor, arg0_t, typename traits::template arg<0>::type>(
              ...
              get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_1_1_dilation", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_1_1", f);
          ...
          if (((dtypes[0] == ScalarType::Half && dtypes[1] == ScalarType::Half) ||
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1_transpose", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1_transpose_fp32", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1_broadcast", f);
            launch_legacy_kernel_maca_2_1_broadcast<64, unroll_factor, arg0_t, typename traits::template arg<0>::type>(
              ...
              offset_calc.strides_[0][0] == 2 && offset_calc.strides_[0][1] == 2 && offset_calc.sizes_[1].divisor == 245760 &&
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1_dim0_pad", f);
            ...
            auto stride10 = offset_calc.strides_[1][0];
            auto stride11 = offset_calc.strides_[1][1];
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1_dim0_contiguous", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1", f);
          ...
          if (((dtypes[0] == ScalarType::Half && dtypes[1] == ScalarType::Half) ||
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1_transpose_half_copy", f);
            launch_legacy_kernel_maca_3_1_transpose_half_copy<arg0_t, typename traits::template arg<0>::type>(
              ...
              f);
//          } else if (dtypes[0] == ScalarType::Short && dtypes[1] == ScalarType::Short && 
//              offset_calc.strides_[0][0] == 2 && offset_calc.strides_[1][0] == 2 * offset_calc.sizes_[0].divisor &&
//              offset_calc.strides_[2][0] == 2 * offset_calc.sizes_[0].divisor * offset_calc.sizes_[1].divisor &&
//              offset_calc.strides_[0][1] == 2 * offset_calc.sizes_[1].divisor && offset_calc.strides_[1][1] == 2 &&
//              offset_calc.strides_[2][1] == offset_calc.strides_[2][0]) {
//            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1_transpose_short_copy");
//            launch_legacy_kernel_maca_3_1_transpose_short_copy<arg0_t, typename traits::template arg<0>::type>(
//              numel,
//              data[0], data[1],  // data
//              offset_calc.sizes_[2].divisor, offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor,
//              f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1_transpose012_half_copy", f);
            launch_legacy_kernel_maca_3_1_transpose012_half_copy<arg0_t, typename traits::template arg<0>::type>(
              ...
              offset_calc.strides_[2][1] == offset_calc.strides_[2][0]) {
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1_transpose_float_copy", f);
            launch_legacy_kernel_maca_3_1_transpose_float_copy<arg0_t, typename traits::template arg<0>::type>(
              ...
              (reinterpret_cast<uintptr_t>(data[0]) % 4) == 0 && (reinterpret_cast<uintptr_t>(data[1]) % 4) == 0){
                get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1_transpose12_half_copy", f);
              ...
              (reinterpret_cast<uintptr_t>(data[0]) % 4) == 0 && (reinterpret_cast<uintptr_t>(data[1]) % 4) == 0 && s2 % 2 == 0) {
                ...
                get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1_copy_opt", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1_broadcast", f);
...
//            std::cout << "match2" << std::endl;
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_3_1", f);
          ...
          if ((sizeof(arg0_t) == 2 || sizeof(arg0_t) == 4) && maca_likely(!disable_elementwise_4_1_transpose12_copy) &&
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_4_1_transpose12_copy", f);
            ...
            launch_legacy_kernel_maca_4_1_transpose12_copy<sizeof(arg0_t), arg0_t, typename traits::template arg<0>::type>(
              ...
              f);
                      ...
                      (reinterpret_cast<uintptr_t>(data[0]) % 4) == 0 && (reinterpret_cast<uintptr_t>(data[1]) % 4) == 0){
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_4_1_input_lowdim_contiuous", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_4_1_dim0_pad", f);
            ...
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_4_1", f);
          ...
          if ((sizeof(arg0_t) == 2 || sizeof(arg0_t) == 4) && s0 % 8 == 0 && maca_likely(!disable_5_1_lowdim_contiguous) &&
               ...
               (reinterpret_cast<uintptr_t>(data[0]) % 4) == 0 && (reinterpret_cast<uintptr_t>(data[1]) % 4) == 0) {
              get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_5_1_lowdim_contiguous", f);
           ...
           get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_5_1", f);
    ...
    if (contiguous) {
      ...
      bool disable_flag = at::maca::get_maca_disable_unroll_float_opt();
      if (disable_flag) {
        launch_unrolled_kernel(numel, f, data, input_offset_calculator, output_offset_calculator, loader, storer);
      } else {
        if (numel != 0 && numel % block_work_size() == 0 && (uint64_t)data[0]%4 == 0 && (uint64_t)data[1]%4 == 0 &&
              dtypes[0] == ScalarType::Half && dtypes[1] == ScalarType::Float) {
          launch_unrolled_kernel_4_float_to_half<func_t>(numel, f, data[0], data[1]);
        } else if (numel != 0 && numel % block_work_size() == 0 && (uint64_t)data[0]%4 == 0 && (uint64_t)data[1]%4 == 0 &&
              dtypes[0] == ScalarType::Float && dtypes[1] == ScalarType::Half) {
          launch_unrolled_kernel_4_half_to_float<func_t>(numel, f, data[0], data[1]);
        } else if (numel != 0 && numel % block_work_size() == 0 && (uint64_t)data[0]%4 == 0 && (uint64_t)data[1]%4 == 0 &&
              dtypes[0] == ScalarType::BFloat16 && dtypes[1] == ScalarType::Float) {
          launch_unrolled_kernel_4_float_to_bfloat<func_t>(numel, f, data[0], data[1]);
        } else if (numel != 0 && numel % block_work_size() == 0 && (uint64_t)data[0]%4 == 0 && (uint64_t)data[1]%4 == 0 &&
              dtypes[0] == ScalarType::Float && dtypes[1] == ScalarType::BFloat16) {
          launch_unrolled_kernel_4_bfloat_to_float<func_t>(numel, f, data[0], data[1]);
        } else {
          launch_unrolled_kernel(numel, f, data, input_offset_calculator, output_offset_calculator, loader, storer);
        }
      }
      ...
      assert(narity == 1);
      if (ndim == 1) {
        get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_1_1_cast", f);
        ...
        if (!disable_elementwise_kernel_2_1_cp_cast_dim0_contiguous && numel > 0 && offset_calc.sizes_[0].divisor % 4 == 0 &&
          ...
          get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1_cast_dim0_contiguous", f);
          ...
          get_elementwise_info<narity + 1>(ndim, narity, offset_calc, "p_e_cp_launch_legacy_kernel_maca_2_1_cast", f);

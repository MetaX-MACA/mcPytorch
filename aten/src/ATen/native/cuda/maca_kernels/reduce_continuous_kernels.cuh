...
static bool is_launch_continuous_reduce_kernel(const TensorIterator& iter, const size_t &input_element_size, const size_t &output_element_size) {
  // Currently imcontiunous reduce kernel support 2 cases
...
template <
    ...
    typename ops_t>
__global__ void InputPerOutputContinuousReduceKernel(
...
#pragma unroll
  ...
  if (dim_x > C10_WARP_SIZE) {
    ...
    scalar_t* shared_val = reinterpret_cast<scalar_t*>(shared_memory);
    int64_t* shared_index = reinterpret_cast<int64_t*>(shared_memory + dim_x * blockDim.y * sizeof(scalar_t));
    ...
    for (size_t offset = dim_x >> 1; offset >= C10_WARP_SIZE; offset >>= 1) {
      ...
      if (threadIdx.x < offset) {
        ...
        scalar_t other_val = shared_val[shared_read_address];
  ...
  if (should_block_y_reduce) {
    extern __shared__ char shared_memory[];
    scalar_t* shared_val = reinterpret_cast<scalar_t*>(shared_memory);
    int64_t* shared_index = reinterpret_cast<int64_t*>(shared_memory + blockDim.y * sizeof(scalar_t));
    ...
    if (threadIdx.x == 0) {
      ...
      for (int offset = blockDim.y >> 1; offset >= 1 ; offset >>= 1) {
        ...
        if (threadIdx.y < offset) {
          scalar_t other_val = shared_val[threadIdx.y + offset];
  ...
  if (gridDim.y > 1) {
    scalar_t *cta_value = reinterpret_cast<scalar_t *>(cta_buf);
    ...
    if (is_last_block_done && is_first_thread_in_block) {
      ...
      for (size_t i = 0; i < gridDim.y; i++) {
        scalar_t other_cta_value = cta_value[reduce_buffer_offset+i];
...
template<typename scalar_t, typename out_scalar_t, int max_threads, typename arg_t, typename R>
static void launch_continuous_reduce_kernel(const ReduceConfigMaca& config, const R& reduction) {
  dim3 block = config.block();
  dim3 grid = config.grid();
  int shared_memory = config.shared_memory_size();
  auto stream = at::cuda::getCurrentCUDAStream();
  ...
  {
  case 8:
    InputPerOutputContinuousReduceKernel<scalar_t, out_scalar_t, arg_t, 8, can_accumulate_in_output><<<grid, block, shared_memory, stream>>>(
      (const scalar_t *)reduction.src,
      reduction.ops,
      reduction.ident,
      config.num_inputs,
  ...
  case 4:
    InputPerOutputContinuousReduceKernel<scalar_t, out_scalar_t, arg_t, 4, can_accumulate_in_output><<<grid, block, shared_memory, stream>>>(
      (const scalar_t *)reduction.src,
      reduction.ops,
      reduction.ident,
      config.num_inputs,
  ...
  case 2:
    InputPerOutputContinuousReduceKernel<scalar_t, out_scalar_t, arg_t, 2, can_accumulate_in_output><<<grid, block, shared_memory, stream>>>(
      (const scalar_t *)reduction.src,
      reduction.ops,
      reduction.ident,
      config.num_inputs,
  ...
  default:
    InputPerOutputContinuousReduceKernel<scalar_t, out_scalar_t, arg_t, 1, can_accumulate_in_output><<<grid, block, shared_memory, stream>>>(
      (const scalar_t *)reduction.src,
      reduction.ops,
      reduction.ident,
      config.num_inputs,

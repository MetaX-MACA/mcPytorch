...
bool launch_group_norm_forward_kernel(
    ...
    if (DxHxW == 40960 && HxW == 4096 && fuse_result) {
        ...
        switch(vec_size) {
            case 8:
            VectorizedRowwiseMomentsCUDAKernel<T, 8><<<grid_size, block_size, 0, cuda_stream>>>(D * HxW, eps, X_data, mean_data, rstd_data);
            break;
            case 4:
            VectorizedRowwiseMomentsCUDAKernel<T, 4><<<grid_size, block_size, 0, cuda_stream>>>(D * HxW, eps, X_data, mean_data, rstd_data);
            break;
            case 2:
            VectorizedRowwiseMomentsCUDAKernel<T, 2><<<grid_size, block_size, 0, cuda_stream>>>(D * HxW, eps, X_data, mean_data, rstd_data);
            break;
            default:
            VectorizedRowwiseMomentsCUDAKernel<T, 1><<<grid_size, block_size, 0, cuda_stream>>>(D * HxW, eps, X_data, mean_data, rstd_data);
            break;
        } 
        AT_CUDA_CHECK(cudaGetLastError());
        return false;

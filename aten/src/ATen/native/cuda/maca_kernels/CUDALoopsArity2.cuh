...
static void launch_legacy_kernel_maca_2_2_template(
  ...
  #define SWITCH_TEMPLATE_KERNEL_ARGS_3f(nt, vt, vt_0, vt_1, vec, f, ...)  \
    if (vt == 4 && vt_0 == 4 && vt_1 == 4) {                           \
      ...
      dim3 grid((N + block.x * 1 - 1) / (block.x * 1));                \
      f(nt, 1, 1, 1, 1, __VA_ARGS__);                                  \
    }
...
#define ELEMENTWISE_KERNEL_TEMPLATE(nt, vt, vt_0, vt_1, vec, res_t, arg0_t, arg1_t, func_t, index_t, stride_t) \
  ...
  SWITCH_TEMPLATE_KERNEL_ARGS_3f(nt, vec_data_0, vec_data_1, vec_data_2, vec, ELEMENTWISE_KERNEL_TEMPLATE,
...
static void launch_legacy_kernel_maca_2_2_cast_template(
    ...
    stride_t stride10, stride_t stride11, stride_t stride12,
    const func_t& f) {
  #define SWITCH_TEMPLATE_KERNEL_ARGS_3_p0(nt, vt, vt_0, vt_1, vec, f, ...)  \
  if (vt == 2 && vt_0 == 2 && vt_1 == 2) {                           \
    dim3 grid((N + block.x * 2 - 1) / (block.x * 2));                \
    f(nt, 2, 2, 2, 2, __VA_ARGS__);                                  \
  } else if (vt == 4 && vt_0 == 4 && vt_1 == 4) {                    \
    dim3 grid((N + block.x * 4 - 1) / (block.x * 4));                \
  ...
  #define SWITCH_TEMPLATE_KERNEL_ARGS_3_p1(nt, vt, vt_0, vt_1, vec, f, ...)  \
  if (vt == 2 && vt_0 == 2 && vt_1 == 2) {                           \
    dim3 grid((N + block.x * 2 - 1) / (block.x * 2));                \
    f(nt, 2, 2, 2, 2, __VA_ARGS__);                                  \
  } else if (vt == 4 && vt_0 == 4 && vt_1 == 4) {                    \
...
#define ELEMENTWISE_KERNEL_TEMPLATE(nt, vt, vt_0, vt_1, vec, res_t, arg0_t, arg1_t, func_t, index_t, stride_t, res_dtype_t, arg0_dtype_t, arg1_dtype_t) \
  ...
  if (st0 == ScalarType::Float && st1 == ScalarType::Char && st2 == ScalarType::Float) {
    SWITCH_TEMPLATE_KERNEL_ARGS_3_p0(nt, vec_data_0, vec_data_1, vec_data_2, vec, ELEMENTWISE_KERNEL_TEMPLATE,
    ...
    SWITCH_TEMPLATE_KERNEL_ARGS_3_p1(nt, vec_data_0, vec_data_1, vec_data_2, vec, ELEMENTWISE_KERNEL_TEMPLATE,
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
int NT, bool signed_strides = false>
static bool launch_legacy_kernel_maca_3_2_opt_tile(
    int64_t N,
    char* data0, char* data1, char* data2,
    index_t size0, index_t size1, index_t size2,
    stride_t stride00, stride_t stride01, stride_t stride02,
    ...
    stride_t stride20, stride_t stride21, stride_t stride22,
    const func_t& f, 
    OffsetCalculator<NT, uint32_t, signed_strides> offset_calc,
    at::detail::Array<char*, NT> data) {
  // ndim = 3, arity = 2
  TORCH_INTERNAL_ASSERT(N >= 0 && N <= std::numeric_limits<int32_t>::max());
  if (N == 0) {
    return true;
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t>
C10_LAUNCH_BOUNDS_2(nt, 4)
__global__ void elementwise_kernel_4_2_broadcast_arg0_dim1(
    int64_t N,
    char* data0, char* data1, char* data2,
    index_t size0, index_t size1, index_t size2, index_t size3,
    stride_t stride00, stride_t stride01, stride_t stride02,
  ...
  int64_t index0 = (blockIdx.x * blockDim.x + threadIdx.x) * vt;
  int64_t index2 = blockIdx.z % size2;
  int64_t index3 = blockIdx.z / size2;

  int64_t offset0 = index0 * stride00 + index2 * stride20 + index3 * stride30;
  int64_t offset1 = index0 * stride01 + index2 * stride21 + index3 * stride31;
  ...
  // int64_t step = blockDim.y * vec1;
  for(int64_t id = blockIdx.y; id < size1; id += gridDim.y) {
    int64_t res_idx = offset0 + id * stride10;
    arg1T ld1 = *(reinterpret_cast<arg1T*>(data2 + res_idx));
    #pragma unroll
    for (int i = 0; i < vt; i++) {
      st.val[i] = f(ld0.val[i], ld1.val[i]);
    }
    *(reinterpret_cast<StoreT*>(data0 + res_idx)) = st;
  }
}
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t>
C10_LAUNCH_BOUNDS_2(nt, 4)
__global__ void elementwise_kernel_4_2_broadcast_arg1_dim3(
    int64_t N,
    char* data0, char* data1, char* data2,
    index_t size0, index_t size1, index_t size2, index_t size3,
    stride_t stride00, stride_t stride01, stride_t stride02,
  ...
  int64_t index012 = (blockIdx.x * blockDim.x + threadIdx.x) * vt;
  int64_t index12  = index012 / size0;
  int64_t index0   = index012 - (index12 * size0);
  int64_t index2   = index12  / size1;
  int64_t index1   = index12 - (index2 * size1);

  int64_t offset0 = index012 * sizeof(res_t);
  int64_t offset1 = offset0;
  int64_t offset2 = index0 * stride02 + index1 * stride12 + index2 * stride22;

  #pragma unroll
  for (int i = 0; i < vt; i++) {
    ld1.val[i] = *(reinterpret_cast<arg1_t*>(data2 + offset2 + i * stride02));
  }

  for (int64_t id = blockIdx.y; id < size3; id += gridDim.y) {
    int64_t res_idx = offset0 + id * stride30;
    arg0T ld0 = *(reinterpret_cast<arg0T*>(data1 + res_idx));
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)!=2 && sizeof(res_t)!=4, int>::type = 0>
static void launch_legacy_kernel_maca_4_2_broadcast_arg0_dim1(
    int64_t N,
    char* data0, char* data1, char* data2,
    index_t size0, index_t size1, index_t size2, index_t size3,
    stride_t stride00, stride_t stride01, stride_t stride02,
...
static void launch_legacy_kernel_maca_4_2_broadcast_arg0_dim1(
  ...
  constexpr int64_t block_dim_x = 64;
  int64_t vec0;
  if (size0 % 256 == 0) {vec0 = 4;}
  else if (size0 % 128 == 0) {vec0 = 2;}
  else {vec0 = 1;}
  int64_t vec1 = 32;

  auto stream = at::cuda::getCurrentCUDAStream();
  dim3 block(block_dim_x);
  dim3 grid(size0/(vec0*block_dim_x), size1/vec1, size2*size3);

  if (vec0 == 2) {
    elementwise_kernel_4_2_broadcast_arg0_dim1<nt, 2, res_t, arg0_t, arg1_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
    N, data0, data1, data2,
    size0, size1, size2, size3,
    stride00, stride01, stride02,
    stride10, stride11, stride12,
    ...
    elementwise_kernel_4_2_broadcast_arg0_dim1<nt, 4, res_t, arg0_t, arg1_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
    N, data0, data1, data2,
    size0, size1, size2, size3,
    stride00, stride01, stride02,
    stride10, stride11, stride12,
    ...
    elementwise_kernel_4_2_broadcast_arg0_dim1<nt, 1, res_t, arg0_t, arg1_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
    N, data0, data1, data2,
    size0, size1, size2, size3,
    stride00, stride01, stride02,
    stride10, stride11, stride12,
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)!=2 && sizeof(res_t)!=4, int>::type = 0>
static void launch_legacy_kernel_maca_4_2_broadcast_arg1_dim3(
    int64_t N,
    char* data0, char* data1, char* data2,
    index_t size0, index_t size1, index_t size2, index_t size3,
    stride_t stride00, stride_t stride01, stride_t stride02,
...
static void launch_legacy_kernel_maca_4_2_broadcast_arg1_dim3(
  ...
  constexpr int64_t block_dim_x = 64;
  int64_t vec0;
  if ((size0 * size1 * size2) % 256 == 0) {vec0 = 4;}
  else {vec0 = 2;}
  int64_t vec1 = 32;
  int64_t gridy = (size3+16-1)/16;

  auto stream = at::cuda::getCurrentCUDAStream();
  dim3 block(block_dim_x);
  dim3 grid((size0*size1*size2)/(vec0*block_dim_x), gridy);

  if (vec0 == 2) {
    elementwise_kernel_4_2_broadcast_arg1_dim3<nt, 2, res_t, arg0_t, arg1_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
    N, data0, data1, data2,
    size0, size1, size2, size3,
    stride00, stride01, stride02,
    stride10, stride11, stride12,
    ...
    elementwise_kernel_4_2_broadcast_arg1_dim3<nt, 4, res_t, arg0_t, arg1_t, func_t, index_t, stride_t><<<grid, block, 0, stream>>>(
    N, data0, data1, data2,
    size0, size1, size2, size3,
    stride00, stride01, stride02,
    stride10, stride11, stride12,
...
static void launch_legacy_kernel_maca_4_2_opt(
  ...
  if (v == 2) {
    elementwise_kernel_4_2_opt_64<512, vt, res_t, arg0_t, arg1_t, func_t, index_t, stride_t, sizeof(arg0_t), 16 / sizeof(arg0_t)><<<grid, block, 0, stream>>>(
      N, data0, data1, data2,
      size0, size1, size2, size3,
      stride00, stride01, stride02,
      stride10, stride11, stride12,
    ...
    elementwise_kernel_4_2_opt_64<1024, vt, res_t, arg0_t, arg1_t, func_t, index_t, stride_t, sizeof(arg0_t), 16 / sizeof(arg0_t)><<<grid, block, 0, stream>>>(
      N, data0, data1, data2,
      size0, size1, size2, size3,
      stride00, stride01, stride02,
      stride10, stride11, stride12,
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)==4 && sizeof(arg0_t)==4 && sizeof(arg1_t)==4, int>::type = 0>
static void launch_legacy_kernel_maca_4_2_template(
    int64_t N,
    char* data0, char* data1, char* data2,
    index_t size0, index_t size1, index_t size2, index_t size3,
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)==2 || 
                        sizeof(res_t)==4 && sizeof(arg0_t)!=4 && sizeof(arg1_t)==4 ||
                        sizeof(res_t)==4 && sizeof(arg0_t)==4 && sizeof(arg1_t)!=4 ||
                        sizeof(res_t)==4 && sizeof(arg0_t)!=4 && sizeof(arg1_t)!=4, int>::type = 0>
static void launch_legacy_kernel_maca_4_2_template(
    int64_t N,
    char* data0, char* data1, char* data2,
    index_t size0, index_t size1, index_t size2, index_t size3,
...
template<int nt, int v_x, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
        typename res_dtype_t=float, typename arg0_dtype_t=float, typename arg1_dtype_t=float>
C10_LAUNCH_BOUNDS_2(nt, 4)
__global__ void elementwise_kernel_3_2_cast_broadcast_arity2_dim1(
    int64_t N,
    char* data0, char* data1, char* data2,
    ScalarType st0, ScalarType st1, ScalarType st2,
    index_t size0, index_t size1, index_t size2,
  ...
  int64_t index02 = (blockIdx.x * blockDim.x + threadIdx.x) * v_x;
  int64_t index2  = index02 / size0;
  int64_t index0  = index02 - (index2 * size0);

  int64_t offset0 = index0 * stride00 + index2 * stride20;
  int64_t offset1 = index0 * stride01 + index2 * stride21;
  int64_t offset2 = index0 * stride02 + index2 * stride22;

  #pragma unroll
  for (int i = 0; i < v_x; i++) {
    ld2.val[i] = *(reinterpret_cast<arg1_dtype_t*>(data2 + offset2 + i * stride02));
  }

  for (int64_t id = blockIdx.y; id < size1; id += gridDim.y) {
    int64_t res_idx = offset0 + id * stride10;
    int64_t ld1_idx = offset1 + id * stride11;
    #pragma unroll
    for (int i = 0; i < v_x; i++) {
      ld1.val[i] = *(reinterpret_cast<arg0_dtype_t*>(data1 + ld1_idx + i * stride01));
      st.val[i]  = c10::convert<res_dtype_t>(f(c10::convert<arg0_t>(ld1.val[i]), c10::convert<arg1_t>(ld2.val[i])));
    }
    *(reinterpret_cast<StoreT*>(data0 + res_idx)) = st;
  }
}
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
typename std::enable_if<sizeof(res_t)!=4, int>::type = 0>
static void launch_legacy_kernel_maca_3_2_cast_broadcast_arity2_dim1(
    int64_t N,
    char* data0, char* data1, char* data2,
    ScalarType st0, ScalarType st1, ScalarType st2,
    index_t size0, index_t size1, index_t size2,
...
static void launch_legacy_kernel_maca_3_2_cast_broadcast_arity2_dim1(
  ...
  auto stream = at::cuda::getCurrentCUDAStream();
  dim3 block(64);
  int64_t gridx = (size0 * size2) / (block.x * 4);
  int64_t gridy = (size1+16-1)/16;
  dim3 grid(gridx, gridy);

  if (st1 == ScalarType::Half) {
    elementwise_kernel_3_2_cast_broadcast_arity2_dim1<nt, 4, res_t, arg0_t, arg1_t, func_t, index_t, stride_t, float, at::Half, float><<<grid, block, 0, stream>>>(
  ...
  } else if (st1 == ScalarType::BFloat16) {
    elementwise_kernel_3_2_cast_broadcast_arity2_dim1<nt, 4, res_t, arg0_t, arg1_t, func_t, index_t, stride_t, float, at::BFloat16, float><<<grid, block, 0, stream>>>(
        N, data0, data1, data2, st0, st1, st2, size0, size1, size2, stride00, stride01, stride02, stride10, stride11, stride12, stride20, stride21, stride22, f);
  } else {
    TORCH_CHECK(false, "elementwise_3_2_cast_broadcast_arity2_dim1 not supported!");
  }
...
template<int nt, int vt, typename res_t , typename arg0_t, typename arg1_t, typename func_t, typename index_t, typename stride_t,
int NT, bool signed_strides = false>
static bool launch_legacy_kernel_maca_3_2_cast_opt_tile(
    int64_t N,
    char* data0, char* data1, char* data2,
    ScalarType st0, ScalarType st1, ScalarType st2,
    index_t size0, index_t size1, index_t size2,
    ...
    OffsetCalculator<NT, uint32_t, signed_strides> offset_calc,
    at::detail::Array<char*, NT> data,
    at::detail::Array<at::ScalarType, NT> dtypes
) {
  // ndim = 3, arity = 2
  TORCH_INTERNAL_ASSERT(N >= 0 && N <= std::numeric_limits<int32_t>::max());
  // check type out of kernel to prevent call the assert func in device to reduce private memory
  ...
  if (!dynamic_casting) {
    if (contiguous) {
      ...
      launch_vectorized_kernel(numel, f, data);
        ...
        if (ndim == 1) {
          ...
          bool enable_align = at::maca::get_maca_enable_elementwise_kernel_2_2_align();
          ...
          if(((sizeof(arg0_t) == 2 && sizeof(arg1_t) == 2 && sizeof(arg2_t) == 2) || (sizeof(arg0_t) == 4 && sizeof(arg1_t) == 4 && sizeof(arg2_t) == 4)) &&
              ...
              offset_calc.sizes_[0].divisor % 8 == 0 && maca_unlikely(enable_align)){
            ...
            launch_legacy_kernel_maca_2_2_align<8*C10_WARP_SIZE, unroll_factor, arg0_t, typename traits::template arg<0>::type, typename traits::template arg<1>::type>(
              ...
              f);
            isElemOpt = true;
          } else if (((sizeof(arg0_t) == 2 && sizeof(arg1_t) == 2 && sizeof(arg2_t) == 2 && offset_calc.sizes_[0].divisor % 4 == 0) ||
          ...
          if (((sizeof(arg0_t) == 2 && sizeof(arg1_t) == 2 && sizeof(arg2_t) == 2) ||
            ...
            if (((sizeof(arg0_t) == 2 && sizeof(arg1_t) == 2 && sizeof(arg2_t) == 2) || (sizeof(arg0_t) == 4 && sizeof(arg1_t) == 4 && sizeof(arg2_t) == 4)) &&
              ...
              launch_legacy_kernel_maca_3_2_opt_tile<128, unroll_factor, arg0_t, typename traits::template arg<0>::type, typename traits::template arg<1>::type>(
                numel,
                data[0], data[1], data[2], // data
                offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor, offset_calc.sizes_[2].divisor,
                offset_calc.strides_[0][0], offset_calc.strides_[0][1], offset_calc.strides_[0][2],
              ...
              get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_2_opt_tile", f);
              isElemOpt = true;
            } else {
          ...
          (reinterpret_cast<uintptr_t>(data[2]) % 4) == 0) {
            ...
            isElemOpt = true;
                      ...
                      (sizeof(arg0_t) == 4 && sizeof(arg1_t) == 4 && sizeof(arg2_t) == 4)) &&
                      st00 == sizeof(arg0_t) && st10 == s0 * sizeof(arg0_t) && st20 == s0 * s1 * sizeof(arg0_t) && st30 == s0 * s1 * s2 * sizeof(arg0_t) &&
                      st02 == st00 && st12 == st10 && st22 == st20 && st32 == st30 &&
                      st11 == 0 && s0 % 64 == 0 && s1 >= 32 &&
                      !at::maca::get_maca_disable_elementwise_kernel_4_2_arg0_dim1_broadcast()) {
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_4_2_broadcast_arg0_dim1", f);
            launch_legacy_kernel_maca_4_2_broadcast_arg0_dim1<128, unroll_factor, arg0_t, typename traits::template arg<0>::type, typename traits::template arg<1>::type>(
                numel,
                data[0], data[1], data[2], // data
                offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor, offset_calc.sizes_[2].divisor, offset_calc.sizes_[3].divisor,
                offset_calc.strides_[0][0], offset_calc.strides_[0][1], offset_calc.strides_[0][2],
            ...
            isElemOpt = true;
                      ...
                      (sizeof(arg0_t) == 4 && sizeof(arg1_t) == 4 && sizeof(arg2_t) == 4)) &&
                      st00 == sizeof(arg0_t) && st10 == s0 * sizeof(arg0_t) && st20 == s0 * s1 * sizeof(arg0_t) && st30 == s0 * s1 * s2 * sizeof(arg0_t) &&
                      st01 == st00 && st11 == st10 && st21 == st20 && st31 == st30 &&
                      st32 == 0 &&
                      (s0 % 4) == 0 && (s0 * s1 * s2) % 128 == 0 &&
                      (reinterpret_cast<uintptr_t>(data[0]) % 4) == 0 &&
                      (reinterpret_cast<uintptr_t>(data[1]) % 4) == 0 &&
                      (reinterpret_cast<uintptr_t>(data[2]) % 4) == 0 &&
                      !at::maca::get_maca_disable_elementwise_kernel_4_2_arg1_dim3_broadcast()) {
            get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_4_2_broadcast_arg1_dim3", f);
            launch_legacy_kernel_maca_4_2_broadcast_arg1_dim3<128, unroll_factor, arg0_t, typename traits::template arg<0>::type, typename traits::template arg<1>::type>(
                numel,
                data[0], data[1], data[2], // data
                offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor, offset_calc.sizes_[2].divisor, offset_calc.sizes_[3].divisor,
                offset_calc.strides_[0][0], offset_calc.strides_[0][1], offset_calc.strides_[0][2],
    ...
    if (contiguous) {
      ...
      if (ndim == 1) {
        ...
        if (((dtypes[0] == ScalarType::Float && dtypes[1] == ScalarType::Half && dtypes[2] == ScalarType::Float && s0 % 4 == 0 &&
            ...
            (dtypes[0] == ScalarType::Float && dtypes[1] == ScalarType::Half && dtypes[2] == ScalarType::Float && s0 % 4 == 0 &&
            offset_calc.strides_[0][0] == 4 && offset_calc.strides_[1][0] == s0 * 4 &&
            offset_calc.strides_[0][1] == 2 && offset_calc.strides_[1][1] == s0 * 2 &&
            offset_calc.strides_[0][2] == 0 && offset_calc.strides_[1][2] == 4) ||
            (dtypes[0] == ScalarType::Float && dtypes[1] == ScalarType::Float && dtypes[2] == ScalarType::Bool && s0 % 8 == 0 &&
            offset_calc.strides_[0][0] == 4 && offset_calc.strides_[1][0] == s0 * 4 &&
            offset_calc.strides_[0][1] == 4 && offset_calc.strides_[1][1] == s0 * 4 &&
            offset_calc.strides_[0][2] == 1 && offset_calc.strides_[1][2] == 0)) &&
            s0 >= C10_WARP_SIZE && maca_likely(!disable_opt)
            && sizeof(arg0_t)==4){ // float, float16, bfloat16
        ...
        if (dtypes[0] == ScalarType::Float && dtypes[1] == ScalarType::Half && dtypes[2] == ScalarType::Float &&
          ...
          isElemOpt = true;
                    ...
                    (dtypes[0] == ScalarType::Float && dtypes[1] == ScalarType::BFloat16 && dtypes[2] == ScalarType::Float)) &&
                   offset_calc.strides_[0][0] == 4 && offset_calc.strides_[1][2] == 0 &&
                   offset_calc.sizes_[0].divisor % C10_WARP_SIZE == 0 &&
                   (offset_calc.sizes_[0].divisor * offset_calc.sizes_[2].divisor) % 256 == 0 &&
                   !at::maca::get_maca_disable_elementwise_kernel_3_2_cast_broadcast_arity2_dim1() &&
                   sizeof(arg0_t)==4) {
          get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_2_cast_broadcast_arity2_dim1", f);
          launch_legacy_kernel_maca_3_2_cast_broadcast_arity2_dim1<128, 4, arg0_t, typename traits::template arg<0>::type, typename traits::template arg<1>::type>(
            numel,
            data[0], data[1], data[2], // data
            dtypes[0], dtypes[1], dtypes[2],
            offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor, offset_calc.sizes_[2].divisor,
          ...
          launch_legacy_kernel_maca_3_2_cast_opt_tile<128, 4, arg0_t, typename traits::template arg<0>::type, typename traits::template arg<1>::type>(
              numel,
              data[0], data[1], data[2], // data
              dtypes[0], dtypes[1], dtypes[2],
              offset_calc.sizes_[0].divisor, offset_calc.sizes_[1].divisor, offset_calc.sizes_[2].divisor,
              ...
              offset_calc.strides_[1][0], offset_calc.strides_[1][1], offset_calc.strides_[1][2],
              offset_calc.strides_[2][0], offset_calc.strides_[2][1], offset_calc.strides_[2][2],
              f, offset_calc, data, dtypes)
        ) {
          get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_2_cast_opt_tile", f);
          isElemOpt = true;
        } else {

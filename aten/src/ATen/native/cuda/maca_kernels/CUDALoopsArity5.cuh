...
__global__ void elementwise_kernel_2_5_cast(
  ...
  for (int i = 0; i < vt; i++) {
    if (idx < N) {
      ...
      res_t result = f(c10::fetch_and_cast_maca<arg0_t>(st1, data1 + offsets[1]),
                       c10::fetch_and_cast_maca<arg1_t>(st2, data2 + offsets[2]),
                       c10::fetch_and_cast_maca<arg2_t>(st3, data3 + offsets[3]),
                       c10::fetch_and_cast_maca<arg3_t>(st4, data4 + offsets[4]),
                       c10::fetch_and_cast_maca<arg4_t>(st5, data5 + offsets[5])
                      );
      c10::cast_and_store_maca<res_t>(st0, out, result);
      idx += nt;
    }
  }
}
...
void gpu_kernel_impl_maca_arity5(TensorIteratorBase& iter, const func_t& f) {
  ...
  if (!dynamic_casting) {
    if (contiguous) {
      get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_vec_1_5", f);
      launch_vectorized_kernel(numel, f, data);
        ...
        if (ndim == 3) {
          ...
          if ((sizeof(arg0_t)==4 && sizeof(arg1_t)==4 && sizeof(arg2_t)==4 && sizeof(arg3_t)==4 && sizeof(arg4_t)==4 && sizeof(arg5_t)==4) &&
              ...
              get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_5_broadcast", f);
              ...
              get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_3_5", f);
        ...
        if (!isElemOpt) {
          get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_5", f);
    ...
    if (contiguous) {
      ...
      get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_unroll_1_5", f);
      launch_unrolled_kernel(numel, f, data, input_offset_calculator, output_offset_calculator, loader, storer);
    } else {
      if(ndim == 2){
        if ((dtypes[0] == ScalarType::Half || dtypes[0] == ScalarType::BFloat16 || dtypes[0] == ScalarType::Float) &&
            (dtypes[1] == ScalarType::Half || dtypes[1] == ScalarType::BFloat16 || dtypes[1] == ScalarType::Float) &&
            (dtypes[2] == ScalarType::Half || dtypes[2] == ScalarType::BFloat16 || dtypes[2] == ScalarType::Float) &&
            (dtypes[3] == ScalarType::Half || dtypes[3] == ScalarType::BFloat16 || dtypes[3] == ScalarType::Float) &&
            (dtypes[4] == ScalarType::Half || dtypes[4] == ScalarType::BFloat16 || dtypes[4] == ScalarType::Float) &&
            (dtypes[5] == ScalarType::Half || dtypes[5] == ScalarType::BFloat16 || dtypes[5] == ScalarType::Float)) {
          get_elementwise_info<narity + 1>(ndim, narity, offset_calc, dtypes, "p_e_launch_legacy_kernel_maca_2_5_cast",f);

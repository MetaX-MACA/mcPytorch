...
#include <ATen/native/cuda/maca_kernels/TensorTopK.cuh>
...
void launch(
    ...
    auto disable_opt = at::maca::get_maca_disable_gatherTopK_opt();
    if (largest && !disable_opt) {
      gatherTopK_opt<T, IndexType, Dim /* WithKthValues= false*/><<<grid, block, 0, c10::cuda::getCurrentCUDAStream()>>>(
        input,
        inputSliceSize,
        outputSliceSize,
        numInputSlices,

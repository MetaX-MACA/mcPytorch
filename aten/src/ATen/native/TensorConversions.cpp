...
Tensor _to_copy(
  ...
  auto default_nchw = at::maca::get_maca_default_nchw_memory_format(); 
  auto default_ndhwc = at::maca::get_maca_default_ndhwc_memory_format(); 
  auto default_nlc = at::maca::get_maca_default_nlc_memory_format(); 
  bool channels_last_1d = false;
  if (self.is_contiguous() && self.is_cpu() && options.device().is_cuda() 
        && (options.layout() == c10::kStrided) && self.dim() >= 3 && self.size(1) > 1) {
    if (!default_nchw && self.dim() == 4) {
      memory_format = MemoryFormat::ChannelsLast;
    } else if (default_ndhwc && self.dim() == 5) {
      memory_format = MemoryFormat::ChannelsLast3d;
    } else if (default_nlc && self.dim() == 3) {
      channels_last_1d = true;
    }
  } 
    ...
    if (options.device().supports_as_strided()) {
      if (self.is_non_overlapping_and_dense()) {
        ...
        if (self.is_quantized()) {
          ...
          if (channels_last_1d) {
            int64_t self_1 = self.size(1);
            int64_t self_2 = self.size(2);
            r = at::empty_strided(
                self.sizes(),
                self.strides(),
                options.pinned_memory(pin_out));
            r.as_strided_(self.sizes(), {self_1 * self_2, 1, self_1});

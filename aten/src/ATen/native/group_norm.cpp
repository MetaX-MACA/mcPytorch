...
std::tuple<Tensor, Tensor, Tensor> native_group_norm(
  ...
  if (X.device().is_cpu()) {
    ...
    const Tensor& X_ = X.contiguous();
    TORCH_CHECK(X_.is_contiguous(memory_format));

    bool mixed_type = is_mixed_type(X_, gamma, beta);
    if (mixed_type) {
      check_mixed_data_type(X_, gamma, beta);
    }
    ...
    GroupNormKernel(
        X_.device().type(), X_, gamma, beta, N, C, HxW, group, eps, Y, mean, rstd);
    return std::make_tuple(Y, mean, rstd);
  }
}

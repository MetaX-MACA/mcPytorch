...
void topk_impl_loop(
  ...
  for (const auto i : c10::irange(n)) {
    ...
    bool enable_cpu_topk_stable_sort = at::maca::get_maca_enable_cpu_topk_stable_sort();
    if (enable_cpu_topk_stable_sort) {
      if (use_partial_sort) {
        if (largest) {
        ...
        if (largest) {
          ...
          if (sorted) {
            std::stable_sort(queue.begin(), queue.begin() + k - 1,
              [](const elem_t& x, const elem_t& y) -> bool {
                return ((_isnan<accscalar_t>(x.first) && !_isnan<accscalar_t>(y.first)) || (x.first > y.first));
              });
          }

  ...
  void try_all(std::function<void(const perf_t& perf)> f) {
...
#ifdef USE_MACA
        TORCH_CHECK(false, "Running the CUDNN operator while hitting the cache encountered an OutOfMemoryError");
#endif
      }
    }
    ...
    for (auto& algoPerf : perfResults) {
      try {
        ...
        std::ignore = cudaGetLastError(); // clear CUDA error
#ifdef USE_MACA
        TORCH_CHECK(false, "Running the CUDNN operator while no hit cache encountered an OutOfMemoryError");
#endif
      } catch (c10::CuDNNError&) {
        std::ignore = cudaGetLastError(); // clear CUDA error
#ifdef USE_MACA
        TORCH_CHECK(false, "Running the CUDNN operator while no hit cache encountered an CuDNNError");
#endif
      }
    }
#ifdef USE_MACA
    TORCH_CHECK(
        false, "Unable to find a valid cuDNN algorithm to run convolution or running cudnn encountered an error");
#else
    TORCH_CHECK(
...
#if defined(USE_MACA)
template <typename func_t>
static inline void split_fused_batch_dim_to_32bit_out(
    const at::Tensor& output,
    const at::Tensor& input,
    const at::Tensor& weight,
    const at::Tensor& z,
  ...
  if (ni <= int_max && no <= int_max) {
    func_32bit(output, input, weight, z, bias, padding, stride, dilation, groups, benchmark, deterministic, allow_tf32);
    return;
  }
  ...
  if (split_size * max_inner_size < int_max) {
    for (const auto i : c10::irange(num_splits)) {
      ...
      Tensor input_ = input.narrow(0, start, split_size_);
      Tensor output_ = output.narrow(0, start, split_size_);
      Tensor z_ = z.narrow(0, start, split_size_);
      func_32bit(output_, input_, weight, z_, bias, padding, stride, dilation, groups, benchmark, deterministic, allow_tf32);
    }
    return;
  }
  TORCH_INTERNAL_ASSERT(false, "This case should not be dispatched to cuDNN.");
...
void raw_cudnn_convolution_bias_fused_out_32bit(
    const Tensor& output, const Tensor& input, const Tensor& weight,  const Tensor& z, const Tensor& bias,
    IntArrayRef padding, IntArrayRef stride, IntArrayRef dilation, int64_t groups,
    bool benchmark, bool deterministic, bool allow_tf32) {
  ...
  ActivationDescriptor adesc;
  adesc.set(CUDNN_ACTIVATION_IDENTITY);
...
// #if !HAS_CUDNN_V8()
// void raw_cudnn_convolution_bias_fused_out(
// #else
void raw_cudnn_convolution_bias_fused_out_v7(
// #endif
    const Tensor& output, const Tensor& input, const Tensor& weight, const Tensor& z, const Tensor& bias,
    IntArrayRef padding, IntArrayRef stride, IntArrayRef dilation, int64_t groups,
    bool benchmark, bool deterministic, bool allow_tf32) {
  split_fused_batch_dim_to_32bit_out(output, input, weight, z, bias, padding, stride, dilation, groups, benchmark, deterministic, allow_tf32,
                               1024 * 1024 * 256, raw_cudnn_convolution_bias_fused_out_32bit);
}
#else
  static_assert(0);
#endif  //USE_MACA

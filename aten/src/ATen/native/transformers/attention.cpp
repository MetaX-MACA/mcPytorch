...
Tensor scaled_dot_product_attention(
    ...
    double dropout_p,
    bool is_causal) {
  bool print_mha_shape = at::maca::get_maca_print_mha_shape();
  if (print_mha_shape){
      printf("query_ info: shape(");
      for (int64_t i = 0; i < query_.dim(); ++i)
      {printf("%ld,",query_.size(i));}
      printf(") stride(");
      for (int64_t i = 0; i < query_.dim(); ++i)
      {printf("%ld,",query_.stride(i));}
      printf(")\n");

      printf("key info: shape(");
      for (int64_t i = 0; i < key.dim(); ++i)
      {printf("%ld,",key.size(i));}
      printf(") stride(");
      for (int64_t i = 0; i < key.dim(); ++i)
      {printf("%ld,",key.stride(i));}
      printf(")\n");

      printf("value info: shape(");
      for (int64_t i = 0; i < value.dim(); ++i)
      {printf("%ld,",value.size(i));}
      printf(") stride(");
      for (int64_t i = 0; i < value.dim(); ++i)
      {printf("%ld,",value.stride(i));}
      printf(")\n");
  }
  ...
  if (query_.device().type() == DeviceType::CUDA){
    choice_int = _fused_sdp_choice_stub(query_.device().type(),
      query_, key, value, attn_mask_, dropout_p, is_causal);
    bool enable_mha_fusion = at::maca::get_maca_enable_mha_fusion();
    if (enable_mha_fusion && dropout_p == 0.0 && is_causal == false && query_.dim() == 4 && key.dim() == 4 && value.dim() == 4 \
        && query_.dtype() == c10::kHalf && key.dtype() == c10::kHalf && value.dtype() == c10::kHalf \
        && query_.size(3) == 64 && key.size(3) == 64 && value.size(3) == 64 \
        && query_.strides()[3] == 1 && key.strides()[3] == 1 && value.strides()[3] == 1 \
        && ((query_.size(2) == 1024 && key.size(2) == 1024 && value.size(2) == 1024) \
        || (query_.size(2) == 256 && key.size(2) == 256 && value.size(2) == 256) \
        || (query_.size(2) == 4096 && key.size(2) == 4096 && value.size(2) == 4096) \
        || (query_.size(2) == 1024 && key.size(2) == 77 && value.size(2) == 77) \
        || (query_.size(2) == 256 && key.size(2) == 77 && value.size(2) == 77) \
        || (query_.size(2) == 4096 && key.size(2) == 77 && value.size(2) == 77) \
        || (query_.size(2) == 64 && key.size(2) == 64 && value.size(2) == 64) \
        || (query_.size(2) == 64 && key.size(2) == 77 && value.size(2) == 77))){
      choice_int = 3;
    }
  }
  ...
  switch (backend) {
    case sdp::SDPBackend::mha_fusion_attention: {
      return at::_scaled_dot_product_mha_attention(query_, key, value, dropout_p, is_causal);
    }

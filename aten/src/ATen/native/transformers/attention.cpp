...
#ifdef USE_MACA
      if (at::maca::get_maca_enable_fa_cpp_interface() && query_device_type == DeviceType::CUDA){
        auto og_scale = sdp::calculate_scale(query_, scale);
        at::Tensor q_padded, k_padded, v_padded;
        const auto sizes = query_.sizes();
        const long int head_size_og = sizes[3];
        if (head_size_og % 8 != 0) {
              q_padded = at::pad(query_, {0, 8 - head_size_og % 8}).contiguous();
              k_padded = at::pad(key, {0, 8 - head_size_og % 8}).contiguous();
              v_padded = at::pad(value, {0, 8 - head_size_og % 8}).contiguous();
          } else {
              q_padded = query_.contiguous();
              k_padded = key.contiguous();
              v_padded = value.contiguous();
        }
        auto output_list = at::_scaled_dot_product_flash_attention(
          q_padded,
          k_padded,
          v_padded,
          dropout_p,
          is_causal,
          false,
          og_scale.guard_float("attention.cpp", 749)
          );
       at::Tensor tensor_out = std::get<0>(output_list);
      //  if (head_size_og % 8 != 0) {
      tensor_out = at::slice(tensor_out, 3, 0, head_size_og);
      //  }
       return tensor_out;
      } else
#endif

...
std::tuple<Tensor, Tensor, Tensor, Tensor, c10::SymInt, c10::SymInt, Tensor, Tensor, Tensor> _scaled_dot_product_flash_attention_cuda(
  ...
  auto query_t = query.contiguous().transpose(-2, -3);
  auto key_t = key.contiguous().transpose(-2, -3);
  auto value_t = value.contiguous().transpose(-2, -3);
  auto q_dtype = query_t.dtype();
  TORCH_CHECK(q_dtype == ScalarType::Half || q_dtype == ScalarType::BFloat16,
              "FlashAttention only support fp16 and bf16 data type");
  TORCH_CHECK(key_t.dtype() == q_dtype, "query and key must have the same dtype");
  TORCH_CHECK(value_t.dtype() == q_dtype, "query and value must have the same dtype");
  CHECK_DEVICE(query_t);
  CHECK_DEVICE(key_t);
  CHECK_DEVICE(value_t);
  TORCH_CHECK(query_t.stride(-1) == 1, "Input tensor must have contiguous last dimension");
  TORCH_CHECK(key_t.stride(-1) == 1, "Input tensor must have contiguous last dimension");
  TORCH_CHECK(value_t.stride(-1) == 1, "Input tensor must have contiguous last dimension");
  const auto sizes = query_t.sizes();
  const int batch_size = sizes[0];
  int seqlen_q = sizes[1];
  int num_heads = sizes[2];
  const int head_size = sizes[3];
  const int seqlen_k = key_t.size(1);
  const int num_heads_k = key_t.size(2);
  TORCH_CHECK(batch_size > 0, "batch size must be poistive");
  TORCH_CHECK(head_size <= 256, "FlashAttention forward only supports head dimension at most 256");
  TORCH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
  CHECK_SHAPE(query_t, batch_size, seqlen_q, num_heads, head_size);
  CHECK_SHAPE(key_t, batch_size, seqlen_k, num_heads_k, head_size);
  CHECK_SHAPE(value_t, batch_size, seqlen_k, num_heads_k, head_size);
  auto tensor_out = at::empty_like(query_t);
  at::Tensor attn_mask = at::empty({0}, query_t.options());
  auto round_multiple = [](int x, int m) { return (x + m - 1) / m * m; };
  const int seqlen_q_rounded = round_multiple(seqlen_q, 128);
  const int seqlen_k_rounded = round_multiple(seqlen_k, 128);
  at::cuda::CUDAGuard device_guard{(signed char)query.get_device()};
  Tensor_t q_mcfa = convert_mcfa_tensor(query_t);
  Tensor_t k_mcfa = convert_mcfa_tensor(key_t);
  Tensor_t v_mcfa = convert_mcfa_tensor(value_t);
  Tensor_t out_mcfa = convert_mcfa_tensor(tensor_out);
  Tensor_t attn_mask_mcfa = convert_mcfa_tensor(attn_mask);
  const double softmax_scale = scale.value();
  int64_t counter_offset = batch_size * num_heads * 32;
  Tensor rng_state = at::zeros({2}, at::kLong);
  if (dropout_p > 0.0)  {
      get_philox_state(std::nullopt, rng_state, counter_offset);
  }
  at::Tensor p = at::empty({ batch_size, num_heads, seqlen_q_rounded, seqlen_k_rounded }, query.options());
  Tensor_t p_mcfa = convert_mcfa_tensor(p);
  Tensor_t rng_state_mcfa = convert_mcfa_tensor(rng_state);
  at::Tensor softmax_lse = at::empty({batch_size, num_heads, seqlen_q}, query.options().dtype(at::kFloat));
  ...
  TORCH_CHECK(ret == MCFLASHATTN_STATUS_SUCCESS, "Call mha_fwd failed.");
  release_mcfa_tensor({q_mcfa ,k_mcfa, v_mcfa, out_mcfa, attn_mask_mcfa, softmax_lse_mcfa, p_mcfa, rng_state_mcfa});

  tensor_out = tensor_out.transpose(-2, -3);
  auto options = at::TensorOptions().device(at::kCUDA).dtype(at::kUInt64);
  return std::make_tuple(tensor_out, softmax_lse, Tensor(), Tensor(), max_seqlen_batch_q, max_seqlen_batch_k, at::empty({2}, options), at::empty({}, options), attn_mask);
}
#else
std::tuple<Tensor, Tensor, Tensor, Tensor, c10::SymInt, c10::SymInt, Tensor, Tensor, Tensor> _scaled_dot_product_flash_attention_cuda(
    const Tensor& query,

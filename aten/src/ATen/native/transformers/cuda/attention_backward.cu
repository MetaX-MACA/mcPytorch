...
#endif
#ifdef USE_MACA
#include "fa_maca_api/py_export_capi_utils.h"
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> _scaled_dot_product_flash_attention_backward_cuda(
    ...
    std::optional<double> scale){
  // printf("grad_out_.mean():%f, grad_out_.std():%f\n", grad_out_.mean().cpu().item<float>(), grad_out_.std().cpu().item<float>());
  // printf("query_padded.mean():%f, query_padded.std():%f\n", query_padded.mean().cpu().item<float>(), query_padded.std().cpu().item<float>());
  // printf("key_padded.mean():%f, key_padded.std():%f\n", key_padded.mean().cpu().item<float>(), key_padded.std().cpu().item<float>());
  // printf("value_padded.mean():%f, value_padded.std():%f\n", value_padded.mean().cpu().item<float>(), value_padded.std().cpu().item<float>());
  // printf("out_padded.mean():%f, out_padded.std():%f\n", out_padded.mean().cpu().item<float>(), out_padded.std().cpu().item<float>());
  // printf("out_padded.size(0):%lld, out_padded.size(1):%lld, out_padded.size(2):%lld, out_padded.size(3):%lld\n", out_padded.size(0),  out_padded.size(1),  out_padded.size(2),  out_padded.size(3));
  // printf("softmax_lse.mean():%f, softmax_lse.std():%f\n", softmax_lse.mean().cpu().item<float>(), softmax_lse.std().cpu().item<float>());
  // printf("dropout_p:%f\n", dropout_p);
  // printf("is_causal:%d\n", is_causal);
  if (!grad_out_.defined()) {
    printf("grad_out_ not defined\n");
    return std::make_tuple(Tensor{}, Tensor{}, Tensor{});
  }
  auto out_t = out.transpose(1,2);
  auto grad_out_t = grad_out_.transpose(1,2);
  auto query_padded_transpose = query.transpose(1, 2);
  auto key_padded_transpose = key.transpose(1, 2);
  auto value_padded_transpose = value.transpose(1, 2);
  at::Tensor attn_mask;
  bool is_dropout = dropout_p > 0.0;
  auto q_dtype = query_padded_transpose.dtype();
  TORCH_CHECK(q_dtype == ScalarType::Half || q_dtype == ScalarType::BFloat16,
              "FlashAttention only support fp16 and bf16 data type");
  TORCH_CHECK(key_padded_transpose.dtype() == q_dtype, "query and key must have the same dtype");
  TORCH_CHECK(value_padded_transpose.dtype() == q_dtype, "query and value must have the same dtype");
  TORCH_CHECK(out.dtype() == q_dtype, "query and out must have the same dtype");
  TORCH_CHECK(grad_out_t.dtype() == q_dtype, "query and dout must have the same dtype");

  CHECK_DEVICE(query_padded_transpose); CHECK_DEVICE(key_padded_transpose); CHECK_DEVICE(value_padded_transpose);
  CHECK_DEVICE(out); CHECK_DEVICE(grad_out_t); CHECK_DEVICE(logsumexp);

  TORCH_CHECK(query_padded_transpose.stride(-1) == 1, "Input tensor query_padded_transpose must have contiguous last dimension");
  TORCH_CHECK(key_padded_transpose.stride(-1) == 1, "Input tensor key_padded_transpose must have contiguous last dimension");
  TORCH_CHECK(value_padded_transpose.stride(-1) == 1, "Input tensor value_padded_transpose must have contiguous last dimension");
  TORCH_CHECK(out.stride(-1) == 1, "out tensor must have contiguous last dimension");
  // printf("grad_out_t.stride(0):%d\n", grad_out_t.stride(0));
  // printf("grad_out_t.stride(1):%d\n", grad_out_t.stride(1));
  // printf("grad_out_t.stride(2):%d\n", grad_out_t.stride(2));
  // printf("grad_out_t.stride(3):%d\n", grad_out_t.stride(3));
  TORCH_CHECK(grad_out_t.stride(-1) == 1, "dout tensor must have contiguous last dimension");
  const auto sizes = query_padded_transpose.sizes();
  const int batch_size = sizes[0];
  const int seqlen_q = sizes[1];
  const int num_heads = sizes[2];
  // printf("head_size_og:%d\n", head_size_og);
  const int head_size = sizes[3];
  // printf("head_size:%d\n", head_size);
  const int seqlen_k = key_padded_transpose.size(1);
  const int num_heads_k = key_padded_transpose.size(2);
  ...
  CHECK_SHAPE(query_padded_transpose, batch_size, seqlen_q, num_heads, head_size);
  CHECK_SHAPE(key_padded_transpose, batch_size, seqlen_k, num_heads_k, head_size);
  CHECK_SHAPE(value_padded_transpose, batch_size, seqlen_k, num_heads_k, head_size);
  CHECK_SHAPE(out_t, batch_size, seqlen_q, num_heads, head_size);
  CHECK_SHAPE(grad_out_t, batch_size, seqlen_q, num_heads, head_size);

  at::Tensor dq, dk, dv;
  dq = at::empty_like(query_padded_transpose);
  dk = at::empty_like(key_padded_transpose);
  dv = at::empty_like(value_padded_transpose);
  at::cuda::CUDAGuard device_guard{(signed char)query_padded_transpose.get_device()};
  auto opts = query_padded_transpose.options();
  ...
  Tensor_t q_mcfa = convert_mcfa_tensor(query_padded_transpose);
  Tensor_t k_mcfa = convert_mcfa_tensor(key_padded_transpose);
  Tensor_t v_mcfa = convert_mcfa_tensor(value_padded_transpose);
  Tensor_t out_mcfa = convert_mcfa_tensor(out_t);
  Tensor_t grad_out_mcfa = convert_mcfa_tensor(grad_out_t);
  Tensor_t dq_mcfa = convert_mcfa_tensor(dq);
  Tensor_t dk_mcfa = convert_mcfa_tensor(dk);
  Tensor_t dv_mcfa = convert_mcfa_tensor(dv);
  Tensor_t dq_accum_mcfa = convert_mcfa_tensor(dq_accum);
  Tensor_t softmax_lse_mcfa = convert_mcfa_tensor(logsumexp);
  Tensor_t softmax_d_mcfa = convert_mcfa_tensor(softmax_d);
  // Tensor_t alibi_slopes_mcfa = convert_mcfa_tensor(alibi_slopes_);
  Tensor_t attn_mask_mcfa = convert_mcfa_tensor(attn_mask);
  int64_t counter_offset = batch_size * num_heads * 32;
  auto rng_state = at::empty({2}, at::kLong);
  if(is_dropout) {
    get_philox_state(std::nullopt, rng_state, counter_offset);
  }
  ...
  auto ret = mha_bwd(
        ...
        /*seqlen_q_=*/seqlen_q,
        /*num_heads_q_=*/num_heads,
        /*seqlen_k_=*/seqlen_k,
        /*num_heads_k_=*/num_heads_k,
        ...
        /*v=*/v_mcfa,
        /*out=*/out_mcfa,
        /*softmax_d=*/softmax_d_mcfa,
        /*softmax_lse=*/softmax_lse_mcfa,
        /*dq=*/dq_mcfa,
        /*dk=*/dk_mcfa,
        /*dv=*/dv_mcfa,
        /*dq_accum=*/dq_accum_mcfa,
        /*alibi_slopes_=*/nullptr,
        /*attn_mask_=*/attn_mask_mcfa,
        /*rng_state=*/rng_state_mcfa,
        /*p_dropout=*/dropout_p,
        /*softmax_scale=*/ scale.value(),
        /*is_causal=*/is_causal,
        /*window_size_left=*/-1,
        /*window_size_right=*/-1,
        /*deterministic=*/false,
        /*stream=*/stream,
        /*extend_parameter_=*/nullptr
        );
  TORCH_CHECK(ret == MCFLASHATTN_STATUS_SUCCESS, "Call mha_bwd failed.");
  release_mcfa_tensor({q_mcfa ,k_mcfa, v_mcfa, out_mcfa,
                        grad_out_mcfa, dq_mcfa, dk_mcfa, dv_mcfa,
                        dq_accum_mcfa, softmax_lse_mcfa,
                        softmax_d_mcfa, attn_mask_mcfa, rng_state_mcfa});
  dq = dq.transpose(1,2);
  dk = dk.transpose(1,2);
  dv = dv.transpose(1,2);
  return std::make_tuple(std::move(dq), std::move(dk), std::move(dv));

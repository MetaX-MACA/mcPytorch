...
#ifdef USE_MACA
    auto otherV = __shfl_xor_sync(UINT64_MAX, myValue, 1);
#else
    auto otherV = __shfl_xor_sync(0xffffffff, myValue, 1);
#endif
    myValue = fn(myValue, otherV);
#ifdef USE_MACA
    otherV = __shfl_xor_sync(UINT64_MAX, myValue, 2);
#else
...
#ifdef USE_MACA
    auto otherV = __shfl_xor_sync(UINT64_MAX, myValue, 1 << 1);
#else
...
#ifdef USE_MACA
    otherV = __shfl_xor_sync(UINT64_MAX, myValue, 1 << 3);
#else
...
#ifdef USE_MACA
      auto otherV = __shfl_xor_sync(UINT64_MAX, myValue, bit);
#else

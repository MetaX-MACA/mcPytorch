...
#ifdef USE_MACA
bool can_use_maca_flash_attention(sdp_params const& params, bool debug) {
  constexpr auto supported_flash_dtypes =
      c10::array_of<at::ScalarType>(at::kBFloat16, at::kHalf);
  bool can_use = params.query.dim() == 4 && params.query.size(-1) <= 256 \
      && check_tensor_dtype(params, supported_flash_dtypes, debug) && check_runtime_disabled_flash(params, debug) \
      && (((params.query.size(-1) == params.key.size(-1)) && (params.key.size(-1)== params.value.size(-1))) \
          || ((params.query.size(-1) == params.key.size(-1) == 192) && params.value.size(-1) == 128)) \
      && !params.attn_mask.has_value();

  if (at::maca::get_maca_enable_fa_input_info()){
      std::ostringstream q_shape;
      std::ostringstream k_shape;
      std::ostringstream v_shape;
      for (auto i=0; i<params.query.dim(); ++i)
        q_shape << params.query.size(i) << ", ";
      for (auto i=0; i<params.key.dim(); ++i)
        k_shape << params.key.size(i) << ", ";
      for (auto i=0; i<params.value.dim(); ++i)
        v_shape << params.value.size(i) << ", ";

      TORCH_WARN(
        "The inputs are ", can_use ? "supported " : "not supported ", "by flash-attn:",
        " dims: ", params.query.dim(), ",",
        " query shape: ", q_shape.str(), "dtype: ", params.query.dtype(), ",",
        " key shape: ", q_shape.str(), "dtype: ", params.key.dtype(), ",",
        " value shape: ", q_shape.str(), "dtype: ", params.value.dtype(), ",",
        " enable_gqa: ", params.enable_gqa, ", ",
        " attn_mask: ", params.attn_mask.has_value(), ", "
      );
  }
  if (can_use){
    return true;
...
#ifdef USE_MACA
        if (at::maca::get_maca_enable_fa_cpp_interface()){
          if (sdp::can_use_maca_flash_attention(kernel_params, print_debug)) {

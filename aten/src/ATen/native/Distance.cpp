...
#if defined(USE_MACA)
void print_cdist_forward_info(
  const Tensor& x1, const Tensor& x2, const double& p,int64_t compute_mode
) {
    std::ostringstream oss;
    oss << "cdistforward info:";

    oss << "x1 shape(";
    for (int64_t i = 0; i < x1.dim(); ++i)
        oss << to_string(x1.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < x1.dim(); ++i)
        oss << to_string(x1.stride(i)) << ",";
    oss << ") dtype:" << c10::str(x1.scalar_type()) <<",";

    oss << "x2 shape(";
    for (int64_t i = 0; i < x2.dim(); ++i)
        oss << to_string(x2.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < x2.dim(); ++i)
        oss << to_string(x2.stride(i)) << ",";
    oss << ") dtype:" << c10::str(x2.scalar_type()) <<",";

    oss << "p:" << to_string(p) << ", compute_mode:" << to_string(compute_mode) << "\n";
...
#ifdef USE_MACA
  if (C10_UNLIKELY(at::maca::get_maca_enable_print_cdist_info())) {
    print_cdist_forward_info(x1, x2, p, compute_mode.value_or(0));
  }
#endif
...
#if defined(USE_MACA)
void print_cdist_backward_info(
  const Tensor& _grad, const Tensor& _x1, const Tensor& _x2, const Tensor& _cdist, const double& p
) {
    std::ostringstream oss;
    oss << "cdistbackward info:";

    oss << "_grad shape(";
    for (int64_t i = 0; i < _grad.dim(); ++i)
        oss << to_string(_grad.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < _grad.dim(); ++i)
        oss << to_string(_grad.stride(i)) << ",";
    oss << ") dtype:" << c10::str(_grad.scalar_type()) <<",";

    oss << "_x1 shape(";
    for (int64_t i = 0; i < _x1.dim(); ++i)
        oss << to_string(_x1.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < _x1.dim(); ++i)
        oss << to_string(_x1.stride(i)) << ",";
    oss << ") dtype:" << c10::str(_x1.scalar_type()) <<",";

    oss << "_x2 shape(";
    for (int64_t i = 0; i < _x2.dim(); ++i)
        oss << to_string(_x2.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < _x2.dim(); ++i)
        oss << to_string(_x2.stride(i)) << ",";
    oss << ") dtype:" << c10::str(_x2.scalar_type()) <<",";

    oss << "_cdist shape(";
    for (int64_t i = 0; i < _cdist.dim(); ++i)
        oss << to_string(_cdist.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < _cdist.dim(); ++i)
        oss << to_string(_cdist.stride(i)) << ",";
    oss << ") dtype:" << c10::str(_cdist.scalar_type()) <<",";
...
#ifdef USE_MACA
  if (C10_UNLIKELY(at::maca::get_maca_enable_print_cdist_info())) {
    print_cdist_backward_info(_grad, _x1, _x2, _cdist, p);
  }
#endif

...
struct ConvParams {
  ...
  bool use_cudnn_depthwise(const at::Tensor& input, const at::Tensor& weight) const  {
    ...
    if (detail::getCUDAHooks().supportsDepthwiseConvolutionWithCuDNN()) {
      ...
      #if defined(CUDNN_VERSION) && defined(USE_MACA)
        cudnn_version = 8200;
      #endif
...
void print_conv_forward_info(
    const Tensor& input_r, const Tensor& weight_r, const c10::optional<Tensor>& bias_r_opt,
    IntArrayRef& stride_, IntArrayRef& padding_, IntArrayRef& dilation_,
    bool& transposed_, IntArrayRef& output_padding_, int64_t& groups_,
    bool& benchmark, bool& deterministic, bool& cudnn_enabled, bool& allow_tf32
) {
    printf("convforward info: input shape(");
    for (int64_t i = 0; i < input_r.dim(); ++i)
    { printf("%d,",input_r.size(i)); }
    printf(") stride(");
    for (int64_t i = 0; i < input_r.dim(); ++i)
    { printf("%d,",input_r.stride(i)); }
    printf(") dtype: %s ; ", c10::str(input_r.scalar_type()).c_str());

    printf("weight shape(");
    for (int64_t i = 0; i < weight_r.dim(); ++i)
    { printf("%d,",weight_r.size(i)); }
    printf(") stride(");
    for (int64_t i = 0; i < weight_r.dim(); ++i)
    { printf("%d,",weight_r.stride(i)); }
    printf(") dtype: %s ; ", c10::str(weight_r.scalar_type()).c_str());
    ...
    printf(");");
    printf(" padding_(");
    for (int64_t i = 0; i < padding_.size(); ++i) {
      printf("%d,",padding_[i]);
    }
    printf(");");
    printf(" dilation_(");
    for (int64_t i = 0; i < dilation_.size(); ++i) {
      printf("%d,",dilation_[i]);
    }
    printf(");");
    printf(" output_padding_(");
    for (int64_t i = 0; i < output_padding_.size(); ++i) {
      printf("%d,",output_padding_[i]);
    }
    printf(");");
    printf("transposed_:%d, groups_%d, benchmark:%d, deterministic:%d cudnn_enabled:%d, allow_tf32:%d\n",
         transposed_, groups_, benchmark, deterministic, cudnn_enabled, allow_tf32);
}
...
at::Tensor _convolution(
    ...
    bool transposed_, IntArrayRef output_padding_, int64_t groups_,
    bool benchmark, bool deterministic, bool cudnn_enabled, bool allow_tf32) {
  bool is_print = at::maca::get_maca_enable_print_conv_info();
  if (C10_UNLIKELY(is_print)) {
    print_conv_forward_info(
      input_r, weight_r, bias_r_opt, stride_, padding_, dilation_, transposed_, 
      output_padding_, groups_, benchmark, deterministic, cudnn_enabled, allow_tf32);
  }
  ...
  // Expand 1d -> 2d.
  // This is only done for backends that don't natively support 1d spatial input.
  auto default_nlc = at::maca::get_maca_default_nlc_memory_format(); 
  if (k == 3 && !input.is_mkldnn() && !input.is_xpu()) {
    // avoid accidentally going through NHWC for permuted 3d input.
    if (!default_nlc) {
      input = input.contiguous();
    } else {
      bool is_input_nlc = false;
      bool is_weight_nlc = false;
      auto input_shape = input.sizes();
      auto input_strides = input.strides();
      auto weight_shape = weight.sizes();
      auto weight_strides = weight.strides();
      std::vector<int64_t> weight_nlc_strides = {weight.size(1) * weight.size(2), 1, weight.size(1)};
      std::vector<int64_t> input_nlc_strides = {input.size(1) * input.size(2), 1, input.size(1)};
      if (input_strides.equals(input_nlc_strides)) {
        is_input_nlc = true;
      }
      if (weight_strides.equals(weight_nlc_strides)) {
        is_weight_nlc = true;
      }
      if (is_input_nlc && !is_weight_nlc) {
        // trans weight to nlc
        weight = weight.transpose(1, 2);
        weight = weight.contiguous();
        weight = weight.as_strided(weight_shape, weight_nlc_strides);

      } else if (!is_input_nlc && is_weight_nlc) {
        // trans input to nlc
        input = input.transpose(1, 2);
        input = input.contiguous();
        input = input.as_strided(input_shape, input_nlc_strides);
      } else if (!is_input_nlc && !is_weight_nlc) {
        // using ncl kernel
        input = input.contiguous();
        weight = weight.contiguous();
  ...
  bool disable_bias_fusion = at::maca::get_maca_disable_mcdnn_bias_fusion();
  // Call the backend.
  Tensor output;
  auto kernel_size = weight.sizes().slice(2);
  switch (backend) {
    ...
    case ConvBackend::Cudnn:
      check_input_same_type_as_parameters(input, weight, bias);
      if (bias.defined() && !disable_bias_fusion && input.scalar_type() == ScalarType::Half) {
        output = at::cudnn_convolution_bias_fused(
...
void print_conv_backward_info(
   const Tensor& grad_output_, const Tensor& input_, const Tensor& weight_,
    ...
    printf("convbackward info: input shape(");
    for (int64_t i = 0; i < input_.dim(); ++i)
    { printf("%d,",input_.size(i)); }
    printf(") stride(");
    for (int64_t i = 0; i < input_.dim(); ++i)
    { printf("%d,",input_.stride(i)); }
    printf(") dtype: %s ; ", c10::str(input_.scalar_type()).c_str());

    printf("weight shape(");
    for (int64_t i = 0; i < weight_.dim(); ++i)
    { printf("%d,",weight_.size(i)); }
    printf(") stride(");
    for (int64_t i = 0; i < weight_.dim(); ++i)
    { printf("%d,",weight_.stride(i)); }
    printf(") dtype: %s ; ", c10::str(weight_.scalar_type()).c_str());
    
    printf("grad_output_ shape(");
    for (int64_t i = 0; i < grad_output_.dim(); ++i)
    { printf("%d,",grad_output_.size(i)); }
    printf(") stride(");
    for (int64_t i = 0; i < grad_output_.dim(); ++i)
    { printf("%d,",grad_output_.stride(i)); }
    printf(") dtype: %s ; ", c10::str(grad_output_.scalar_type()).c_str());
    ...
    printf(");");
    printf(" dilation_(");
    for (int64_t i = 0; i < dilation.size(); ++i) {
      printf("%d,",dilation[i]);
    }
    printf(");");
    printf(" output_padding_(");
    for (int64_t i = 0; i < output_padding.size(); ++i) {
      printf("%d,",output_padding[i]);
    }
    printf(");");
    printf("transposed_:%d, groups_%d\n", transposed, groups);
}
...
std::tuple<Tensor, Tensor, Tensor> convolution_backward(
  ...
  bool is_print = at::maca::get_maca_enable_print_conv_info();
  if (C10_UNLIKELY(is_print)) {
    print_conv_backward_info(
      grad_output_, input_, weight_, bias_sizes_opt, stride, padding, dilation,
  ...
  if (output_mask[2]) {
    if (!backend_grad_bias.defined()) {
      bool push_on_cpu = at::maca::get_maca_push_conv_backend_grad_bias_on_cpu();      
      if(maca_unlikely(push_on_cpu && input.is_cuda())){
         auto grad_output_cpu = grad_output.cpu();
         backend_grad_bias = grad_output_cpu.sum((dim == 3) ? IntArrayRef{0, 2, 3, 4} : IntArrayRef{0, 2, 3});
         backend_grad_bias = backend_grad_bias.cuda();
      }else{

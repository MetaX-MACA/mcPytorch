...
#ifdef USE_MACA
      if (!at::maca::get_maca_enable_conv_bf16_enter_origin_logic()) {
        return true;
      } else {
#endif
...
#ifdef USE_MACA
  if (at::maca::get_maca_enable_cudnn_depthwise_backend()) {
    return true;
  }
#endif
...
void print_conv_forward_info(
    ...
    oss << "convforward info:";

    oss << "input_r shape(";
    for (int64_t i = 0; i < input_r.dim(); ++i)
        oss << to_string(input_r.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < input_r.dim(); ++i)
        oss << to_string(input_r.stride(i)) << ",";
    oss << ") dtype:" << c10::str(input_r.scalar_type()) <<",";

    oss << "weight_r shape(";
    for (int64_t i = 0; i < weight_r.dim(); ++i)
        oss << to_string(weight_r.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < weight_r.dim(); ++i)
        oss << to_string(weight_r.stride(i)) << ",";
    oss << ") dtype:" << c10::str(weight_r.scalar_type()) <<",";

    oss << " stride_(";
    for (int64_t i = 0; i < stride_.size(); ++i) {
      oss << to_string(stride_[i]) << ",";
    }
    oss << ");";

    oss << " padding_(";
    for (int64_t i = 0; i < padding_.size(); ++i) {
      oss << to_string(padding_[i]) << ",";
    }
    oss << ");";

    oss << " dilation_(";
    for (int64_t i = 0; i < dilation_.size(); ++i) {
      oss << to_string(dilation_[i]) << ",";
    }
    oss << ");";

    oss << " output_padding_(";
    for (int64_t i = 0; i < output_padding_.size(); ++i) {
      oss << to_string(output_padding_[i]) << ",";
    }
    oss << ");";

    oss << "transposed_:" << to_string(transposed_) << ", groups_:" << to_string(groups_)
        << ", benchmark:" << to_string(benchmark) << ", deterministic:" << to_string(deterministic)
        << ", cudnn_enabled:" << to_string(cudnn_enabled) << ", allow_tf32:" << to_string(allow_tf32) << "\n";
...
void print_conv_backward_info(
   const Tensor& grad_output_, const Tensor& input_, const Tensor& weight_,
    ...
    std::ostringstream oss;
    oss << "convbackward info: ";

    oss << "input_ shape(";
    for (int64_t i = 0; i < input_.dim(); ++i)
        oss << to_string(input_.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < input_.dim(); ++i)
        oss << to_string(input_.stride(i)) << ",";
    oss << ") dtype:" << c10::str(input_.scalar_type()) <<",";

    oss << "weight_ shape(";
    for (int64_t i = 0; i < weight_.dim(); ++i)
        oss << to_string(weight_.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < weight_.dim(); ++i)
        oss << to_string(weight_.stride(i)) << ",";
    oss << ") dtype:" << c10::str(weight_.scalar_type()) <<",";

    oss << "grad_output_ shape(";
    for (int64_t i = 0; i < grad_output_.dim(); ++i)
        oss << to_string(grad_output_.size(i)) << ",";
    oss << ") stride(";
    for (int64_t i = 0; i < grad_output_.dim(); ++i)
        oss << to_string(grad_output_.stride(i)) << ",";
    oss << ") dtype:" << c10::str(grad_output_.scalar_type()) <<",";

    oss << " stride(";
    for (int64_t i = 0; i < stride.size(); ++i) {
      oss << to_string(stride[i]) << ",";
    }
    oss << ");";

    oss << " padding(";
    for (int64_t i = 0; i < padding.size(); ++i) {
      oss << to_string(padding[i]) << ",";
    }
    oss << ");";

    oss << " dilation(";
    for (int64_t i = 0; i < dilation.size(); ++i) {
      oss << to_string(dilation[i]) << ",";
    }
    oss << ");";

    oss << " output_padding(";
    for (int64_t i = 0; i < output_padding.size(); ++i) {
      oss << to_string(output_padding[i]) << ",";
    }
    oss << ");";

    oss << "transposed_:" << to_string(transposed) << ", groups_:" << to_string(groups) << "\n";
    printf(":%s", oss.str().c_str());
}
#endif

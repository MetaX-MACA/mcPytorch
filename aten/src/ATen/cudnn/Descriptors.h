...
{
  switch (dataType) {
#if defined(CUDNN_VERSION) && (CUDNN_VERSION >= 8200 || defined(USE_MACA))
...
struct TORCH_CUDA_CPP_API ConvolutionDescriptor
  ...
  void set(cudnnDataType_t dataType, int dim, int* pad, int* stride, int * upscale /* aka dilation */, int groups, bool allow_tf32) {
    ...
    //TODO(): cudnn v7 is not supported bf16 in ConvolutionDescriptor.
    if (dataType == CUDNN_DATA_BFLOAT16) mathType = CUDNN_DATA_FLOAT;
...
struct TORCH_CUDA_CPP_API ActivationDescriptor
  ...
  void set(cudnnActivationMode_t mode) {
    AT_ASSERT(
        mode == CUDNN_ACTIVATION_RELU || mode == CUDNN_ACTIVATION_IDENTITY,
...
union Constant
{
  ...
  Constant(cudnnDataType_t dataType, double value) {
    if (dataType == CUDNN_DATA_HALF || dataType == CUDNN_DATA_FLOAT || dataType == CUDNN_DATA_BFLOAT16) {

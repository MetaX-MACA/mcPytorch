...
#if defined(__CUDACC__) || defined(__HIPCC__)
  auto min_val = - std::numeric_limits<T>::epsilon() / 2;
  auto log = val >= (static_cast<T>(1.) + min_val) ? min_val : __logf(val);
  return static_cast<T>(-1.0) * __builtin_mxc_rcpf(lambda) * log;
#else

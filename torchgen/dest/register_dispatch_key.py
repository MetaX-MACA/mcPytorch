...
class StructuredRegisterDispatchKey(RegisterDispatchKey):
    ...
    def gen_class_set_output_functions(
        self, k: SchemaKind, parent_class: str, generate_super: bool, class_name: str
    ) -> str:
        if generate_super:
        ...
        def gen_set_output_function(name: str, maybe_create_proxy: bool, class_name: str) -> str:
...
{textwrap.indent(self.gen_class_set_output_body(k, maybe_create_proxy, class_name, name), "    ")}
    if (!names.empty()) {{
...
"""
        ...
        return f"""
{gen_set_output_function("strided", maybe_create_proxy=True, class_name=class_name)}
{gen_set_output_function("raw_strided", maybe_create_proxy=False, class_name=class_name)}
"""

    def gen_class_set_output_body(self, k: SchemaKind, maybe_create_proxy: bool, class_name: str, name: str) -> str:
...
"""
        ...
        if k is SchemaKind.functional:
            ...
            if (class_name == "structured_bmm_out_cuda_functional" or class_name == "structured_mm_out_cuda_functional") and name == "raw_strided" and self.backend_index.dispatch_key == DispatchKey.CUDA:
                add_int8_support = """
if (typeMetaToScalarType(options.dtype()) == ScalarType::Char){
    options.set_dtype(ScalarType::Int);
}
outputs_[output_idx] = create_out(sizes, strides, options);
"""
                return f"""{maybe_set_guard_line}""" + add_int8_support
            else:
                return f"""{maybe_set_guard_line}outputs_[output_idx] = create_out(sizes, strides, options);"""
    ...
    def gen_class(
        ...
        *,
        ...
        lines = (
            ...
            f"{textwrap.indent(self.gen_class_set_output_functions(k, parent_class, generate_super, class_name), indent)}",

...
std::string FusionExecutor::getStructuredCode(const std::string& kernel) {
  ...
  std::string code = "";
#ifdef USE_MACA
  code += std::string("#include <mcr/mc_runtime.h>\n") +
      std::string("#include <maca_bfloat16.h>\n") +
      std::string("#include <maca_fp16.h>\n");
...
#else
          const int align_size = 8; // see codegen.cpp for MACA
#endif
...
#else
      TORCH_INTERNAL_ASSERT(
          false, "Cross grid communication not supported with MACA.");
#endif
    }
...
#else
      TORCH_INTERNAL_ASSERT(
          false, "cuFuncSetAttribute not supported with MACA.");
#endif
    }
    if (!kernel()->summary().has_cooperative_grid_reduction) {

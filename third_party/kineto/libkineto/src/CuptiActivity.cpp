...
#ifdef USE_MACA
template<>
inline ActivityType GpuActivity<CUpti_ActivityGraphTrace>::type() const {
  return ActivityType::GRAPH_TRACE;
}

template<>
inline const std::string GpuActivity<CUpti_ActivityGraphTrace>::name() const {
  return "Graph " + std::to_string(raw().graphId);
}
#endif
...
inline bool RuntimeActivity::flowStart() const {
  bool should_correlate =
      ...
      #ifdef USE_MACA
      activity_.cbid == CUPTI_DRIVER_TRACE_CBID_cuLaunchKernel ||
      #endif

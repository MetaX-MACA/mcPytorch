...
void CuptiActivityProfiler::handleCuptiActivity(
  ...
  switch (record->kind) {
    ...
    case CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL:
    {
      ...
      LOG(1) << "[TORCH]: CUpti_ActivityKernel4: start ts: " << kernel->start << ", end ts: " << kernel->end;
    ...
    {
      const CUpti_ActivityCudaEventType *obj = reinterpret_cast<const CUpti_ActivityCudaEventType*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityCudaEventType: correlationId: " << obj->correlationId << ", eventId: " << obj->eventId;
      handleCudaEventActivity(obj, logger);
      break;
    }

...
void CuptiActivityProfiler::handleCudaSyncActivity(
  ...
  if (isEventSync(activity->type)) {
    ...
    auto maybe_wait_event_info = getWaitEventInfo(
        activity->contextId, activity->macaEventId);
#else
...
void CuptiActivityProfiler::handleCuptiActivity(
  ...
  switch (record->kind) {
    ...
    {
      const CUpti_ActivityExternalCorrelation *obj = reinterpret_cast<const CUpti_ActivityExternalCorrelation*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityExternalCorrelation: correlationId: " << obj->correlationId << ", externalId: " << obj->externalId;
      handleCorrelationActivity(obj);
      break;
    }
    case CUPTI_ACTIVITY_KIND_RUNTIME:
    {
      const CUpti_ActivityAPI *obj = reinterpret_cast<const CUpti_ActivityAPI*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityAPI: start ts: " << obj->start << ", end ts: " << obj->end;
      handleRuntimeActivity(obj, logger);
      break;
    }
    case CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL:
    {
      const CUpti_ActivityKernel4 *obj = reinterpret_cast<const CUpti_ActivityKernel4*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityKernel4: start ts: " << obj->start << ", end ts: " << obj->end;
      handleGpuActivity(obj, logger);
      updateCtxToDeviceId(
    ...
    {
      const CUpti_ActivitySynchronization *obj = reinterpret_cast<const CUpti_ActivitySynchronization*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivitySynchronization: start ts: " << obj->start << ", end ts: " << obj->end;
      handleCudaSyncActivity(obj, logger);
      break;
    }
    case CUPTI_ACTIVITY_KIND_CUDA_EVENT:
    {
      const CUpti_ActivityCudaEvent *obj = reinterpret_cast<const CUpti_ActivityCudaEvent*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityCudaEvent: correlationId: " << obj->correlationId << ", eventId: " << obj->eventId;
      handleCudaEventActivity(obj);
      break;
    }
    case CUPTI_ACTIVITY_KIND_MEMCPY:
    {
      const CUpti_ActivityMemcpy *obj = reinterpret_cast<const CUpti_ActivityMemcpy*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityMemcpy: start ts: " << obj->start << ", end ts: " << obj->end;
      handleGpuActivity(obj, logger);
      break;
    }
    case CUPTI_ACTIVITY_KIND_MEMCPY2:
    {
      const CUpti_ActivityMemcpy2 *obj = reinterpret_cast<const CUpti_ActivityMemcpy2*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityMemcpy2: start ts: " << obj->start << ", end ts: " << obj->end;
      handleGpuActivity(obj, logger);
      break;
    }
    case CUPTI_ACTIVITY_KIND_MEMSET:
    {
      const CUpti_ActivityMemset *obj = reinterpret_cast<const CUpti_ActivityMemset*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityMemset: start ts: " << obj->start << ", end ts: " << obj->end;
      handleGpuActivity(obj, logger);
      break;
    }
    case CUPTI_ACTIVITY_KIND_OVERHEAD:
    {
      const CUpti_ActivityOverhead *obj = reinterpret_cast<const CUpti_ActivityOverhead*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityOverhead: start ts: " << obj->start << ", end ts: " << obj->end;
      handleOverheadActivity(obj, logger);
      break;
    }
    case CUPTI_ACTIVITY_KIND_DRIVER:
    {
      const CUpti_ActivityAPI *obj = reinterpret_cast<const CUpti_ActivityAPI*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityAPI: start ts: " << obj->start << ", end ts: " << obj->end;
      handleDriverActivity(obj, logger);
      break;
    }
    case CUPTI_ACTIVITY_KIND_GRAPH_TRACE:
    {
      const CUpti_ActivityGraphTrace *obj = reinterpret_cast<const CUpti_ActivityGraphTrace*>(record);
      LOG(1) << "[TORCH]: CUpti_ActivityGraphTrace: start ts: " << obj->start << ", end ts: " << obj->end;
      handleGpuActivity(obj, logger);
      updateCtxToDeviceId(
          reinterpret_cast<const CUpti_ActivityGraphTrace*>(record));
      break;
    }
    default:
...
#else
#if !defined(USE_MACA) && CUDA_VERSION >= 11060
...
#ifdef USE_MACA
  if (std::getenv("TORCH_PROFILER_RESTORE_DEFAULT_RELEASE_ORDER")) {
    #ifdef HAS_CUPTI
      cupti_.disableCuptiActivities(derivedConfig_->profileActivityTypes());
    #else
        cupti_.disableActivities(derivedConfig_->profileActivityTypes());
...
#ifdef USE_MACA
  void CuptiActivityProfiler::resetTraceData_STOP() {
  #if defined(HAS_CUPTI) || defined(HAS_ROCTRACER)
    if (!cpuOnly_) {
      cupti_.clearActivities();
      cupti_.teardownContext();

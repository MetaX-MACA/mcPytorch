...
def get_torch_version(sha: str | None = None) -> str:
    ...
    # USE_MACA
    if os.getenv("PYTORCH_MACA_VERSION"):
        version = open(pytorch_root / "version.txt", "r").read().strip()
        version += "+metax" + os.getenv("PYTORCH_MACA_VERSION")
    elif os.getenv("PYTORCH_BUILD_VERSION"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--maca-version", "--maca_version", type=str)
    ...
    args.maca_version = None if args.maca_version == "" else args.maca_version
    ...
    with open(version_path, "w") as f:
        ...
        f.write(f"maca: Optional[str] = {repr(args.maca_version)}\n")

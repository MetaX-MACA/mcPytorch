...
namespace c10::cuda {
  c10::once_flag stream_init_flag;
  std::vector<int> stream_pools = {32, 32, 32};
  //export PYTORCH_CONFIG_STREAM_POOLS=8,16,32
  static char* stream_pool_env = std::getenv("PYTORCH_CONFIG_STREAM_POOLS");

  void set_stream_pools() {
    if(stream_pool_env) {
      std::string ss = std::string(stream_pool_env);
      std::string token;
...
static void initDeviceStreamState(DeviceIndex device_index) {
  ...
  if (stream_pool_env) {
    for (const auto p : c10::irange(max_stream_priorities)) {
      for (const auto i : c10::irange(stream_pools[p])) {
...
#ifdef USE_MACA
  c10::call_once(stream_init_flag, set_stream_pools);
#endif
...
#ifdef USE_MACA
  static uint32_t get_idx(std::atomic<uint32_t>& counter, int priority) {
    TORCH_INTERNAL_ASSERT(stream_pools[priority]>0);
    auto raw_idx = counter++;
    return raw_idx % stream_pools[priority];
  }
#endif

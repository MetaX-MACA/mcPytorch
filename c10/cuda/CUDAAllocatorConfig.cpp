...
size_t getGranularitySize() {
    ...
    if(granularity == -1) {
        int current_device;
        C10_CUDA_DRIVER_CHECK(cuCtxGetDevice(&current_device));
        CUmemAllocationProp prop = {};
        prop.type = CU_MEM_ALLOCATION_TYPE_PINNED;
        prop.location.type = CU_MEM_LOCATION_TYPE_DEVICE;
        prop.location.id = current_device;
        C10_CUDA_DRIVER_CHECK(cuMemGetAllocationGranularity(&granularity, &prop, CU_MEM_ALLOC_GRANULARITY_MINIMUM));
    }
    return granularity;
}
#endif
  ...
  for (size_t i = 0; i < config.size(); i++) {
    ...
    if (config_item_view == "max_split_size_mb") {
      ...
      if (m_expandable_segments) {
        auto granulariy = getGranularitySize();
        if (granulariy != 2 * 1024 * 1024) {
          m_expandable_segments = false;
          TORCH_WARN(
              "Expandable_segments option is ignored since it's not supported "
              "when vram pagesize is not 2M.");
        }
      }
#endif

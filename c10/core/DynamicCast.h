...
#ifdef USE_MACA
  template <typename dest_t>
  C10_HOST_DEVICE inline dest_t fetch_and_cast_maca(
      const ScalarType src_type,
      const void* ptr) {
    switch (src_type) {
      AT_FORALL_SCALAR_TYPES_WITH_COMPLEX_MACA(FETCH_AND_CAST_CASE)
      default:
        ERROR_UNSUPPORTED_CAST
    }
    return dest_t(0); // just to avoid compiler warning
...
#ifdef USE_MACA
  template <typename src_t>
  C10_HOST_DEVICE inline void cast_and_store_maca(
      const ScalarType dest_type,
      void* ptr,
      src_t value) {
    switch (dest_type) {
      AT_FORALL_SCALAR_TYPES_WITH_COMPLEX_MACA(CAST_AND_STORE_CASE)
      default:;
    }
    ERROR_UNSUPPORTED_CAST
  }
...
#ifdef USE_MACA
#define FETCH_AND_CAST_CASE_NULL(type, scalartype) \
  case ScalarType::scalartype:                \
    break;

inline void fetch_and_cast_check(const ScalarType src_type) {
  switch (src_type) {
    AT_FORALL_SCALAR_TYPES_WITH_COMPLEX(FETCH_AND_CAST_CASE_NULL)
    default:
...
template <typename dest_t>
C10_HOST_DEVICE inline dest_t fetch_and_cast_without_assert(
    const ScalarType src_type,
    const void* ptr) {
  switch (src_type) {
    AT_FORALL_SCALAR_TYPES_WITH_COMPLEX(FETCH_AND_CAST_CASE)
...
template <typename src_t>
C10_HOST_DEVICE inline void cast_and_store_without_assert(
    const ScalarType dest_type,
    void* ptr,
    src_t value) {
  switch (dest_type) {

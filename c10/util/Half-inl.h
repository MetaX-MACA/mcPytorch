...
inline C10_HOST_DEVICE Half::Half(float value)
    :
#if defined(__CUDA_ARCH__) || defined(__HIP_DEVICE_COMPILE__) || defined(__MACA_ARCH__)
...
#if defined(__CUDACC__) || defined(__HIPCC__) || defined(__MACA_ARCH__)
...
// CUDA intrinsics
    ...
    (defined(__clang__) && defined(__CUDA__)) || (defined(__MACA_ARCH__))
...
inline C10_HOST_DEVICE Half operator-(const Half& a) {
    ...
    defined(__HIP_DEVICE_COMPILE__) || defined(__MACA_ARCH__)
  return __hneg(a);
#elif defined(__SYCL_DEVICE_ONLY__)

...
inline C10_HOST_DEVICE __half mcpytorch_float2half_internal(float value) {
    __half_raw r;
    r.x = __llvm_convert_to_fp16(value);
    return r;
}
#endif
...
inline C10_HOST_DEVICE Half::Half(float value)
    :
#if defined(__CUDA_ARCH__) || defined(__HIP_DEVICE_COMPILE__) || defined(__MACA_ARCH__)
      x(__half_as_short(mcpytorch_float2half_internal(value)))

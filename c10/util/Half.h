...
#elif defined(__clang__) && defined(__MACA_ARCH__)
#define C10_DEVICE_HOST_FUNCTION __device__ __host__
#else
...
C10_DEVICE_HOST_FUNCTION inline float fp32_from_bits(uint32_t w) {
  ...
  return as_float(w);
#elif defined(__CUDA_ARCH__) || defined(__MACA_ARCH__)
...
#if defined(__OPENCL_VERSION__)
  return as_uint(f);
#elif defined(__CUDA_ARCH__) && !defined(__MACA_ARCH__) // as mxcc required, and this function is only host, we comment this function

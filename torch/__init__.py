...
class _TorchCompileInductorWrapper:
    ...
    def apply_mode(self, mode: Optional[str]):
        if mode is None or mode == "default":
            ...
            self.apply_options({
                # cudagrapy is not supported by maca yet
                "triton.cudagraphs": False,
...
def compile(model: Optional[Callable] = None, *,
    ...
    """
    if os.getenv("PYTORCH_SKIP_COMPILE_CHECK") != "1":
        raise RuntimeError("Does not support torch.compile mode now.")
...
try:
    ...
    import warnings
    warnings.warn(f"import flash_attn failed and set torch.disable_flash_attn True", stacklevel=2)

...
class AdaptiveAvgPool2d(_AdaptiveAvgPoolNd):
    ...
    output_size: _size_2_opt_t
    run_on_cuda = USE_MACA and (platform.machine() == "ARM64" or platform.machine() == "aarch64") and torch.cuda.device_count() > 0
                                             
    def forward(self, input: Tensor) -> Tensor:
        if self.run_on_cuda and input.is_cpu:
            input = input.cuda()
            res = F.adaptive_avg_pool2d(input, self.output_size)
            return res.cpu()
        else:

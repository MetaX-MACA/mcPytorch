...
default_nlc = os.getenv("PYTORCH_DEFAULT_NLC")
...
class _ConvNd(Module):
    ...
    def reset_parameters(self) -> None:
        ...
        if default_nlc and self.weight.dim() == 3 and self.weight.shape[1] != 1:
            shapes = self.weight.shape
            strides = (shapes[1] * shapes[2], 1, shapes[1])
            self.weight.data = self.weight.data.transpose(1, 2)
            self.weight.data = self.weight.data.contiguous()
            self.weight.data = self.weight.data.as_strided(shapes, strides)

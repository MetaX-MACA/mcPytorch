...
def mm_configs():
    ...
    mm_triton_configs = (
        ...
        (
            [
                # TODO(MACA):
                # maca triton mm config(temporary solution with an extension, not guaranteed for performance)
                {
                    "config": {"BLOCK_M": 32, "BLOCK_N": 32, "BLOCK_K": 16},
                    "num_stages": 2,
                    "num_warps": 2,
                ...
                {
                    "config": {"BLOCK_M": 128, "BLOCK_N": 128, "BLOCK_K": 16},
                    "num_stages": 2,
                    "num_warps": 8,
                    "cond": True,
                }
            ] if not os.getenv("TORCH_INDUCTOR_ENABLE_TRITON_CONV", False)
            else []
        )
    )

...
kernel_configs = (
    [
        ...
        [
            # TODO(MACA):
            # maca triton conv config(temporary solution with an extension, not guaranteed for performance)
            # "BLOCK_M", "BLOCK_N", "BLOCK_K", "num_stages", "num_warps"
            {"config": (256, 64, 16, 1, 8), "cond": True},
        ] if os.getenv("TORCH_INDUCTOR_ENABLE_TRITON_CONV", False)
        else []
    )
)

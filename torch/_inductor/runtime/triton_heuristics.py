...
class CachingAutotuner(KernelInterface):
    ...
    def get_device_interface(self):
        ...
        return get_interface_for_device(self.device_props.type.replace("hip", "cuda").replace("maca", "cuda"))
        ...
        if ASTSource:
            ...
            if GPUTarget:
                target = GPUTarget(
                    ...
                    compile_meta["cc"],
                    rocm_warp_size if torch.version.hip else( maca_warp_size if USE_MACA else 32),
                )
            else:
                target = (
                    (compile_meta["device_type"], compile_meta["cc"])
                    if not (torch.version.hip or USE_MACA)
                    else [
                        compile_meta["device_type"],
                        compile_meta["cc"],
                        maca_warp_size if USE_MACA else rocm_warp_size,
                    ]
                )
    ...
    def save_cuda_kernel(self, grid, stream, launcher):
        ...
        binary = (
            (launcher.bin.asm["cubin"] if self.device_props.type != "maca" else "mcfatbin")
            if self.device_props.type != "hip"

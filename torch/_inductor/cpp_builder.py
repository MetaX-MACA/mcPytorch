...
def cpp_compiler_search(search: str) -> str:
    ...
    if USE_MACA:
        return os.path.join(os.getenv("MACA_PATH"), "tools", "cu-bridge", "bin", "cucc")
    for cxx in search:
        try:
            if cxx is None:
...
def get_cpp_torch_options(
    ...
    definitions = (
        ...
        + _get_use_maca_macros()  # USE_MACA as cpu/cuda all need this macro, so add here
    )
    include_dirs = (
        sys_libs_include_dirs
        + python_include_dirs

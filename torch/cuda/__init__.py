...
from .._utils_internal import USE_MACA, PYTORCH_SKIP_NVML_COUNT
...
from .._utils_internal import USE_MACA
...
try:
    ...
    try:
        if not _version.hip:
            if USE_MACA: # USE_MACA
                import pymxsml.mxsml_extension
            else:
...
def _parse_visible_devices() -> Union[list[int], list[str]]:
    ...
    # USE_MACA
    var = os.getenv("MACA_VISIBLE_DEVICES") or os.getenv("CUDA_VISIBLE_DEVICES")
...
def _raw_device_count_nvml() -> int:
    ...
    if USE_MACA:
        mxsml_h = CDLL("libmxsml.so")
        rc = mxsml_h.mxSmlExInit()
        if rc != 0:
            warnings.warn("Can't initialize MXSML", stacklevel=2)
            return -1
        dev_count = c_int(-1)

        try:
            # mxSml new api adapt to new chip interface
            rc = mxsml_h.mxSmlExMacaDeviceGetCount(byref(dev_count))
        except Exception as e:
            # use old mxSml api
            rc = mxsml_h.mxSmlExDeviceGetCount(byref(dev_count))

        if rc != 0:
            warnings.warn("Can't get mxsml device count", stacklevel=2)
            return -1
        del mxsml_h
    else:
...
# USE_MACA
def _raw_device_uuid_mxml() -> Optional[list[str]]:
    r"""Return list of device UUID as reported by MXML or None if MXM discovery/initialization failed."""
    from ctypes import byref, c_int, c_void_p, CDLL, create_string_buffer

    mxml_h = CDLL("libmxsml.so")
    rc = mxml_h.mxSmlExInit()

    if rc != 0:
        warnings.warn("Can't initialize MXML")
        return None
    dev_count = c_int(-1)
    rc = mxml_h.mxSmlExDeviceGetCount(byref(dev_count))
    ...
    for idx in range(dev_count.value):
        dev_id = c_void_p()
        rc = mxml_h.mxSmlExGetDeviceHandleByIndex(idx, byref(dev_id))
        ...
        buf_len = 96
        buf = create_string_buffer(buf_len)
        rc = mxml_h.mxSmlExDeviceGetUUID(dev_id, buf, buf_len)
...
def device_count() -> int:
    ...
    if PYTORCH_SKIP_NVML_COUNT:
        nvml_count = -1
    else:
        nvml_count = _device_count_amdsmi() if torch.version.hip \
                else _device_count_nvml() # bypass _device_count_nvml() if rocm (not supported)
...
# USE_MACA
def _get_pymxsml_handler(device: "Device" = None):
    if not _HAS_PYNVML:
        raise ModuleNotFoundError(
            "pymxsml does not seem to be installed or it can't be imported."
        ) from _PYNVML_ERR

    from pymxsml.mxsml_extension import  MXSMLEX_ERROR_DRIVER_NOT_LOADED

    try:
        pymxsml.mxsml_extension.mxSmlExInit()
    except MXSMLEX_ERROR_DRIVER_NOT_LOADED as e:
        raise RuntimeError("cuda driver can't be loaded, is cuda enabled?") from e

    device = _get_nvml_device_index(device)
    handle = pymxsml.mxsml_extension.mxSmlExDeviceGetHandleByIndex(device)
    return handle
...
def device_memory_used(device: "Device" = None) -> int:
    ...
    if not torch.version.hip:
        if USE_MACA: # USE_MACA
            handle = _get_pymxsml_handler()
            device = _get_nvml_device_index(device)
            handle = pymxsml.mxsml_extension.mxSmlExDeviceGetHandleByIndex(device)
            return pymxsml.mxsml_extension.mxSmlExDeviceGetMemoryInfo(handle).used
        else:
...
def memory_usage(device: "Device" = None) -> int:
    ...
    if not torch.version.hip:
        if USE_MACA: # USE_MACA
            ...
            device = _get_nvml_device_index(device)
            handle = pymxsml.mxsml_extension.mxSmlExDeviceGetHandleByIndex(device)
            return pymxsml.mxsml_extension.mxSmlExDeviceGetUtilizationRates(handle).memory
        else:
            handle = _get_pynvml_handler()
            device = _get_nvml_device_index(device)
            handle = pynvml.nvmlDeviceGetHandleByIndex(device)
...
def utilization(device: "Device" = None) -> int:
    ...
    if not torch.version.hip:
        if USE_MACA: # USE_MACA
            handle = _get_pymxsml_handler(device)
            device = _get_nvml_device_index(device)
            handle = pymxsml.mxsml_extension.mxSmlExDeviceGetHandleByIndex(device)
            return pymxsml.mxsml_extension.mxSmlExDeviceGetUtilizationRates(handle).gpu
        else:
...
def temperature(device: "Device" = None) -> int:
    ...
    if not torch.version.hip:
        if USE_MACA: # USE_MACA:
            handle = _get_pymxsml_handler(device)
            return pymxsml.mxsml_extension.mxSmlExDeviceGetTemperature(handle, 0)
        else:
            handle = _get_pynvml_handler(device)
...
def power_draw(device: "Device" = None) -> int:
    ...
    if not torch.version.hip:
        if USE_MACA: # USE_MACA:
            handle = _get_pymxsml_handler(device)
            return pymxsml.mxsml_extension.mxSmlExDeviceGetPowerUsage(handle)
        else:
            handle = _get_pynvml_handler(device)
...
def clock_rate(device: "Device" = None) -> int:
    ...
    if not torch.version.hip:
        if USE_MACA: # USE_MACA:
            handle = _get_pymxsml_handler(device)
            return pymxsml.mxsml_extension.mxSmlExDeviceGetClockInfo(handle, 1)
        else:
            handle = _get_pynvml_handler(device)

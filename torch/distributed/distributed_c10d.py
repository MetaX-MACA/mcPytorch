...
def _check_op(op) -> None:
    ...
    if op not in [isend, irecv, isend_sdma, irecv_sdma]:
        raise ValueError(
...
def _new_process_group_helper(
    ...
    for device, backend_str in backend_config.get_device_backend_map().items():
        ...
        if backend_str == Backend.MPI:
            ...
            # -- USE_MACA --
            if not os.getenv("PYTORCH_DISABLE_PG_HIGH_PRIORITY_STREAM"):
                backend_options.is_high_priority_stream = True
            # -- USE_MACA --
...
def isend_sdma(
    ...
    return group.send_ext([tensor], group_dst, tag)
...
def irecv_sdma(
    ...
    if src is None and group_src is None:
        ...
        group_src = _canonicalize_group_rank(group, src, group_src)
        return group.recv_ext([tensor], group_src, tag)
...
def _coalescing_manager(
    ...
    if op_list:
        ...
        # USE_MACA
            ...
            all_reduce_opts = AllreduceCoalescedOptions()
            all_reduce_opts.reduceOp = not_none(op_list[0].redop)
            work = group.allreduce_coalesced_sdma(tensors, all_reduce_opts)
        ...
        # USE_MACA
        elif op0 == all_gather_into_tensor_sdma:
            inputs = []
            outputs = []
            for op in op_list:
                inputs.append(op.tensor)
                outputs.append(not_none(op.dst_tensor))
            work = group.allgather_into_tensor_coalesced_sdma(outputs, inputs)
        ...
        # USE_MACA
        elif op0 == reduce_scatter_tensor_sdma:
            inputs = []
            outputs = []
            for op in op_list:
                inputs.append(op.tensor)
            ...
            reduce_opts = ReduceScatterOptions()
            reduce_opts.reduceOp = not_none(op_list[0].redop)
            work = group.reduce_scatter_tensor_coalesced_sdma(outputs, inputs, reduce_opts)
        else:
...
def batch_isend_irecv(p2p_op_list: list[P2POp]) -> list[Work]:
    ...
    def peer_kwarg(op: P2POp) -> dict[str, int]:
        key = "group_dst" if op.op == isend or op.op == isend_sdma else "group_src"
...
@_exception_logger
def all_reduce_sdma(tensor, op=ReduceOp.SUM, group=None, async_op=False):
    """
    Reduces the tensor data across all machines in a way that all get the final result.
    ...
    if group in _world.pg_coalesce_state.keys():
        # We are in coalescing context, do not issue single operation, just append a collective representation
        coll = _CollOp(all_reduce_sdma, tensor, None, op, None)
        _world.pg_coalesce_state[group].append(coll)
        if async_op:
            return _IllegalWork()
        else:
    ...
    work = group.allreduce_sdma([tensor], opts)
...
@_exception_logger
def all_gather_into_tensor_sdma(output_tensor, input_tensor, group=None, async_op=False):
    """
    Gather tensors from all ranks and put them in a single output tensor.
    ...
    if group in _world.pg_coalesce_state.keys():
        # We are in coalescing context, do not issue single operation, just append a collective representation
        coll = _CollOp(all_gather_into_tensor_sdma, input_tensor, output_tensor)
        _world.pg_coalesce_state[group].append(coll)
        if async_op:
            return _IllegalWork()
        else:
    ...
    work = group._allgather_base_sdma(output_tensor, input_tensor, opts)
...
def _all_gather_base_sdma(output_tensor, input_tensor, group=None, async_op=False):
    """
    Single tensor all gather. Gathers a single tensor from all ranks, and puts them in a single output tensor.
    ...
    """
    return all_gather_into_tensor_sdma(output_tensor, input_tensor, group, async_op)
...
#USE_MACA
@_exception_logger
def reduce_scatter_tensor_sdma(output, input, op=ReduceOp.SUM, group=None, async_op=False):
    """
    Reduces, then scatters a tensor to all ranks in a group.
    ...
    # If we are, do not issue single operation, just append a collective representation
    if group in _world.pg_coalesce_state.keys():
        coll = _CollOp(reduce_scatter_tensor_sdma, input, output, op, None)
        _world.pg_coalesce_state[group].append(coll)
        if async_op:
            return _IllegalWork()
        else:
    ...
    work = group._reduce_scatter_base_sdma(output, input, opts)
...
def _reduce_scatter_base_sdma(output, input, op=ReduceOp.SUM, group=None, async_op=False):
    """
    Reduces, then scatters a flattened tensor to all processes in a group.
    ...
    """
    return reduce_scatter_tensor_sdma(output, input, op, group, async_op)

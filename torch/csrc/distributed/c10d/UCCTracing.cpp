...
#if defined(USE_MACA) && ((defined(__GNUC__) && __GNUC__ < 9) ||  defined(__aarch64__))
...
void ProcessGroupUCCLogger::flushComms(int rank, int world_size) {
...
#if defined(USE_MACA) && ((defined(__GNUC__) && __GNUC__ < 9) ||  defined(__aarch64__))
  std::experimental::filesystem::path fullpath = std::experimental::filesystem::path("/tmp") / dirname;
#else
  std::filesystem::path fullpath = std::experimental::filesystem::path("/tmp") / dirname;
#endif
...
#if defined(USE_MACA) && ((defined(__GNUC__) && __GNUC__ < 9) ||  defined(__aarch64__))
  std::experimental::filesystem::path trace_filename =
  ...
  std::error_code ec{};
#if defined(USE_MACA) && ((defined(__GNUC__) && __GNUC__ < 9) ||  defined(__aarch64__))
  if (!std::experimental::filesystem::create_directories(fullpath, ec)) {
#else

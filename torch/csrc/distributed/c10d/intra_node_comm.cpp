...
#if !defined(USE_ROCM) && defined(PYTORCH_C10_DRIVER_API_SUPPORTED) && !defined(USE_MACA)
...
bool IntraNodeComm::rendezvous() {
  ...
  //(todo)bincheng.xiong@metax-tech.com, add NVML_DEVICE_PCI_BUS_ID_FMT for passing compile
  ...
  {
#if defined(USE_MACA) && defined(__GNUC__) && __GNUC__ < 9
    std::unordered_set<std::string> uniqueBusIds(rankToBusId.begin(), rankToBusId.end());
#else

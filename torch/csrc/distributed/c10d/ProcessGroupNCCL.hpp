...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> allreduce_sdma(
      std::vector<at::Tensor>& tensors,
      const AllreduceOptions& opts = AllreduceOptions()) override;
#endif
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> allreduce_coalesced_sdma(
      std::vector<at::Tensor>& tensors,
      const AllreduceCoalescedOptions& opts =
          AllreduceCoalescedOptions()) override;
#endif
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> _allgather_base_sdma(
      at::Tensor& outputbuffer,
      at::Tensor& inputbuffer,
      const AllgatherOptions& opts = AllgatherOptions()) override;
#endif
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> allgather_into_tensor_coalesced_sdma(
      std::vector<at::Tensor>& outputs,
      std::vector<at::Tensor>& inputs,
      const AllgatherOptions& opts = AllgatherOptions()) override;
#endif
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> _reduce_scatter_base_sdma(
      at::Tensor& outputTensor,
      at::Tensor& inputTensor,
      const ReduceScatterOptions& opts = ReduceScatterOptions()) override;
#endif
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> reduce_scatter_tensor_coalesced_sdma(
      std::vector<at::Tensor>& outputs,
      std::vector<at::Tensor>& inputs,
      const ReduceScatterOptions& opts = ReduceScatterOptions()) override;
#endif
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> alltoall_base_sdma(
      at::Tensor& outputTensor,
      at::Tensor& inputTensor,
      std::vector<int64_t>& outputSplitSizes,
      std::vector<int64_t>& inputSplitSizes,
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> send_ext(
      std::vector<at::Tensor>& tensors,
      int dstRank,
      int tag) override;

  c10::intrusive_ptr<Work> recv_ext(
      std::vector<at::Tensor>& tensors,
      int srcRank,
      int tag) override;
#endif
...
#ifdef USE_MACA
  c10::intrusive_ptr<Work> allreduce_impl_sdma(
      at::Tensor& tensor,
      const char* profilingTitle = "nccl:all_reduce_sdma",
      const AllreduceOptions& opts = AllreduceOptions());
#endif

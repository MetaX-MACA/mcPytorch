...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::allreduce_impl_sdma(
    at::Tensor& tensor,
    const char* profilingTitle,
    const AllreduceOptions& opts) {
  return collective(
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::allreduce_sdma(
    std::vector<at::Tensor>& tensors,
    const AllreduceOptions& opts) {
  TORCH_CHECK(tensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
  auto tensor = tensors.back();
  ...
  // avoidRecordStreams_ note: collective() will stash tensors.
  return allreduce_impl_sdma(tensor, "nccl:all_reduce", opts);
}
#endif
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::allreduce_coalesced_sdma(
    std::vector<at::Tensor>& tensors,
    const AllreduceCoalescedOptions& opts) {
  auto total_numel = check_gpu_tensors_same_device(tensors);
  TORCH_CHECK(
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      tensors, // outputTensors
      rank_, // rank
      "allreduce_coalesced_sdma", // collective name
      total_numel, // inNelems
      total_numel, // outNelems
      tensors[0].scalar_type(), // dType
      // I'm not sure what in,outSplitSizes mean here.
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::allgather_into_tensor_coalesced_sdma(
    std::vector<at::Tensor>& outputs,
    std::vector<at::Tensor>& inputs,
    const AllgatherOptions& opts) {
  // @lint-ignore CLANGTIDY
  RECORD_PARAM_COMMS_DATA(
      ...
      outputs, // outputTensors
      rank_, // rank
      "allgather_into_tensor_coalesced_sdma", // collective name
      getTensorsNumel(inputs), // inNelems
      getTensorsNumel(outputs), // outNelems
      inputs[0].scalar_type(), // dType
      std::vector<int64_t>(), // inSplitSizes
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::_reduce_scatter_base_sdma(
    at::Tensor& outputTensor,
    at::Tensor& inputTensor,
    const ReduceScatterOptions& opts) {
  if (inputTensor.dtype() != outputTensor.dtype()) {
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      outputTensor, // outputTensor
      rank_, // rank
      "_reduce_scatter_base_sdma", // collective name
      inputTensor.numel(), // inNelems
      tensor.numel(), // outNelems
      tensor.scalar_type(), // dtype
      std::vector<int64_t>(), // inSplitSizes
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::reduce_scatter_tensor_coalesced_sdma(
    std::vector<at::Tensor>& outputs,
    std::vector<at::Tensor>& inputs,
    const ReduceScatterOptions& opts) {
  TORCH_CHECK(
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      outputs, // outputTensors
      rank_, // rank
      "reduce_scatter_tensor_coalesced_sdma", // collective name
      getTensorsNumel(inputs), // inNelems
      getTensorsNumel(outputs), // outNelems
      inputs[0].scalar_type(), // dType
      std::vector<int64_t>(), // inSplitSizes
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::alltoall_base_sdma(
    at::Tensor& outputTensor,
    at::Tensor& inputTensor,
    std::vector<int64_t>& outputSplitSizes,
    std::vector<int64_t>& inputSplitSizes,
  ...
  if (outputSplitSizes.empty() && inputSplitSizes.empty()) {
    ...
    return collective(
        ...
        [&](at::Tensor& input,
          ...
          torch::cuda::nccl::all2all_single_equal_split_sdma(
              input, output, this->getSize(), comm, stream);
          return ncclSuccess;
        },
        OpType::ALLTOALL_BASE,
    ...
    return collective(
        ...
        [&](at::Tensor& input,
          ...
          torch::cuda::nccl::all2all_single_unequal_split_sdma(
              input.data_ptr(),
              send_lengths.data(),
              send_offsets.data(),
              output.data_ptr(),
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::send_ext(
    std::vector<at::Tensor>& tensors,
    int dstRank,
    int /* unused */) {
  TORCH_CHECK(tensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::recv_ext(
    std::vector<at::Tensor>& tensors,
    int srcRank,
    int /* unused */) {
  TORCH_CHECK(tensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
...
#ifdef USE_MACA
c10::intrusive_ptr<Work> ProcessGroupNCCL::_allgather_base_sdma(
    at::Tensor& output_tensor,
    at::Tensor& input_tensor,
    const AllgatherOptions& opts) {
  check_gpu_single_tensor(input_tensor);
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      output_tensor, // outputTensors
      rank_, // rank
      "_allgather_base_sdma", // collective name
      input_tensor.numel(), // inNelems
      output_tensor.numel(), // outNelems
      output_tensor.scalar_type(), // dType
      std::vector<int64_t>(), // inSplitSizes

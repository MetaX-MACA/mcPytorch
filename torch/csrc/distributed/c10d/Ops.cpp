...
#ifdef USE_MACA
  m.def(
      "allreduce_sdma_(Tensor[] tensors, __torch__.torch.classes.c10d.ProcessGroup process_group, __torch__.torch.classes.c10d.ReduceOp reduce_op, Tensor? sparse_indices, int timeout) -> (Tensor[], __torch__.torch.classes.c10d.Work)");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "allreduce_coalesced_sdma_(Tensor[] tensors, __torch__.torch.classes.c10d.ProcessGroup process_group, __torch__.torch.classes.c10d.ReduceOp reduce_op, int timeout) -> __torch__.torch.classes.c10d.Work");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "_allgather_base_sdma_(Tensor output_tensor, Tensor input_tensor, __torch__.torch.classes.c10d.ProcessGroup process_group, bool asyncOp, int timeout) -> (Tensor, __torch__.torch.classes.c10d.Work)");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "allgather_into_tensor_coalesced_sdma_(Tensor[] outputs, Tensor[] inputs, __torch__.torch.classes.c10d.ProcessGroup process_group) -> __torch__.torch.classes.c10d.Work");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "_reduce_scatter_base_sdma_(Tensor output_tensor, Tensor input_tensor, __torch__.torch.classes.c10d.ProcessGroup process_group, __torch__.torch.classes.c10d.ReduceOp reduce_op, bool asyncOp, int timeout) -> (Tensor, __torch__.torch.classes.c10d.Work)");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "reduce_scatter_tensor_coalesced_sdma_(Tensor[] outputs, Tensor[] inputs, __torch__.torch.classes.c10d.ProcessGroup process_group, __torch__.torch.classes.c10d.ReduceOp reduce_op, int timeout) -> __torch__.torch.classes.c10d.Work");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "alltoall_base_sdma_(Tensor output, Tensor input, __torch__.torch.classes.c10d.ProcessGroup process_group, int[] output_split_sizes, int[] input_split_sizes, int timeout) -> __torch__.torch.classes.c10d.Work");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "send_ext(Tensor[] tensors, __torch__.torch.classes.c10d.ProcessGroup process_group, int dst, int tag) -> __torch__.torch.classes.c10d.Work");
#endif
  m.def(
...
#ifdef USE_MACA
  m.def(
      "recv_ext_(Tensor[] tensors, __torch__.torch.classes.c10d.ProcessGroup process_group, int src, int tag) -> __torch__.torch.classes.c10d.Work");
#endif
  m.def(
...
#ifdef USE_MACA
#define IMPL_SEND_SDMA(DEV)                                                        \
  c10::intrusive_ptr<Work> send_ext##DEV(                                         \
      at::TensorList tensors,                                                 \
      const c10::intrusive_ptr<ProcessGroup>& process_group,                  \
      int64_t dstRank,                                                        \
      int64_t tag) {                                                          \
    auto tensor_vec = tensors.vec();                                          \
    return process_group->getBackend(c10::DeviceType::DEV)                    \
        ->send_ext(tensor_vec, static_cast<int>(dstRank), static_cast<int>(tag)); \
  }

IMPL_SEND_SDMA(CPU)
IMPL_SEND_SDMA(CUDA)
IMPL_SEND_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
#define IMPL_RECV_SDMA(DEV)                                                        \
  c10::intrusive_ptr<Work> recv_ext_##DEV(                                        \
      at::TensorList tensors,                                                 \
      const c10::intrusive_ptr<ProcessGroup>& process_group,                  \
      int64_t srcRank,                                                        \
      int64_t tag) {                                                          \
    auto tensor_vec = tensors.vec();                                          \
    return process_group->getBackend(c10::DeviceType::DEV)                    \
        ->recv_ext(tensor_vec, static_cast<int>(srcRank), static_cast<int>(tag)); \
  }

IMPL_RECV_SDMA(CPU)
IMPL_RECV_SDMA(CUDA)
IMPL_RECV_SDMA(PrivateUse1)
#endif
...
// a functional API, so that make_fx can correctly build the dependencies in
// the graph later.
#define IMPL_ALLREDUCE_SDMA(DEV)                                                 \
  std::tuple<std::vector<at::Tensor>, c10::intrusive_ptr<Work>>             \
      allreduce_sdma_##DEV(                                                      \
          at::TensorList tensors,                                           \
          const c10::intrusive_ptr<ProcessGroup>& process_group,            \
...
IMPL_ALLREDUCE_SDMA(CPU)
IMPL_ALLREDUCE_SDMA(CUDA)
IMPL_ALLREDUCE_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
#define IMPL_ALLREDUCE_COALESCED_SDMA(DEV)                             \
  c10::intrusive_ptr<Work> allreduce_coalesced_sdma_##DEV(             \
      at::TensorList tensors,                                     \
      const c10::intrusive_ptr<ProcessGroup>& process_group,      \
      const c10::intrusive_ptr<ReduceOp>& reduce_op,              \
      int64_t timeout) {                                          \
    ...
    opts.timeout = std::chrono::milliseconds(timeout);            \
    return process_group->getBackend(c10::DeviceType::DEV)        \
        ->allreduce_coalesced_sdma(tensor_vec, opts);                  \
  }

IMPL_ALLREDUCE_COALESCED_SDMA(CPU)
IMPL_ALLREDUCE_COALESCED_SDMA(CUDA)
IMPL_ALLREDUCE_COALESCED_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
#define IMPL__ALLGATHER_BASE_SDMA(DEV)                                           \
  std::tuple<at::Tensor, c10::intrusive_ptr<Work>> _allgather_base_sdma_##DEV(   \
      at::Tensor& output_tensor,                                            \
      at::Tensor& input_tensor,                                             \
      const c10::intrusive_ptr<ProcessGroup>& process_group,                \
      bool asyncOp,                                                         \
      int64_t timeout) {                                                    \
    auto work =                                                             \
        process_group->getBackend(c10::DeviceType::DEV) -> _allgather_base_sdma( \
            output_tensor,                                                  \
            input_tensor,                                                   \
...
IMPL__ALLGATHER_BASE_SDMA(CPU)
IMPL__ALLGATHER_BASE_SDMA(CUDA)
IMPL__ALLGATHER_BASE_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
#define IMPL_ALLGATHER_INTO_TENSOR_COALESCED_SDMA(DEV)                       \
  c10::intrusive_ptr<c10d::Work> allgather_into_tensor_coalesced_sdma_##DEV( \
      at::TensorList outputs,                                           \
      at::TensorList inputs,                                            \
      const c10::intrusive_ptr<ProcessGroup>& process_group) {          \
    auto output_vec = outputs.vec();                                    \
    auto input_vec = inputs.vec();                                      \
    return process_group->getBackend(c10::DeviceType::DEV)              \
        ->allgather_into_tensor_coalesced_sdma(output_vec, input_vec);       \
  }

IMPL_ALLGATHER_INTO_TENSOR_COALESCED_SDMA(CPU)
IMPL_ALLGATHER_INTO_TENSOR_COALESCED_SDMA(CUDA)
IMPL_ALLGATHER_INTO_TENSOR_COALESCED_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
#define IMPL__REDUCE_SCATTER_BASE_SDMA(DEV)                                         \
  std::tuple<at::Tensor, c10::intrusive_ptr<Work>> _reduce_scatter_base_sdma_##DEV( \
      at::Tensor& output_tensor,                                               \
      at::Tensor& input_tensor,                                                \
      const c10::intrusive_ptr<ProcessGroup>& process_group,                   \
      const c10::intrusive_ptr<ReduceOp>& reduce_op,                           \
...
IMPL__REDUCE_SCATTER_BASE_SDMA(CPU)
IMPL__REDUCE_SCATTER_BASE_SDMA(CUDA)
IMPL__REDUCE_SCATTER_BASE_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
#define IMPL_REDUCE_SCATTER_TENSOR_COALESCED_SDMA(DEV)                       \
  c10::intrusive_ptr<c10d::Work> reduce_scatter_tensor_coalesced_sdma_##DEV( \
      at::TensorList outputs,                                           \
      at::TensorList inputs,                                            \
      const c10::intrusive_ptr<ProcessGroup>& process_group,            \
      const c10::intrusive_ptr<ReduceOp>& reduce_op,                    \
    ...
    auto input_vec = inputs.vec();                                      \
    return process_group->getBackend(c10::DeviceType::DEV)              \
        ->reduce_scatter_tensor_coalesced_sdma(                              \
            output_vec,                                                 \
            input_vec,                                                  \
            ReduceScatterOptions{                                       \
                *reduce_op.get(), std::chrono::milliseconds(timeout)}); \
...
IMPL_REDUCE_SCATTER_TENSOR_COALESCED_SDMA(CPU)
IMPL_REDUCE_SCATTER_TENSOR_COALESCED_SDMA(CUDA)
IMPL_REDUCE_SCATTER_TENSOR_COALESCED_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
#define IMPL_ALLTOALL_BASE_SDMA(DEV)                                   \
  c10::intrusive_ptr<Work> alltoall_base_sdma_##DEV(                   \
      at::Tensor& output,                                         \
      at::Tensor& input,                                          \
      const c10::intrusive_ptr<ProcessGroup>& process_group,      \
      std::vector<int64_t> output_split_sizes,                    \
...
IMPL_ALLTOALL_BASE_SDMA(CPU)
IMPL_ALLTOALL_BASE_SDMA(CUDA)
IMPL_ALLTOALL_BASE_SDMA(PrivateUse1)
#endif
...
#ifdef USE_MACA
REGISTER_C10D_OP(send_ext)
REGISTER_C10D_OP(recv_ext_)
#endif
...
#ifdef USE_MACA
REGISTER_C10D_OP(alltoall_base_sdma_)
#endif

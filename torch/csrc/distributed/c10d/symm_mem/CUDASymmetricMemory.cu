...
#ifdef USE_MACA
if(!ptr){
  C10_CUDA_CHECK(mcIpcCloseMemHandle(ptr));
  C10_CUDA_CHECK(mcFree(ptr));
}
#else
...
#if defined(USE_MACA)
  void* ptr = nullptr;
  C10_CUDA_CHECK(mcExtMallocWithFlags(&ptr, block_size, mcDeviceMallocFinegrained));
  C10_CUDA_CHECK(cudaIpcGetMemHandle(&handle, ptr));
  // printf("##### device_index: %d, malloc ptr: %p\n", device_idx, ptr);
#else
...
c10::intrusive_ptr<CUDAPeerAllocInfo> make_peer_alloc_info(
  ...
  using BlockHandleType = cudaIpcMemHandle_t;
#else
  using BlockHandleType =
      std::conditional_t<use_fabric_handle, CUmemFabricHandle, int>;
#endif
#if defined(USE_MACA)
  BlockHandleType& block_handle = block->alloc_ref->handle;
#else
...
#ifdef USE_MACA
  std::vector<HandleType> handles = std::move(imported_handles);
#else
  ...
  for (int r = 0; r < world_size; ++r) {
    if (r == rank) {
      ...
      // printf("mcIpcOpenMemHandle== rank: %d, buf[%d]: %p\n", r, r, buffers[r]);
      continue;
    }

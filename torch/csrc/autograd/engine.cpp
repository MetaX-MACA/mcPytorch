...
void Engine::evaluate_function(
  ...
  if (AnomalyMode::is_enabled() && AnomalyMode::should_check_nan()) {
    ...
    for (const auto i : c10::irange(num_outputs)) {
      ...
      if (at::maca::get_maca_check_anomaly_inf() && output.defined() && isinf(output)._is_any_true().item<bool>()) {
        std::stringstream ss;
        ss << "Function '" << fn.name() << "' returned inf values in its " << i
           << "th output.";
        throw std::runtime_error(ss.str());
      }
    }
...
// TODO(mingwei.zhang):
// #if (defined(USE_ROCM) && (ROCM_VERSION < 50000)) || defined(USE_MACA)
#if 1

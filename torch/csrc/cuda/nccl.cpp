...
void all2all_single_equal_split(
  ...
  if (!at::maca::get_maca_disable_alltoall_opt()){
...
void all2all_single_unequal_split(
  ...
  auto comm = to_nccl_comm(_comm);
  if (!at::maca::get_maca_disable_alltoall_opt()){
    NCCL_CHECK(ncclAllToAllv(sendbuff, sendcounts, senddispls, recvbuff, recvcounts, recvdispls, type, comm, stream));
  } else{
    int numranks;
    NCCL_CHECK(ncclCommCount(comm, &numranks));
    NCCL_CHECK(ncclGroupStart());
...
void all2all(
  ...
  if (!at::maca::get_maca_disable_alltoall_opt()){ 
    ...
    std::vector<void*> output_ptr_list;
    std::vector<size_t> input_count_list;
    std::vector<size_t> output_count_list;
    for (const auto r : c10::irange(outputTensors.size())) {
      at::Tensor& input = inputTensors[r];
      at::Tensor& output = outputTensors[r];
      auto* sendbuff = reinterpret_cast<void*>(input.data_ptr());
      input_ptr_list.push_back(sendbuff);
      input_count_list.push_back(input.numel());
      auto* recvbuff = reinterpret_cast<void*>(output.data_ptr());
      output_ptr_list.push_back(recvbuff);
      output_count_list.push_back(output.numel());
    }
    if(output_ptr_list.size() > 0){
      auto inptr = reinterpret_cast<void **>(&input_ptr_list[0]);
      auto outptr = reinterpret_cast<void **>(&output_ptr_list[0]);
      NCCL_CHECK(ncclAllToAlld((const void**)inptr, &input_count_list[0], outptr,  &output_count_list[0], to_nccl_data_type(inputTensors[0]), comm, stream));
    }
  } else{
  NCCL_CHECK(ncclGroupStart());
  for (const auto r : c10::irange(outputTensors.size())) {

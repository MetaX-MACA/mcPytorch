...
#if defined(USE_ROCM) || defined(NCCL_ALLTOALL_SUPPORTED) || defined(USE_MACA)
...
#ifdef USE_MACA
void all2all_single_equal_split_sdma(
    at::Tensor& input,
    at::Tensor& output,
    int size,
    ncclComm_t _comm,
  ...
  // operations issued as a part of the collective (e.g. alltoall) vs those
  // inside traditional p2p operations.
  NCCL_CHECK(mcclAllToAllExt(sendbuff, recvbuff, count, type, comm, stream));
#else
  int numranks = 0;
  NCCL_CHECK(ncclCommCount(comm, &numranks));
  NCCL_CHECK(ncclGroupStart());
  for (const auto r : c10::irange(numranks)) {
    if (_nccl_should_send_recv(count)) {
      NCCL_CHECK(
          mcclSendExt(sendbuff + r * rankdiff, count, type, r, comm, stream));
      NCCL_CHECK(
          mcclRecvExt(recvbuff + r * rankdiff, count, type, r, comm, stream));
    }
  }
#ifndef NCCL_HAS_COMM_NONBLOCKING
  NCCL_CHECK(ncclGroupEnd());
...
void all2all_single_unequal_split(
  ...
  auto comm = to_nccl_comm(_comm);
#if defined(USE_ROCM) || defined(NCCL_ALLTOALLV_SUPPORTED) || defined(USE_MACA)
...
#ifdef USE_MACA
void all2all_single_unequal_split_sdma(
    void* sendbuff,
    const size_t* sendcounts,
    const size_t* senddispls,
    void* recvbuff,

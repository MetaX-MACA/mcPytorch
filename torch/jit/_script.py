...
PYTORCH_DISABLE_JIT_CHECK = True if os.getenv("PYTORCH_DISABLE_JIT_CHECK") else False
...
# NOTICE: remove this skip if supported scaled_dot_product_attention in maca torch, not the flash_attn lib
def skip_jit(obj):
    if isinstance(obj, torch.nn.Module):
        for name, sub_module in obj.__dict__.items():
            qualified_name = str(sub_module)
            if "MultiheadAttention" in qualified_name or "flash_attn_func" in qualified_name:
                return True
    elif inspect.isfunction(obj) or inspect.ismethod(obj):
        source = inspect.getsource(obj)
        if "MultiheadAttention" in source or "flash_attn_func" in source:
            return True
    return False
...
def script(obj, optimize=None, _frames_up=0, _rcb=None,
    ...
    **Scripting an nn.Module**
    ...
    """
    if USE_MACA and not PYTORCH_DISABLE_JIT_CHECK and skip_jit(obj):
        return obj

...
# USE_MACA: custom fuse op
@register_meta(torch.ops.aten.custom_fused_rsm)
def custom_fused_rsm(x, var, weight):
    out_shape = (var.shape[0], weight.shape[0])
    out = x.new_empty(out_shape).to(torch.bfloat16)

    x = torch.ops.aten.split_with_sizes.default(x, [768, 128, 128], -1)
    x = x[0]

...
# USE_MACA
def get_wcuda_gnu_path():
    gnu_path = os.path.join(CUDA_HOME, 'bin', 'gnu') if CUDA_HOME else ""
    if os.path.exists(gnu_path):
        return gnu_path
    assert 0, f"Error: no cu-bridge gnu found! Please ensure cu-bridge is installed, 'CUDA_HOME' or 'CUDA_PATH' env is set to cu-bridge directory."
...
MACA_HOME = os.getenv('MACA_PATH')
with warnings.catch_warnings():
    if not (MACA_HOME and os.path.exists(MACA_HOME)):
        warnings.simplefilter("always", UserWarning)
        warnings.warn(f"Warning: MACA_PATH is not set or your setting is invalid. Current MACA_PATH: {MACA_HOME}", UserWarning)
...
def _find_cuda_home() -> Optional[str]:
    ...
    if cuda_home is None and not USE_MACA:
        # Guess #2
        ...
        if nvcc_path is not None:
            ...
            if not os.path.exists(cuda_home):
                cuda_home = None
    # Guess #4 for cu-bridge
    if cuda_home is None:
        if os.path.exists(MACA_HOME):
            cuda_home = os.path.join(MACA_HOME, 'tools', 'cu-bridge')
...
def _find_rocm_home() -> Optional[str]:
    ...
    if rocm_home and torch.version.hip is None and not USE_MACA:
...
class BuildExtension(build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        if cuda_ext and not IS_HIP_EXTENSION and not USE_MACA:
        ...
        def unix_wrap_single_compile(obj, src, ext, cc_args, extra_postargs, pp_opts) -> None:
            ...
            try:
                ...
                if USE_MACA:
                    original_compiler[0] = os.environ.get('CXX', 'gcc')
                ...
                # USE_MACA
                with EnvGuard('WCUDA_GUN_COMPILER', 'g++'):
        ...
        # there have 2 paths:
        # 1. default, use distutils, it would init compiler base on CXX, so we change it by below code
        # 2. USE_NINJA, it would read compile from get_cxx_compiler()
        if USE_MACA:
            wcuda_gnu = get_wcuda_gnu_path()
            self.compiler.compiler_cxx[0] = wcuda_gnu
            self.compiler.compiler_so[0] = wcuda_gnu
            self.compiler.linker_so[0] = wcuda_gnu
...
def include_paths(device_type: str = "cpu") -> list[str]:
    ...
    if USE_MACA:
        # include cu-bridge wrapper headers.
        paths.append(os.path.join(MACA_HOME, 'include'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcr'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcblas'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcfft'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcsolver'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcdnn'))
        paths.append(os.path.join(MACA_HOME, 'include', 'common'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcsparse'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcrand'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mckl'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mcsml'))
        paths.append(os.path.join(MACA_HOME, 'include', 'mctx'))
        paths.append(os.path.join(MACA_HOME, 'include', 'thrust', 'detail'))
        paths.append(os.path.join(MACA_HOME, 'tools', 'cu-bridge', 'include'))
        paths.append(os.path.join(lib_include, 'ATen', 'native', 'cuda'))   # include maca_kerels
...
def _get_cuda_arch_flags(cflags: Optional[list[str]] = None) -> list[str]:
    ...
    if not _arch_list:
        ...
        # this logic is added to support compile without maca device
        # cubridge needs "-gencode=arch=compute_xx,code=sm_xx" to
        # define __CUDA_ARCH__ and gen compute capability,
        # and user's code may use them
        if USE_MACA and len(arch_list) == 0:
            arch_list = ['8.0']
...
def _join_cuda_home(*paths) -> str:
    ...
    if USE_MACA and len(paths) == 2 and paths[1] == "nvcc":
        if not os.path.exists(os.path.join(CUDA_HOME, *paths)):
            paths_lst = list(paths)
            paths_lst[1] = "cucc"
            paths = tuple(paths_lst)
            if not os.path.exists(os.path.join(CUDA_HOME, *paths)):
                raise EnvironmentError('There is no nvcc or cucc compiler in ' + CUDA_HOME)

...
IS_MACA_EXTENSION = True if (MACA_HOME is not None) else False
...
def _find_cuda_home() -> Optional[str]:
    ...
    if cuda_home is None and IS_MACA_EXTENSION is None:
        # Guess #2
        try:
...
def get_compiler_abi_compatibility_and_version(compiler) -> Tuple[bool, TorchVersion]:
    ...
    if IS_MACOS or IS_MACA_EXTENSION:
...
class BuildExtension(build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        if cuda_ext and not IS_HIP_EXTENSION and not IS_MACA_EXTENSION:
        ...
        def unix_cuda_flags(cflags):
            ...
            if IS_MACA_EXTENSION:
                cflags += COMMON_WCUDA_FLAGS
...
def include_paths(cuda: bool = False) -> List[str]:
    ...
    paths = [
        ...
        os.path.join(lib_include, 'ATen', 'native', 'cuda')
    ]
    if IS_MACA_EXTENSION:
...
def _join_cuda_home(*paths) -> str:
    ...
    if len(paths) == 2 and paths[1] == "nvcc":

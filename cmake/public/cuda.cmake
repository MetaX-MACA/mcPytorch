...
if(CUDA_VERSION VERSION_LESS 12.0 AND NOT USE_MACA)
...
if(CUDA_FOUND)
  ...
  if(NOT CMAKE_CROSSCOMPILING)
    if (NOT USE_MACA)
      ...
      set(cuda_version_from_header ${CUDA_VERSION_STRING})
      message(STATUS "Caffe2: Header version now is: " ${cuda_version_from_header})
    endif()
...
# set MACA_VERSION
if(USE_MACA)
  if(DEFINED ENV{MACA_VERSION})
      set(MACA_VERSION $ENV{MACA_VERSION})
      message(STATUS "Using MACA_VERSION from environment variable: ${MACA_VERSION}")
  else()
      if(DEFINED ENV{MACA_PATH})
        if(EXISTS "$ENV{MACA_PATH}")
          if(EXISTS "$ENV{MACA_PATH}/Version.txt")
              file(READ "$ENV{MACA_PATH}/Version.txt" VERSION_FILE_CONTENT)
              # The format of ${MACA_PATH}/Version.txt should be "Version:xxx"
              string(REGEX MATCH "Version:([^\n]+)" _ ${VERSION_FILE_CONTENT})
              if(CMAKE_MATCH_1)
                  set(MACA_VERSION ${CMAKE_MATCH_1})
                  message(STATUS "Found version in version.txt: ${MACA_VERSION}")
              else()
                  message(WARNING "$ENV{MACA_PATH}/Version.txt does not contain valid Version: information")
              endif()
          else()
              message(WARNING "Version.txt not found in $ENV{MACA_PATH}")
          endif()
        else()
            message(WARNING "Directory $ENV{MACA_PATH} does not exist")
        endif()
      else()
          message(WARNING "Neither MACA_VERSION nor MACA_PATH environment variables are defined")
      endif()
  endif()

  if(DEFINED MACA_VERSION)
      message(STATUS "Final MACA_VERSION value: ${MACA_VERSION}")
  else()
      message(WARNING "MACA_VERSION variable was not set")
  endif()
endif()
...
if(USE_MACA)
  list(APPEND CUDA_NVCC_FLAGS "-mllvm" "-enable-misched=true")
  list(APPEND CUDA_NVCC_FLAGS "-mllvm" "-metaxgpu-disable-bsm-offset=1")
  list(APPEND CUDA_NVCC_FLAGS "-mllvm" "-metaxgpu-inlinescope=2")
  list(APPEND CUDA_NVCC_FLAGS "-mllvm" "-metaxgpu-hoist-arrive=true")
  list(APPEND CUDA_NVCC_FLAGS "-D__FAST_BLOCK_RED__")
  list(APPEND CUDA_NVCC_FLAGS "-compress")
  if(DEFINED ENV{PYTORCH_KEEP_IR_BYTECODE})
    message(STATUS "MACA_KEEP_IR_BYTECODE ON")
    list(APPEND CUDA_NVCC_FLAGS "-aop")
    list(APPEND CUDA_NVCC_FLAGS "--keep")
    list(APPEND CUDA_NVCC_FLAGS "--save-temps")
    list(APPEND CUDA_NVCC_FLAGS "-lineinfo")
  endif()
  if(DEFINED ENV{PYTORCH_ADDITION_KERNEL_INFO})
    list(APPEND CUDA_NVCC_FLAGS "--addition-kernel-info")
  endif()
endif()

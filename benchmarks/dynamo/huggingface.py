...
USE_MACA = (
    True if os.getenv("MACA_PATH") else False
)  # here use MACA_PATH as use mxcc label
if USE_MACA:
    # Modified by MACA: config pretrain model from local path or from default path
    AllenaiLongformerBase_pretrain_path = os.environ.get('ALLENAI_MODEL_PATH', \
    'allenai/longformer-base-4096')
    T5Small_pretrain_path = os.environ.get('T5SMALL_MODEL_PATH', 't5-small')
    DistillGPT2_pretrain_path = os.environ.get('DISTILLGPT2_MODEL_PATH', 'distilgpt2')
    GoogleFnet_pretrain_path = os.environ.get('GOOGLEFNET_MODEL_PATH', 'google/fnet-base')
    YituTechConvBert_pretrain_path = os.environ.get('YITUTECH_MODEL_PATH', 'YituTech/conv-bert-base')
    CamemBert_pretrain_path = os.environ.get('CAMEMBERT_MODEL_PATH', 'camembert-base')

    EXTRA_MODELS = {
        "AllenaiLongformerBase": (
            AutoConfig.from_pretrained(f"{AllenaiLongformerBase_pretrain_path}"),
            AutoModelForMaskedLM,
        ),
        "Reformer": (
            ReformerConfig(),
        ...
        "T5Small": (
            AutoConfig.from_pretrained(f"{T5Small_pretrain_path}"),
            AutoModelForSeq2SeqLM,
        ),
        # "BigBird": (
        ...
        # ),
        "DistillGPT2": (
            AutoConfig.from_pretrained(f"{DistillGPT2_pretrain_path}"),
            AutoModelForCausalLM,
        ),
        "GoogleFnet": (
            AutoConfig.from_pretrained(f"{GoogleFnet_pretrain_path}"),
            AutoModelForMaskedLM,
        ),
        "YituTechConvBert": (
            AutoConfig.from_pretrained(f"{YituTechConvBert_pretrain_path}"),
            AutoModelForMaskedLM,
        ),
        "CamemBert": (
            AutoConfig.from_pretrained(f"{CamemBert_pretrain_path}"),
            AutoModelForMaskedLM,
        ),
    }
else:

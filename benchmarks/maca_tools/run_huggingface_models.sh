...
# config load huggingface pretrain model from local path
export ALLENAI_MODEL_PATH=/netapp/pytorch/huggingface_pretrain_model/allenai/longformer-base-4096
export T5SMALL_MODEL_PATH=/netapp/pytorch/huggingface_pretrain_model/t5-small
export DISTILLGPT2_MODEL_PATH=/netapp/pytorch/huggingface_pretrain_model/distilgpt2
export GOOGLEFNET_MODEL_PATH=/netapp/pytorch/huggingface_pretrain_model/google/fnet-base
export YITUTECH_MODEL_PATH=/netapp/pytorch/huggingface_pretrain_model/YituTech/conv-bert-base
export CAMEMBERT_MODEL_PATH=/netapp/pytorch/huggingface_pretrain_model/camembert-base

MODEL_LIST="../dynamo/huggingface_models_list.txt"
...
if $found; then
    ...
    if [[ $enable_inductor_profile == "true" && $acc_or_perf == "performance" ]]; then
        ...
        TORCHINDUCTOR_UNIQUE_KERNEL_NAMES=1 TORCHINDUCTOR_BENCHMARK_KERNEL=1 python ../dynamo/huggingface.py --$acc_or_perf -dcuda \
        ...
        python ../dynamo/huggingface.py --$acc_or_perf -dcuda \
                            --output=$current_path/dump_graphs/torchbench_fp32_training_perf.csv \
                            --$test_mode --inductor --no-skip --dashboard \
                            --only $model_name --batch-size $batch_size --$precision \
                            2>&1 | tee $current_path/${model_name}_bs${batch_size}_${precision}_${test_mode}.txt_py

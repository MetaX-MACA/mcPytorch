...
from utils.general import check_version, check_yaml, make_divisible, print_args
from utils.torch_utils import (fuse_conv_and_bn, initialize_weights, model_info, profile, scale_img, select_device,
                               time_sync)
from utils.utils import hook_forward_fn, hook_forward_fn_conv, hook_backward_fn, hook_backward_fn_conv
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None, is_check=False, null_hardware=False):  # model, input channels, number of classes
        super().__init__()
    ...
    def register_hook(self):
        list_op = ['Conv2d', 'BatchNorm2d', 'SiLU', 'MaxPool2d', 'Upsample']
        custom_op_list = ['Conv']
                    ...
                    for bottlenect_name_op, bottlenect_type_op in vars(bottlenect_module)['_modules'].items():
                        if bottlenect_type_op.__class__.__name__ in list_op:
                            if self.is_check:
                                ...
                                vars(bottlenect_module)['_modules'][bottlenect_name_op].register_forward_hook(hook_forward_fn)
                                vars(bottlenect_module)['_modules'][bottlenect_name_op].register_backward_hook(hook_backward_fn)
                        elif bottlenect_type_op.__class__.__name__ == 'Sequential':
                            for _, bottleneck in vars(bottlenect_type_op)['_modules'].items():
                                for _, conv in vars(bottleneck)['_modules'].items():
                                    for op_name in vars(conv)['_modules']:
                                        downsample_Seq = vars(conv)['_modules'][op_name]
                                        if self.is_check:
                        ...
                        elif bottlenect_type_op.__class__.__name__ in custom_op_list:
                            for op_name in vars(bottlenect_type_op)['_modules']:
                                downsample_Seq = vars(bottlenect_type_op)['_modules'][op_name]
                                if self.is_check:
                                    downsample_Seq.register_forward_hook(hook_forward_fn_conv)
                                    downsample_Seq.register_backward_hook(hook_backward_fn_conv)
                                else:

...
def main():
    ...
    with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CUDA, torch.profiler.ProfilerActivity.CPU],
                    record_shapes=enable_shape, # enable shape tracing
                    with_flops=True) as profiler:
      torch.manual_seed(0)    # always set seed before run test
      # func(*args, **kwargs)
      with torch.no_grad():
        start_logits, end_logits = model(input_ids, segment_ids, input_mask)
    info = profiler.key_averages(group_by_input_shape=enable_shape).table(sort_by="cuda_time_total", max_name_column_width=1000, row_limit=-1)
    print(info)
    file_path = log_name
    if file_path == "":

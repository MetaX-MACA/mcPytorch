...
USE_MACA = True
profile_enable_shape = True
profile_forward_log = "bert_forward"
profile_backward_log = "bert_backward"


def save_log(log_name, info, profiler):
  file_path = log_name
  dir = "./log/"
  ...
  with open(file_path + ".txt", "w") as f:
      f.writelines(info)
  profiler.export_chrome_trace(file_path + ".json")
            ...
            for step, batch in enumerate(train_iter):
                ...
                if args.profile:
                  with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CUDA, torch.profiler.ProfilerActivity.CPU],
                          record_shapes=profile_enable_shape, # enable shape tracing
                    ...
                    if args.fp16 and USE_MACA:
                      ...
                      start_logits, end_logits = model(input_ids, segment_ids, input_mask)
                  info = profiler.key_averages(group_by_input_shape=profile_enable_shape).table(sort_by="cuda_time_total", max_name_column_width=1000, row_limit=-1)
                  print("-----------------------forward profile------------------", info)
                  save_log(profile_forward_log, info, profiler)
                else:
                  if args.fp16 and USE_MACA:
                    with torch.cuda.amp.autocast():
                      start_logits, end_logits = model(input_ids, segment_ids, input_mask)
